...
3. **Add the `/api/corsair` route** with the adapter for the user's server:
   - Next.js → `toNextJsHandler(corsair, { basePath: '/api/corsair' })`
   - Express → `toExpressHandler(corsair)` mounted at `/api/corsair`
   - Hono → `toHonoHandler(corsair)` at `/api/corsair/*`
   - Any Web-standard runtime (SvelteKit, Remix, Astro, Bun, Deno, Cloudflare Workers) → `createHubRouteHandlers(corsair)` from `corsair/hub`, whose `delivery` is a `(Request) => Promise<Response>` you wire to GET/POST/OPTIONS. Nuxt/Nitro wrap it with h3's `fromWebHandler`.
...
- Dashboard & keys: https://app.corsair.dev
