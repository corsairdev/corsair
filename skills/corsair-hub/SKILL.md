...
4. **Add keys to `.env`** — `CORSAIR_API_KEY`, `CORSAIR_SIGNING_SECRET`, and `CORSAIR_KEK`, copied from the project's **Keys** tab. The same names cover both environments; the key value (`ck_dev_…` / `ck_prod_…`) selects which. Never log or commit secrets.
