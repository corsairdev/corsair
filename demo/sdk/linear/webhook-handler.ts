...
export class LinearWebhookHandler {
  ...
  private handlers: Map<LinearEventName, LinearEventHandler<LinearEventName>[]> = new Map();
  ...
  off<T extends LinearEventName>(
    ...
    const existingHandlers = this.handlers.get(eventName) || [];
    const index = existingHandlers.indexOf(handler as LinearEventHandler<LinearEventName>);
    if (index !== -1) {
  ...
  async handleWebhook(
    ...
    const parsedPayload: LinearWebhookEvent = typeof payload === 'string' 
      ? JSON.parse(payload) 
      : payload;
    ...
    const specificEventName = `${eventType}${action.charAt(0).toUpperCase() + action.slice(1)}` as LinearEventName;
...
export function createWebhookHandler(options?: LinearWebhookHandlerOptions): LinearWebhookHandler {
