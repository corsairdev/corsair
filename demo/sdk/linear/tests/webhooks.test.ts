...
import { LinearWebhookHandler, createWebhookHandler } from '../webhook-handler';
import type {
  IssueCreatedEvent,
  IssueUpdatedEvent,
  IssueDeletedEvent,
...
function extractEventInfo(filename: string): { eventType: string; action: string } {
...
describe('Real Linear Webhook Events - Dynamic Tests', () => {
  ...
  beforeAll(() => {
    ...
    if (allFixtures.length === 0) {
      ...
      console.log('   4. Trigger events in Linear (create/update issues, comments, projects)\n');
    }
  });
    ...
    if (commentFixtures.length === 0) {
      ...
      commentFixtures.forEach((fixture, index) => {
        it(`should process real Comment event #${index + 1} (${fixture.filename})`, async () => {
          ...
          console.log(`   💬 Comment ${event.action}: ${event.data.body?.substring(0, 50)}...`);
        });
      });
    }
  });
  ...
  describe('Webhook Handler Features', () => {
    ...
    it('should verify webhook signature when secret is provided', () => {
      const handler = new LinearWebhookHandler({ webhookSecret: 'test-secret' });
...
describe('Fixture Statistics', () => {
  it('should report available fixtures', () => {
    ...
    if (fixtures.length === 0) {
      ...
      const byEvent = fixtures.reduce((acc, f) => {
        const key = `${f.eventType}.${f.action}`;
        acc[key] = (acc[key] || 0) + 1;
        return acc;
      }, {} as Record<string, number>);
