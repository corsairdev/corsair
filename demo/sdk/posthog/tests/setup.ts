...
beforeAll(() => {
	OpenAPI.BASE = process.env.POSTHOG_API_HOST || 'https://app.posthog.com';
	...
	console.log('PostHog API Test Configuration:');
...
export function requireToken(): boolean {
	if (!isTokenConfigured()) {
		console.warn('POSTHOG_API_KEY not set - skipping test');
		return true;
	}
	return false;
}
