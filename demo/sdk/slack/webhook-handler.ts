...
export class SlackWebhookHandler {
  ...
  private handlers: Map<SlackEventName, SlackEventHandler<SlackEventName>[]> = new Map();
  ...
  off<T extends SlackEventName>(
    ...
    const existingHandlers = this.handlers.get(eventName) || [];
    const index = existingHandlers.indexOf(handler as SlackEventHandler<SlackEventName>);
    if (index !== -1) {
  ...
  async handleWebhook(
    ...
    const parsedPayload: SlackWebhookPayload = typeof payload === 'string' 
      ? JSON.parse(payload) 
      : payload;
...
export function createWebhookHandler(options?: SlackWebhookHandlerOptions): SlackWebhookHandler {
