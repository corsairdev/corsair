...
import { SpotifyArtist, SpotifyAlbum, SpotifyTracksResponse } from "@/lib/types";
...
export async function getAllAlbumsByArtistId(artistId: string): Promise<SpotifyAlbum[]> {
  await delay();
...
export async function addArtistGenre(artistId: string, genre: string): Promise<SpotifyArtist | null> {
  await delay();
  const artist = artists.find((a) => a.id === artistId);
...
export async function removeArtistGenre(artistId: string, genre: string): Promise<SpotifyArtist | null> {
  await delay();
  const artist = artists.find((a) => a.id === artistId);
  if (artist && artist.genres) {
