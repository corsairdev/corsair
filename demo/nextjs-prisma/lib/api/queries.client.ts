...
import { SpotifyArtist, SpotifyAlbum, SpotifyTrack } from "@/lib/types";
...
export function useArtistById(id: string | null): UseQueryResult<SpotifyArtist | null> {
  return useQuery(() => (id ? getArtistById(id) : Promise.resolve(null)), `artist-${id}`);
}

export function useAlbumById(id: string | null): UseQueryResult<SpotifyAlbum | null> {
  return useQuery(() => (id ? getAlbumById(id) : Promise.resolve(null)), `album-${id}`);
}

export function useAlbumsByArtistId(artistId: string | null): UseQueryResult<SpotifyAlbum[]> {
  return useQuery(
...
export function useTracksByAlbumId(albumId: string | null): UseQueryResult<SpotifyTrack[]> {
  return useQuery(
...
export function useTracksByArtistId(artistId: string | null): UseQueryResult<SpotifyTrack[]> {
  return useQuery(
