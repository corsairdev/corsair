---
title: Database
description: "CollegeFootballData local sync: searchable entities, `.search()` filters, and operators."
---
...
const rows = await tenant.collegefootballdata.db.coaches.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.collegefootballdata.db.conferences.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.collegefootballdata.db.teams.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.collegefootballdata.db.venues.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
