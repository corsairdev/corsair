...
Synced entities support `tenant.googlecalendar.db.<entity>.search()` and `.list()`: `calendars`, `events`.

See [Database](/plugins/googlecalendar/database) for filters and operators.
...
See [Webhooks](/plugins/googlecalendar/webhooks) for every event path and payload shape, and [Webhooks concept](/concepts/webhooks) for routing.
