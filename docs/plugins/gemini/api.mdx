...
<Info>
The Corsair-only `stripFences?: boolean` input controls formatting of the
convenience `text` field and is removed before the request is sent to Gemini.
Each response part is typed as `thought?: boolean`; parts marked `thought: true`
are excluded from `text`, while all other text parts from the first candidate
are concatenated in response order.
</Info>
...
| `stripFences` | `boolean` | No | Strip markdown code fences from the convenience text field |
...
| `text` | `string` | No | Convenience field: concatenated non-thought text from the first candidate |
