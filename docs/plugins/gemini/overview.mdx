...
Use **Gemini** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/gemini
```
```bash yarn
yarn add corsair @corsair-dev/gemini
```
```bash pnpm
pnpm install corsair @corsair-dev/gemini
```
```bash bun
bun add corsair @corsair-dev/gemini
```
</CodeGroup>
...
See the full list on the [API](/plugins/gemini/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/gemini/api">
    Every `gemini.api.*` operation with input and output types.
  </Card>
