---
title: API
description: "API reference for Uniswap Trading API: every `uniswapapi.api.*` operation with input and output types."
---
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.uniswapapi.api.delegation.check({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.uniswapapi.api.order.getStatus({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.uniswapapi.api.quote.get({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.uniswapapi.api.swap.getStatus({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.uniswapapi.api.swappableTokens.get({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.uniswapapi.api.transaction.encode7702({});
```
