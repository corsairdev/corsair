---
title: Overview
description: "Customer.io plugin for Corsair"
---

Use **Customer.io** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/customerio
```
```bash yarn
yarn add corsair @corsair-dev/customerio
```
```bash pnpm
pnpm install corsair @corsair-dev/customerio
```
```bash bun
bun add corsair @corsair-dev/customerio
```
</CodeGroup>
...
await tenant.customerio.api.broadcasts.getTrigger({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.customerio.api.broadcasts.trigger({});
```


See the full list on the [API](/plugins/customerio/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/customerio/api">
    Every `customerio.api.*` operation with input and output types.
  </Card>
