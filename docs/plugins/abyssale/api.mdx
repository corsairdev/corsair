...
```ts
const tenant = corsair.withTenant('acme');
await tenant.abyssale.api.designs.list({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.abyssale.api.fonts.list({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.abyssale.api.generation.image({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.abyssale.api.generation.status({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.abyssale.api.projects.create({});
```
