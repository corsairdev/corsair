---
title: Overview
description: "WebScraping.AI plugin for Corsair"
---

Use **WebScraping.AI** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/webscrapingai
```
```bash yarn
yarn add corsair @corsair-dev/webscrapingai
```
```bash pnpm
pnpm install corsair @corsair-dev/webscrapingai
```
```bash bun
bun add corsair @corsair-dev/webscrapingai
```
</CodeGroup>
...
import { webscrapingai } from '@corsair-dev/webscrapingai';
...
await tenant.webscrapingai.api.account.getInfo({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/webscrapingai/api) reference._


See the full list on the [API](/plugins/webscrapingai/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/webscrapingai/api">
    Every `webscrapingai.api.*` operation with input and output types.
  </Card>
