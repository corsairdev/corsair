---
title: Overview
description: "Token Metrics plugin for Corsair"
---

Use **Token Metrics** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/tokenmetrics
```
```bash yarn
yarn add corsair @corsair-dev/tokenmetrics
```
```bash pnpm
pnpm install corsair @corsair-dev/tokenmetrics
```
```bash bun
bun add corsair @corsair-dev/tokenmetrics
```
</CodeGroup>
...
import { tokenmetrics } from '@corsair-dev/tokenmetrics';
...
await tenant.tokenmetrics.api.market.getPrice({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/tokenmetrics/api) reference._


See the full list on the [API](/plugins/tokenmetrics/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/tokenmetrics/api">
    Every `tokenmetrics.api.*` operation with input and output types.
  </Card>
