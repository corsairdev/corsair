---
title: API
description: "API reference for OnePassword: every `onepassword.api.*` operation with input and output types."
---
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.onepassword.api.items.delete({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.onepassword.api.items.list({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.onepassword.api.items.update({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.onepassword.api.vaults.get({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.onepassword.api.vaults.list({});
```
