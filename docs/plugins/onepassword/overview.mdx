...
Use **OnePassword** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 7 typed API operations
- 2 synced entities (`items`, `vaults`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/onepassword
```
```bash yarn
yarn add corsair @corsair-dev/onepassword
```
```bash pnpm
pnpm install corsair @corsair-dev/onepassword
```
```bash bun
bun add corsair @corsair-dev/onepassword
```
</CodeGroup>
...
await tenant.onepassword.api.items.get({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.onepassword.api.items.create({});
```


See the full list on the [API](/plugins/onepassword/api) page.

## Query synced data

Synced entities support `tenant.onepassword.db.<entity>.search()` and `.list()`: `items`, `vaults`.

See [Database](/plugins/onepassword/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/onepassword/api">
    Every `onepassword.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/onepassword/database">
