...
Use **AsinDataApi** through Corsair: one client, typed API calls, local DB sync, and incoming webhooks.

**What you get:**

- 22 typed API operations
- 4 synced entities (`collections`, `destinations`, `requests`, `resultSets`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/asindataapi
```
```bash yarn
yarn add corsair @corsair-dev/asindataapi
```
```bash pnpm
pnpm install corsair @corsair-dev/asindataapi
```
```bash bun
bun add corsair @corsair-dev/asindataapi
```
</CodeGroup>
...
await tenant.asindataapi.api.categories.get({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.asindataapi.api.collections.create({});
```


See the full list on the [API](/plugins/asindataapi/api) page.

## Query synced data

Synced entities support `tenant.asindataapi.db.<entity>.search()` and `.list()`: `collections`, `destinations`, `requests`, `resultSets`.

See [Database](/plugins/asindataapi/database) for filters and operators.
...
See [Webhooks](/plugins/asindataapi/webhooks) for every event path and payload shape, and [Webhooks concept](/concepts/webhooks) for routing.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/asindataapi/api">
    Every `asindataapi.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/asindataapi/database">
    Synced entities, search filters, and operators.
  </Card>
  <Card title="Webhooks" href="/plugins/asindataapi/webhooks">
