---
title: Database
description: "AsinDataApi local sync: searchable entities, `.search()` filters, and operators."
---
...
const rows = await tenant.asindataapi.db.collections.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.asindataapi.db.destinations.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.asindataapi.db.requests.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.asindataapi.db.resultSets.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
