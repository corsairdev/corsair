...
Use **DigitalOcean** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 47 typed API operations
- 3 synced entities (`databases`, `droplets`, `volumes`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/digitalocean
```
```bash yarn
yarn add corsair @corsair-dev/digitalocean
```
```bash pnpm
pnpm install corsair @corsair-dev/digitalocean
```
```bash bun
bun add corsair @corsair-dev/digitalocean
```
</CodeGroup>
...
See the full list on the [API](/plugins/digitalocean/api) page.

## Query synced data

Synced entities support `tenant.digitalocean.db.<entity>.search()` and `.list()`: `databases`, `droplets`, `volumes`.

See [Database](/plugins/digitalocean/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/digitalocean/api">
    Every `digitalocean.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/digitalocean/database">
