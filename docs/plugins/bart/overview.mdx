...
Use **Bart** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 13 typed API operations
- 3 synced entities (`advisories`, `routes`, `stations`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/bart
```
```bash yarn
yarn add corsair @corsair-dev/bart
```
```bash pnpm
pnpm install corsair @corsair-dev/bart
```
```bash bun
bun add corsair @corsair-dev/bart
```
</CodeGroup>
...
await tenant.bart.api.advisories.elevators({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/bart/api) reference._


See the full list on the [API](/plugins/bart/api) page.

## Query synced data

Synced entities support `tenant.bart.db.<entity>.search()` and `.list()`: `advisories`, `routes`, `stations`.

See [Database](/plugins/bart/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/bart/api">
    Every `bart.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/bart/database">
