---
title: Overview
description: "SafetyCulture plugin for Corsair"
---

Use **SafetyCulture** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/safetyculture
```
```bash yarn
yarn add corsair @corsair-dev/safetyculture
```
```bash pnpm
pnpm install corsair @corsair-dev/safetyculture
```
```bash bun
bun add corsair @corsair-dev/safetyculture
```
</CodeGroup>
...
await tenant.safetyculture.api.actions.list({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/safetyculture/api) reference._


See the full list on the [API](/plugins/safetyculture/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/safetyculture/api">
    Every `safetyculture.api.*` operation with input and output types.
  </Card>
