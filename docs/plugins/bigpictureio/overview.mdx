---
title: Overview
description: "BigPicture.io plugin for Corsair"
---

Use **BigPicture.io** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/bigpictureio
```
```bash yarn
yarn add corsair @corsair-dev/bigpictureio
```
```bash pnpm
pnpm install corsair @corsair-dev/bigpictureio
```
```bash bun
bun add corsair @corsair-dev/bigpictureio
```
</CodeGroup>
...
import { bigpictureio } from '@corsair-dev/bigpictureio';
...
await tenant.bigpictureio.api.company.find({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/bigpictureio/api) reference._


See the full list on the [API](/plugins/bigpictureio/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/bigpictureio/api">
    Every `bigpictureio.api.*` operation with input and output types.
  </Card>
