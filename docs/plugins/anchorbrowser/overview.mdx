...
Use **Anchor Browser** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 64 typed API operations
- 3 synced entities (`profiles`, `sessions`, `tasks`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/anchorbrowser
```
```bash yarn
yarn add corsair @corsair-dev/anchorbrowser
```
```bash pnpm
pnpm install corsair @corsair-dev/anchorbrowser
```
```bash bun
bun add corsair @corsair-dev/anchorbrowser
```
</CodeGroup>
...
See the full list on the [API](/plugins/anchorbrowser/api) page.

## Query synced data

Synced entities support `tenant.anchorbrowser.db.<entity>.search()` and `.list()`: `profiles`, `sessions`, `tasks`.

See [Database](/plugins/anchorbrowser/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/anchorbrowser/api">
    Every `anchorbrowser.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/anchorbrowser/database">
