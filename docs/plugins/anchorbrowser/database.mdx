...
const rows = await tenant.anchorbrowser.db.profiles.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.anchorbrowser.db.sessions.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.anchorbrowser.db.tasks.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
