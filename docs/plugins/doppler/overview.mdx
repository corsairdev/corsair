...
Use **Doppler** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 62 typed API operations
- 5 synced entities (`configs`, `environments`, `projects`, `webhooks`, `workplace`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/doppler
```
```bash yarn
yarn add corsair @corsair-dev/doppler
```
```bash pnpm
pnpm install corsair @corsair-dev/doppler
```
```bash bun
bun add corsair @corsair-dev/doppler
```
</CodeGroup>
...
See the full list on the [API](/plugins/doppler/api) page.

## Query synced data

Synced entities support `tenant.doppler.db.<entity>.search()` and `.list()`: `configs`, `environments`, `projects`, `webhooks`, `workplace`.

See [Database](/plugins/doppler/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/doppler/api">
    Every `doppler.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/doppler/database">
