---
title: Overview
description: "Faraday plugin for Corsair"
---

Use **Faraday** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 120 typed API operations
- 13 synced entities (`accounts`, `cohorts`, `connections`, `datasets`, `graphEdges`, `outcomes`, and 7 more) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/faraday
```
```bash yarn
yarn add corsair @corsair-dev/faraday
```
```bash pnpm
pnpm install corsair @corsair-dev/faraday
```
```bash bun
bun add corsair @corsair-dev/faraday
```
</CodeGroup>
...
import { faraday } from '@corsair-dev/faraday';
...
See the full list on the [API](/plugins/faraday/api) page.

## Query synced data

Synced entities support `tenant.faraday.db.<entity>.search()` and `.list()`: `accounts`, `cohorts`, `connections`, `datasets`, `graphEdges`, `outcomes`, `personaSets`, `places`, `scopes`, `streams`, `targets`, `traits`, `webhookEndpoints`.

See [Database](/plugins/faraday/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/faraday/api">
    Every `faraday.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/faraday/database">
