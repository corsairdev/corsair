---
title: Overview
description: "AI-powered sales engagement platform for contacts, multichannel sequences, email accounts, and sending schedules."
---

Use **Reply.io** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/replyio
```
```bash yarn
yarn add corsair @corsair-dev/replyio
```
```bash pnpm
pnpm install corsair @corsair-dev/replyio
```
```bash bun
bun add corsair @corsair-dev/replyio
```
</CodeGroup>
...
import { replyio } from '@corsair-dev/replyio';
...
await tenant.replyio.api.sequences.list({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.replyio.api.contacts.create({ email: 'prospect@example.com', firstName: 'Ada', lastName: 'Lovelace' });
```


See the full list on the [API](/plugins/replyio/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/replyio/api">
    Every `replyio.api.*` operation with input and output types.
  </Card>
