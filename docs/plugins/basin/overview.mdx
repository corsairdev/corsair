...
Use **Basin** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 26 typed API operations
- 6 synced entities (`domains`, `forms`, `formViews`, `projects`, `submissions`, `webhooks`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/basin
```
```bash yarn
yarn add corsair @corsair-dev/basin
```
```bash pnpm
pnpm install corsair @corsair-dev/basin
```
```bash bun
bun add corsair @corsair-dev/basin
```
</CodeGroup>
...
See the full list on the [API](/plugins/basin/api) page.

## Query synced data

Synced entities support `tenant.basin.db.<entity>.search()` and `.list()`: `domains`, `forms`, `formViews`, `projects`, `submissions`, `webhooks`.

See [Database](/plugins/basin/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/basin/api">
    Every `basin.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/basin/database">
