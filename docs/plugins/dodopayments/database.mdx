...
const rows = await tenant.dodopayments.db.customers.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.dodopayments.db.payments.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.dodopayments.db.refunds.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.dodopayments.db.subscriptions.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
