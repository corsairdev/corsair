...
Use **Ayrshare** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 4 typed API operations
- 2 synced entities (`autoSchedules`, `posts`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/ayrshare
```
```bash yarn
yarn add corsair @corsair-dev/ayrshare
```
```bash pnpm
pnpm install corsair @corsair-dev/ayrshare
```
```bash bun
bun add corsair @corsair-dev/ayrshare
```
</CodeGroup>
...
await tenant.ayrshare.api.autoSchedule.list({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.ayrshare.api.autoSchedule.set({});
```


See the full list on the [API](/plugins/ayrshare/api) page.

## Query synced data

Synced entities support `tenant.ayrshare.db.<entity>.search()` and `.list()`: `autoSchedules`, `posts`.

See [Database](/plugins/ayrshare/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/ayrshare/api">
    Every `ayrshare.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/ayrshare/database">
