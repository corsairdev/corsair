---
title: Overview
description: "Tinyurl plugin for Corsair"
---

Use **Tinyurl** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 2 typed API operations
- 1 synced entity (`links`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/tinyurl
```
```bash yarn
yarn add corsair @corsair-dev/tinyurl
```
```bash pnpm
pnpm install corsair @corsair-dev/tinyurl
```
```bash bun
bun add corsair @corsair-dev/tinyurl
```
</CodeGroup>
...
import { tinyurl } from '@corsair-dev/tinyurl';
...
await tenant.tinyurl.api.urls.list({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.tinyurl.api.urls.create({});
```


See the full list on the [API](/plugins/tinyurl/api) page.

## Query synced data

Synced entities support `tenant.tinyurl.db.<entity>.search()` and `.list()`: `links`.

See [Database](/plugins/tinyurl/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/tinyurl/api">
    Every `tinyurl.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/tinyurl/database">
