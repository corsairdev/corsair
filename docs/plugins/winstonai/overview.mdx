---
title: Overview
description: "Winston AI plugin for Corsair"
---

Use **Winston AI** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/winstonai
```
```bash yarn
yarn add corsair @corsair-dev/winstonai
```
```bash pnpm
pnpm install corsair @corsair-dev/winstonai
```
```bash bun
bun add corsair @corsair-dev/winstonai
```
</CodeGroup>
...
import { winstonai } from '@corsair-dev/winstonai';
...
await tenant.winstonai.api.detect.aiImage({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/winstonai/api) reference._


See the full list on the [API](/plugins/winstonai/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/winstonai/api">
    Every `winstonai.api.*` operation with input and output types.
  </Card>
