---
title: Overview
description: "Analyze Retell AI calls and chats with transcripts, recordings, and post-call analysis."
---

Use **Retell AI** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 4 typed API operations
- 2 synced entities (`calls`, `chats`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/retellai
```
```bash yarn
yarn add corsair @corsair-dev/retellai
```
```bash pnpm
pnpm install corsair @corsair-dev/retellai
```
```bash bun
bun add corsair @corsair-dev/retellai
```
</CodeGroup>
...
import { retellai } from '@corsair-dev/retellai';
...
await tenant.retellai.api.calls.get({ id: 'call-id' });
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/retellai/api) reference._


See the full list on the [API](/plugins/retellai/api) page.

## Query synced data

Synced entities support `tenant.retellai.db.<entity>.search()` and `.list()`: `calls`, `chats`.

See [Database](/plugins/retellai/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/retellai/api">
    Every `retellai.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/retellai/database">
