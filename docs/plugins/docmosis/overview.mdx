---
title: Overview
description: "Generate documents from templates with Docmosis Cloud, including template/image management and environment diagnostics."
---

Use **Docmosis** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/docmosis
```
```bash yarn
yarn add corsair @corsair-dev/docmosis
```
```bash pnpm
pnpm install corsair @corsair-dev/docmosis
```
```bash bun
bun add corsair @corsair-dev/docmosis
```
</CodeGroup>
...
import { docmosis } from '@corsair-dev/docmosis';
...
await tenant.docmosis.api.admin.environmentReady({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.docmosis.api.templates.render({
	templateName: '/samples/WelcomeTemplate.docx',
	data: { title: 'Hello from Corsair' },
	outputName: 'output.pdf',
});
```


See the full list on the [API](/plugins/docmosis/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/docmosis/api">
    Every `docmosis.api.*` operation with input and output types.
  </Card>
