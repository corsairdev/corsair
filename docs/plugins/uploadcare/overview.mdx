---
title: Overview
description: "Uploadcare plugin for Corsair"
---

Use **Uploadcare** through Corsair: one client, typed API calls, and incoming webhooks.
...
await tenant.uploadcare.api.files.get({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.uploadcare.api.files.batchDelete({});
```
