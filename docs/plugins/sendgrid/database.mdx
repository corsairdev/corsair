---
title: Database
description: "SendGrid v3 local sync: searchable entities, `.search()` filters, and operators."
---

The SendGrid v3 plugin syncs data locally. Use `corsair.sendgrid.db.<entity>.search({ data, limit?, offset? })` with the filters listed per entity.
...
const rows = await tenant.sendgrid.db.bounces.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.sendgrid.db.contacts.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.sendgrid.db.lists.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.sendgrid.db.senders.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
