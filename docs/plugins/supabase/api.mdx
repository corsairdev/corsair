---
title: API
description: "API reference for Supabase: every `supabase.api.*` operation with input and output types."
---

Every `supabase.api.*` operation is listed below with parameter shapes and return types from the plugin Zod schemas.

<Info>
**New to Corsair?** See [API access](/concepts/api), [authentication](/concepts/auth), and [error handling](/concepts/error-handling).
</Info>

## Advisors

### getPerformanceAdvisors

`advisors.getPerformanceAdvisors`

Get performance advisors

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.advisors.getPerformanceAdvisors({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getSecurityAdvisors

`advisors.getSecurityAdvisors`

Get security advisors

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.advisors.getSecurityAdvisors({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

## Analytics

### getProjectLogs

`analytics.getProjectLogs`

Get project logs

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.analytics.getProjectLogs({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

## Auth

### createProjectSigningKey

`auth.createProjectSigningKey`

Create a project signing key

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.auth.createProjectSigningKey({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### createSsoProvider

`auth.createSsoProvider`

Create an SSO provider

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.auth.createSsoProvider({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### createThirdPartyAuthIntegration

`auth.createThirdPartyAuthIntegration`

Create a third-party auth integration

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.auth.createThirdPartyAuthIntegration({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### deleteSsoProvider

`auth.deleteSsoProvider`

Remove an SSO provider

**Risk:** `destructive` · **Irreversible**

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.auth.deleteSsoProvider({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | Yes | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### deleteThirdPartyAuthIntegration

`auth.deleteThirdPartyAuthIntegration`

Delete a third-party auth integration

**Risk:** `destructive` · **Irreversible**

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.auth.deleteThirdPartyAuthIntegration({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | Yes | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getLegacySigningKey

`auth.getLegacySigningKey`

Get legacy signing key

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.auth.getLegacySigningKey({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getProjectAuthConfig

`auth.getProjectAuthConfig`

Get project auth config

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.auth.getProjectAuthConfig({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getProjectSigningKeys

`auth.getProjectSigningKeys`

Get project signing keys

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.auth.getProjectSigningKeys({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getSsoProvider

`auth.getSsoProvider`

Get an SSO provider

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.auth.getSsoProvider({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | Yes | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getThirdPartyIntegration

`auth.getThirdPartyIntegration`

Get a third-party auth integration

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.auth.getThirdPartyIntegration({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | Yes | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### listSsoProviders

`auth.listSsoProviders`

List SSO providers

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.auth.listSsoProviders({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### listThirdPartyAuthIntegrations

`auth.listThirdPartyAuthIntegrations`

List third-party auth integrations

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.auth.listThirdPartyAuthIntegrations({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### updateProjectAuthConfig

`auth.updateProjectAuthConfig`

Update project auth config

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.auth.updateProjectAuthConfig({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### updateSsoProvider

`auth.updateSsoProvider`

Update an SSO provider

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.auth.updateSsoProvider({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | Yes | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

## Database

### applyMigration

`database.applyMigration`

Apply a database migration

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.applyMigration({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### createLoginRole

`database.createLoginRole`

Create a CLI login role

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.createLoginRole({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### createReadReplica

`database.createReadReplica`

Set up a read replica

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.createReadReplica({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### deleteLoginRoles

`database.deleteLoginRoles`

Delete CLI login roles

**Risk:** `destructive` · **Irreversible**

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.deleteLoginRoles({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### disableProjectReadonly

`database.disableProjectReadonly`

Temporarily disable project read-only mode

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.disableProjectReadonly({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### enableDatabaseWebhooks

`database.enableDatabaseWebhooks`

Enable database webhooks

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.enableDatabaseWebhooks({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### generateTypescriptTypes

`database.generateTypescriptTypes`

Generate TypeScript database types

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.generateTypescriptTypes({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getDatabaseMetadata

`database.getDatabaseMetadata`

Get database metadata

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.getDatabaseMetadata({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getJitAccessConfig

`database.getJitAccessConfig`

Get JIT access config

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.getJitAccessConfig({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getMigration

`database.getMigration`

Get a migration

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.getMigration({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | Yes | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getProjectPgbouncerConfig

`database.getProjectPgbouncerConfig`

Get project PgBouncer config

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.getProjectPgbouncerConfig({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getProjectPostgresConfig

`database.getProjectPostgresConfig`

Get project postgres config

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.getProjectPostgresConfig({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getProjectReadonlyModeStatus

`database.getProjectReadonlyModeStatus`

Get project read-only mode status

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.getProjectReadonlyModeStatus({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getProjectSslEnforcementConfig

`database.getProjectSslEnforcementConfig`

Get project SSL enforcement config

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.getProjectSslEnforcementConfig({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getProjectSupavisorConfig

`database.getProjectSupavisorConfig`

Get project Supavisor config

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.getProjectSupavisorConfig({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getSqlSnippet

`database.getSqlSnippet`

Get a SQL snippet

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.getSqlSnippet({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | No | — |
| `id` | `string` | Yes | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getTableSchemas

`database.getTableSchemas`

Get database table schemas

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.getTableSchemas({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### listBackups

`database.listBackups`

List project database backups

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.listBackups({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### listMigrationHistory

`database.listMigrationHistory`

List migration history

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.listMigrationHistory({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### listSqlSnippets

`database.listSqlSnippets`

List SQL snippets

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.listSqlSnippets({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | No | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### listTables

`database.listTables`

List database tables

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.listTables({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### patchMigration

`database.patchMigration`

Patch a migration

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.patchMigration({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | Yes | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### removeReadReplica

`database.removeReadReplica`

Remove a read replica

**Risk:** `destructive` · **Irreversible**

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.removeReadReplica({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### restorePitrBackup

`database.restorePitrBackup`

Restore database PITR backup

**Risk:** `destructive` · **Irreversible**

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.restorePitrBackup({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### runReadOnlyQuery

`database.runReadOnlyQuery`

Execute a read-only database query

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.runReadOnlyQuery({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### runSqlQuery

`database.runSqlQuery`

Execute a project database query

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.runSqlQuery({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### selectFromTable

`database.selectFromTable`

Select rows from a database table

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.selectFromTable({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### updateDatabasePassword

`database.updateDatabasePassword`

Update database password

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.updateDatabasePassword({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### updateJitAccessConfig

`database.updateJitAccessConfig`

Update JIT access config

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.updateJitAccessConfig({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### updateProjectPostgresConfig

`database.updateProjectPostgresConfig`

Update project postgres config

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.updateProjectPostgresConfig({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### updateProjectSupavisorConfig

`database.updateProjectSupavisorConfig`

Update project Supavisor config

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.updateProjectSupavisorConfig({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### updateSslEnforcementConfig

`database.updateSslEnforcementConfig`

Update SSL enforcement config

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.updateSslEnforcementConfig({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### upsertMigration

`database.upsertMigration`

Upsert a migration

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.database.upsertMigration({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

## Domains

### activateCustomHostname

`domains.activateCustomHostname`

Activate a configured custom hostname

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.domains.activateCustomHostname({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### activateVanitySubdomain

`domains.activateVanitySubdomain`

Activate a vanity subdomain for a project

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.domains.activateVanitySubdomain({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### checkVanitySubdomainAvailability

`domains.checkVanitySubdomainAvailability`

Check vanity subdomain availability

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.domains.checkVanitySubdomainAvailability({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### deleteCustomHostnameConfig

`domains.deleteCustomHostnameConfig`

Delete custom hostname configuration

**Risk:** `destructive` · **Irreversible**

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.domains.deleteCustomHostnameConfig({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### deleteProjectVanitySubdomain

`domains.deleteProjectVanitySubdomain`

Delete a project vanity subdomain

**Risk:** `destructive` · **Irreversible**

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.domains.deleteProjectVanitySubdomain({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getProjectCustomHostnameConfig

`domains.getProjectCustomHostnameConfig`

Get a project custom hostname configuration

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.domains.getProjectCustomHostnameConfig({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getVanitySubdomainConfig

`domains.getVanitySubdomainConfig`

Get current vanity subdomain config

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.domains.getVanitySubdomainConfig({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### updateProjectCustomHostname

`domains.updateProjectCustomHostname`

Update project custom hostname

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.domains.updateProjectCustomHostname({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### verifyCustomHostnameDns

`domains.verifyCustomHostnameDns`

Reverify custom hostname DNS

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.domains.verifyCustomHostnameDns({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

## Edge Functions

### createFunction

`edgeFunctions.createFunction`

Create an Edge Function

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.edgeFunctions.createFunction({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### deleteFunction

`edgeFunctions.deleteFunction`

Delete an Edge Function

**Risk:** `destructive` · **Irreversible**

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.edgeFunctions.deleteFunction({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | Yes | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### deployFunction

`edgeFunctions.deployFunction`

Deploy an Edge Function

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.edgeFunctions.deployFunction({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getFunction

`edgeFunctions.getFunction`

Retrieve an Edge Function

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.edgeFunctions.getFunction({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | Yes | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getFunctionBody

`edgeFunctions.getFunctionBody`

Retrieve an Edge Function body

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.edgeFunctions.getFunctionBody({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | Yes | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### invokeEdgeFunction

`edgeFunctions.invokeEdgeFunction`

Invoke an Edge Function

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.edgeFunctions.invokeEdgeFunction({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | Yes | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### listFunctions

`edgeFunctions.listFunctions`

List Edge Functions

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.edgeFunctions.listFunctions({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### updateFunction

`edgeFunctions.updateFunction`

Update an Edge Function

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.edgeFunctions.updateFunction({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | Yes | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### updateFunctions

`edgeFunctions.updateFunctions`

Bulk update Edge Functions

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.edgeFunctions.updateFunctions({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

## Environments

### countActionRuns

`environments.countActionRuns`

Count action runs

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.environments.countActionRuns({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### createDatabaseBranch

`environments.createDatabaseBranch`

Create a database branch

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.environments.createDatabaseBranch({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### deleteDatabaseBranch

`environments.deleteDatabaseBranch`

Delete a database branch

**Risk:** `destructive` · **Irreversible**

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.environments.deleteDatabaseBranch({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | No | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | Yes | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### disablePreviewBranching

`environments.disablePreviewBranching`

Disable preview branching

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.environments.disablePreviewBranching({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getActionRun

`environments.getActionRun`

Get action run status

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.environments.getActionRun({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | Yes | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getActionRunLogs

`environments.getActionRunLogs`

Get action run logs

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.environments.getActionRunLogs({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | Yes | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getBranch

`environments.getBranch`

Get a database branch by name

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.environments.getBranch({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | Yes | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getDatabaseBranchConfig

`environments.getDatabaseBranchConfig`

Get database branch config

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.environments.getDatabaseBranchConfig({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | No | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | Yes | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### listDatabaseBranches

`environments.listDatabaseBranches`

List database branches

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.environments.listDatabaseBranches({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### pushBranch

`environments.pushBranch`

Push a database branch

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.environments.pushBranch({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | No | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | Yes | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### resetDatabaseBranch

`environments.resetDatabaseBranch`

Reset a database branch

**Risk:** `destructive` · **Irreversible**

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.environments.resetDatabaseBranch({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | No | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | Yes | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### updateDatabaseBranchConfig

`environments.updateDatabaseBranchConfig`

Update database branch config

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.environments.updateDatabaseBranchConfig({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | No | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | Yes | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

## Oauth

### authorizeUserThroughOauth

`oauth.authorizeUserThroughOauth`

Generate a Supabase OAuth authorization URL

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.oauth.authorizeUserThroughOauth({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | No | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### exchangeOauthToken

`oauth.exchangeOauthToken`

Exchange an OAuth authorization code or refresh token

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.oauth.exchangeOauthToken({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | No | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

## Organizations

### createOrganization

`organizations.createOrganization`

Create an organization

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.organizations.createOrganization({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | No | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getOrganization

`organizations.getOrganization`

Get organization information

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.organizations.getOrganization({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | No | — |
| `id` | `string` | No | — |
| `slug` | `string` | Yes | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### listAllOrganizations

`organizations.listAllOrganizations`

List organizations

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.organizations.listAllOrganizations({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | No | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### listOrganizationMembers

`organizations.listOrganizationMembers`

List organization members

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.organizations.listOrganizationMembers({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | No | — |
| `id` | `string` | No | — |
| `slug` | `string` | Yes | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

## Projects

### createProject

`projects.createProject`

Create a project

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.projects.createProject({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | No | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### deleteProject

`projects.deleteProject`

Delete a project

**Risk:** `destructive` · **Irreversible**

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.projects.deleteProject({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getAvailableRegions

`projects.getAvailableRegions`

Get available project regions

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.projects.getAvailableRegions({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | No | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getHealth

`projects.getHealth`

Get project API health status

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.projects.getHealth({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getProject

`projects.getProject`

Get a project

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.projects.getProject({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getProjectNetworkBans

`projects.getProjectNetworkBans`

Retrieve project network bans

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.projects.getProjectNetworkBans({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getProjectNetworkRestrictions

`projects.getProjectNetworkRestrictions`

Get project network restrictions

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.projects.getProjectNetworkRestrictions({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getProjectServiceHealthStatus

`projects.getProjectServiceHealthStatus`

Get project service health status

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.projects.getProjectServiceHealthStatus({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getProjectUpgradeEligibility

`projects.getProjectUpgradeEligibility`

Get project upgrade eligibility

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.projects.getProjectUpgradeEligibility({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getProjectUpgradeStatus

`projects.getProjectUpgradeStatus`

Get project upgrade status

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.projects.getProjectUpgradeStatus({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### listAllProjects

`projects.listAllProjects`

List projects

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.projects.listAllProjects({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | No | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### patchNetworkRestrictions

`projects.patchNetworkRestrictions`

Patch project network restrictions

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.projects.patchNetworkRestrictions({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### removeNetworkBans

`projects.removeNetworkBans`

Remove project network bans

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.projects.removeNetworkBans({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### updateProject

`projects.updateProject`

Update a project

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.projects.updateProject({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### updateProjectNetworkRestrictions

`projects.updateProjectNetworkRestrictions`

Apply project network restrictions

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.projects.updateProjectNetworkRestrictions({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### upgradeProjectPostgresVersion

`projects.upgradeProjectPostgresVersion`

Upgrade project PostgreSQL version

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.projects.upgradeProjectPostgresVersion({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

## Rest

### getProjectPostgrestConfig

`rest.getProjectPostgrestConfig`

Get project PostgREST config

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.rest.getProjectPostgrestConfig({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### updateProjectPostgrestConfig

`rest.updateProjectPostgrestConfig`

Update project PostgREST config

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.rest.updateProjectPostgrestConfig({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

## Secrets

### createApiKey

`secrets.createApiKey`

Create a project API key

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.secrets.createApiKey({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### createBulkSecrets

`secrets.createBulkSecrets`

Create multiple project secrets

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.secrets.createBulkSecrets({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### deleteApiKey

`secrets.deleteApiKey`

Delete a project API key

**Risk:** `destructive` · **Irreversible**

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.secrets.deleteApiKey({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | Yes | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### deleteSecrets

`secrets.deleteSecrets`

Delete multiple project secrets

**Risk:** `destructive` · **Irreversible**

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.secrets.deleteSecrets({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getProjectApiKey

`secrets.getProjectApiKey`

Get a project API key

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.secrets.getProjectApiKey({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | Yes | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getProjectApiKeys

`secrets.getProjectApiKeys`

Get project API keys

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.secrets.getProjectApiKeys({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getProjectLegacyApiKeys

`secrets.getProjectLegacyApiKeys`

Get project legacy API keys status

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.secrets.getProjectLegacyApiKeys({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getProjectPgsodiumConfig

`secrets.getProjectPgsodiumConfig`

Get project pgsodium config

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.secrets.getProjectPgsodiumConfig({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### listSecrets

`secrets.listSecrets`

List project secrets

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.secrets.listSecrets({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### updateApiKey

`secrets.updateApiKey`

Update a project API key

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.secrets.updateApiKey({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | Yes | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### updatePgsodiumConfig

`secrets.updatePgsodiumConfig`

Update pgsodium root key

**Risk:** `destructive`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.secrets.updatePgsodiumConfig({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### updateProjectLegacyApiKeys

`secrets.updateProjectLegacyApiKeys`

Update project legacy API keys

**Risk:** `write`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.secrets.updateProjectLegacyApiKeys({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

## Storage

### getResumableUploadBaseOptions

`storage.getResumableUploadBaseOptions`

Get TUS resumable upload base options

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.storage.getResumableUploadBaseOptions({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### getResumableUploadOptions

`storage.getResumableUploadOptions`

Get TUS resumable upload options

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.storage.getResumableUploadOptions({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | Yes | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### handleResumableUploadSignOptions

`storage.handleResumableUploadSignOptions`

Handle TUS resumable upload sign options

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.storage.handleResumableUploadSignOptions({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### handleResumableUploadSignOptionsWithId

`storage.handleResumableUploadSignOptionsWithId`

Handle TUS resumable upload sign options with ID

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.storage.handleResumableUploadSignOptionsWithId({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | Yes | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

### listBuckets

`storage.listBuckets`

List storage buckets

**Risk:** `read`

```ts
const tenant = corsair.withTenant('acme');
await tenant.supabase.api.storage.listBuckets({});
```

**Input**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `ref` | `string` | Yes | — |
| `id` | `string` | No | — |
| `slug` | `string` | No | — |
| `name` | `string` | No | — |
| `version` | `string` | No | — |
| `runId` | `string` | No | — |
| `tpaId` | `string` | No | — |
| `providerId` | `string` | No | — |
| `branchId` | `string` | No | — |
| `functionSlug` | `string` | No | — |
| `uploadId` | `string` | No | — |
| `schema` | `string` | No | — |
| `table` | `string` | No | — |
| `columns` | `string[]` | No | — |
| `limit` | `number` | No | — |
| `offset` | `number` | No | — |
| `body` | `any` | No | — |
| `query` | `object` | No | — |
| `headers` | `object` | No | — |
| `mediaType` | `string` | No | — |
| `baseUrl` | `string` | No | — |
| `projectApiKey` | `string` | No | — |

<AccordionGroup>
<Accordion title="query full type">

```ts
{
}
```
</Accordion>

<Accordion title="headers full type">

```ts
{
}
```
</Accordion>
</AccordionGroup>



**Output:** `any`


---

