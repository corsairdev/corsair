...
Use **Boloforms** through Corsair: one client, typed API calls, local DB sync.
...
```
```bash pnpm
pnpm install corsair @corsair-dev/boloforms
```
```bash bun
...
await tenant.boloforms.api.documents.list({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/boloforms/api) reference._


See the full list on the [API](/plugins/boloforms/api) page.

## Query synced data

Synced entities support `tenant.boloforms.db.<entity>.search()` and `.list()`: `documents`.

See [Database](/plugins/boloforms/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/boloforms/api">
    Every `boloforms.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/boloforms/database">
