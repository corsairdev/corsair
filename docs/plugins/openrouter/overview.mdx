...
Use **OpenRouter** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 13 typed API operations
- 3 synced entities (`generations`, `models`, `providers`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/openrouter
```
```bash yarn
yarn add corsair @corsair-dev/openrouter
```
```bash pnpm
pnpm install corsair @corsair-dev/openrouter
```
```bash bun
bun add corsair @corsair-dev/openrouter
```
</CodeGroup>
...
See the full list on the [API](/plugins/openrouter/api) page.

## Query synced data

Synced entities support `tenant.openrouter.db.<entity>.search()` and `.list()`: `generations`, `models`, `providers`.

See [Database](/plugins/openrouter/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/openrouter/api">
    Every `openrouter.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/openrouter/database">
