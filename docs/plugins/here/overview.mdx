---
title: Overview
description: "HERE Technologies plugin for Corsair"
---

Use **HERE Technologies** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 28 typed API operations
- 6 synced entities (`addresses`, `places`, `positions`, `routes`, `trafficResults`, `weatherPlaces`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/here
```
```bash yarn
yarn add corsair @corsair-dev/here
```
```bash pnpm
pnpm install corsair @corsair-dev/here
```
```bash bun
bun add corsair @corsair-dev/here
```
</CodeGroup>
...
import { here } from '@corsair-dev/here';
...
_No write-style endpoint inferred; pick any operation from the [API](/plugins/here/api) reference._


See the full list on the [API](/plugins/here/api) page.

## Query synced data

Synced entities support `tenant.here.db.<entity>.search()` and `.list()`: `addresses`, `places`, `positions`, `routes`, `trafficResults`, `weatherPlaces`.

See [Database](/plugins/here/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/here/api">
    Every `here.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/here/database">
