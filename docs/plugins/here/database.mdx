---
title: Database
description: "HERE Technologies local sync: searchable entities, `.search()` filters, and operators."
---

The HERE Technologies plugin syncs data locally. Use `corsair.here.db.<entity>.search({ data, limit?, offset? })` with the filters listed per entity.
...
const rows = await tenant.here.db.addresses.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.here.db.places.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.here.db.positions.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.here.db.routes.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.here.db.trafficResults.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.here.db.weatherPlaces.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
