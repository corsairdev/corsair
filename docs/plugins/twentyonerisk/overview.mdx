...
Use **TwentyOneRisk** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/twentyonerisk
```
```bash yarn
yarn add corsair @corsair-dev/twentyonerisk
```
```bash pnpm
pnpm install corsair @corsair-dev/twentyonerisk
```
```bash bun
bun add corsair @corsair-dev/twentyonerisk
```
</CodeGroup>
...
await tenant.twentyonerisk.api.organizations.get({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/twentyonerisk/api) reference._


See the full list on the [API](/plugins/twentyonerisk/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/twentyonerisk/api">
    Every `twentyonerisk.api.*` operation with input and output types.
  </Card>
