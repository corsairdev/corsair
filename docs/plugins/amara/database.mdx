---
title: Database
description: "Amara (subtitling platform) local sync: searchable entities, `.search()` filters, and operators."
---
...
const rows = await tenant.amara.db.teams.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.amara.db.users.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.amara.db.videos.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
