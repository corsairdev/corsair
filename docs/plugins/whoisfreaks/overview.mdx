---
title: Overview
description: "Whoisfreaks plugin for Corsair"
---

Use **Whoisfreaks** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/whoisfreaks
```
```bash yarn
yarn add corsair @corsair-dev/whoisfreaks
```
```bash pnpm
pnpm install corsair @corsair-dev/whoisfreaks
```
```bash bun
bun add corsair @corsair-dev/whoisfreaks
```
</CodeGroup>
...
import { whoisfreaks } from '@corsair-dev/whoisfreaks';
...
await tenant.whoisfreaks.api.asnWhois.lookup({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/whoisfreaks/api) reference._


See the full list on the [API](/plugins/whoisfreaks/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/whoisfreaks/api">
    Every `whoisfreaks.api.*` operation with input and output types.
  </Card>
