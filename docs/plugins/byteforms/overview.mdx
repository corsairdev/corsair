---
title: Overview
description: "ByteForms plugin for Corsair"
---

Use **ByteForms** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/byteforms
```
```bash yarn
yarn add corsair @corsair-dev/byteforms
```
```bash pnpm
pnpm install corsair @corsair-dev/byteforms
```
```bash bun
bun add corsair @corsair-dev/byteforms
```
</CodeGroup>
...
await tenant.byteforms.api.forms.get({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.byteforms.api.forms.create({});
```


See the full list on the [API](/plugins/byteforms/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/byteforms/api">
    Every `byteforms.api.*` operation with input and output types.
  </Card>
