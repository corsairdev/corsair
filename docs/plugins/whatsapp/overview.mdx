...
Use **WhatsApp** through Corsair: one client, typed API calls, local DB sync, and incoming webhooks.

**What you get:**

- 11 typed API operations
- 4 synced entities (`businessProfiles`, `contacts`, `messages`, `phoneNumbers`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/whatsapp
```
```bash yarn
yarn add corsair @corsair-dev/whatsapp
```
```bash pnpm
pnpm install corsair @corsair-dev/whatsapp
```
```bash bun
bun add corsair @corsair-dev/whatsapp
```
</CodeGroup>
...
await tenant.whatsapp.api.businessProfiles.get({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.whatsapp.api.media.upload({});
```


See the full list on the [API](/plugins/whatsapp/api) page.

## Query synced data

Synced entities support `tenant.whatsapp.db.<entity>.search()` and `.list()`: `businessProfiles`, `contacts`, `messages`, `phoneNumbers`.

See [Database](/plugins/whatsapp/database) for filters and operators.
...
See [Webhooks](/plugins/whatsapp/webhooks) for every event path and payload shape, and [Webhooks concept](/concepts/webhooks) for routing.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/whatsapp/api">
    Every `whatsapp.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/whatsapp/database">
    Synced entities, search filters, and operators.
  </Card>
  <Card title="Webhooks" href="/plugins/whatsapp/webhooks">
