...
const rows = await tenant.whatsapp.db.businessProfiles.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.whatsapp.db.contacts.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.whatsapp.db.messages.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.whatsapp.db.phoneNumbers.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
