---
title: Database
description: "WorldNewsApi local sync: searchable entities, `.search()` filters, and operators."
---

The WorldNewsApi plugin syncs data locally. Use `corsair.worldnewsapi.db.<entity>.search({ data, limit?, offset? })` with the filters listed per entity.
...
const rows = await tenant.worldnewsapi.db.articles.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.worldnewsapi.db.extractedArticles.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.worldnewsapi.db.geoCoordinates.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.worldnewsapi.db.sources.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
