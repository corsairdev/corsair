...
Use **OCR.space** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 3 typed API operations
- 2 synced entities (`conversions`, `ocrResults`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/ocrspace
```
```bash yarn
yarn add corsair @corsair-dev/ocrspace
```
```bash pnpm
pnpm install corsair @corsair-dev/ocrspace
```
```bash bun
bun add corsair @corsair-dev/ocrspace
```
</CodeGroup>
...
_No write-style endpoint inferred; pick any operation from the [API](/plugins/ocrspace/api) reference._


See the full list on the [API](/plugins/ocrspace/api) page.

## Query synced data

Synced entities support `tenant.ocrspace.db.<entity>.search()` and `.list()`: `conversions`, `ocrResults`.

See [Database](/plugins/ocrspace/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/ocrspace/api">
    Every `ocrspace.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/ocrspace/database">
