...
Use **Zendesk** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 11 typed API operations
- 3 synced entities (`comments`, `tickets`, `users`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/zendesk
```
```bash yarn
yarn add corsair @corsair-dev/zendesk
```
```bash pnpm
pnpm install corsair @corsair-dev/zendesk
```
```bash bun
bun add corsair @corsair-dev/zendesk
```
</CodeGroup>
...
await tenant.zendesk.api.comments.list({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.zendesk.api.tickets.create({});
```


See the full list on the [API](/plugins/zendesk/api) page.

## Query synced data

Synced entities support `tenant.zendesk.db.<entity>.search()` and `.list()`: `comments`, `tickets`, `users`.

See [Database](/plugins/zendesk/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/zendesk/api">
    Every `zendesk.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/zendesk/database">
