...
const rows = await tenant.dynapictures.db.media.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.dynapictures.db.templates.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.dynapictures.db.webhooks.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.dynapictures.db.workspaces.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
