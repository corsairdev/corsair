---
title: API
description: "API reference for TavilyMcp: every `tavilymcp.api.*` operation with input and output types."
---
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.tavilymcp.api.tavily.extract({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.tavilymcp.api.tavily.map({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.tavilymcp.api.tavily.research({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.tavilymcp.api.tavily.search({});
```
