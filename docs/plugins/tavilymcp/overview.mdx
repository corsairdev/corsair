...
Use **TavilyMcp** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 5 typed API operations
- 5 synced entities (`crawlResults`, `extractResults`, `mapResults`, `researchResults`, `searchResults`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/tavilymcp
```
```bash yarn
yarn add corsair @corsair-dev/tavilymcp
```
```bash pnpm
pnpm install corsair @corsair-dev/tavilymcp
```
```bash bun
bun add corsair @corsair-dev/tavilymcp
```
</CodeGroup>
...
await tenant.tavilymcp.api.tavily.crawl({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/tavilymcp/api) reference._


See the full list on the [API](/plugins/tavilymcp/api) page.

## Query synced data

Synced entities support `tenant.tavilymcp.db.<entity>.search()` and `.list()`: `crawlResults`, `extractResults`, `mapResults`, `researchResults`, `searchResults`.

See [Database](/plugins/tavilymcp/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/tavilymcp/api">
    Every `tavilymcp.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/tavilymcp/database">
