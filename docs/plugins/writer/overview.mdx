---
title: Overview
description: "Writer plugin for Corsair"
---

Use **Writer** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/writer
```
```bash yarn
yarn add corsair @corsair-dev/writer
```
```bash pnpm
pnpm install corsair @corsair-dev/writer
```
```bash bun
bun add corsair @corsair-dev/writer
```
</CodeGroup>
...
await tenant.writer.api.applications.list({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.writer.api.chat.create({});
```


See the full list on the [API](/plugins/writer/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/writer/api">
    Every `writer.api.*` operation with input and output types.
  </Card>
