...
const rows = await tenant.tally.db.forms.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.tally.db.submissions.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
| `respondentId` | `string` | equals, contains, startsWith, endsWith, in |
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.tally.db.workspaces.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
