---
title: API
description: "API reference for APIpie AI: every `apipie.api.*` operation with input and output types."
---
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.apipie.api.embeddings.create({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.apipie.api.images.generate({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.apipie.api.models.listDetailed({});
```
