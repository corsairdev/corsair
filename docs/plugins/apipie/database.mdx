---
title: Database
description: "APIpie AI local sync: searchable entities, `.search()` filters, and operators."
---
...
const rows = await tenant.apipie.db.images.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.apipie.db.modelDetails.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.apipie.db.models.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
