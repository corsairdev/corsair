---
title: Overview
description: "DataRobot plugin for Corsair"
---

Use **DataRobot** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 107 typed API operations
- 5 synced entities (`dataset`, `deployment`, `project`, `useCase`, `user`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/datarobot
```
```bash yarn
yarn add corsair @corsair-dev/datarobot
```
```bash pnpm
pnpm install corsair @corsair-dev/datarobot
```
```bash bun
bun add corsair @corsair-dev/datarobot
```
</CodeGroup>
...
import { datarobot } from '@corsair-dev/datarobot';
...
See the full list on the [API](/plugins/datarobot/api) page.

## Query synced data

Synced entities support `tenant.datarobot.db.<entity>.search()` and `.list()`: `dataset`, `deployment`, `project`, `useCase`, `user`.

See [Database](/plugins/datarobot/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/datarobot/api">
    Every `datarobot.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/datarobot/database">
