...
Use **AimlApi** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 29 typed API operations
- 4 synced entities (`assistants`, `batches`, `models`, `threads`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/aimlapi
```
```bash yarn
yarn add corsair @corsair-dev/aimlapi
```
```bash pnpm
pnpm install corsair @corsair-dev/aimlapi
```
```bash bun
bun add corsair @corsair-dev/aimlapi
```
</CodeGroup>
...
See the full list on the [API](/plugins/aimlapi/api) page.

## Query synced data

Synced entities support `tenant.aimlapi.db.<entity>.search()` and `.list()`: `assistants`, `batches`, `models`, `threads`.

See [Database](/plugins/aimlapi/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/aimlapi/api">
    Every `aimlapi.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/aimlapi/database">
