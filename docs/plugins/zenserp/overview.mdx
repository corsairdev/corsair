...
Use **Zenserp** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/zenserp
```
```bash yarn
yarn add corsair @corsair-dev/zenserp
```
```bash pnpm
pnpm install corsair @corsair-dev/zenserp
```
```bash bun
bun add corsair @corsair-dev/zenserp
```
</CodeGroup>
...
import { zenserp } from '@corsair-dev/zenserp';
...
await tenant.zenserp.api.account.getStatus({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/zenserp/api) reference._


See the full list on the [API](/plugins/zenserp/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/zenserp/api">
    Every `zenserp.api.*` operation with input and output types.
  </Card>
