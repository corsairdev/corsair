...
Use **Ably** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/ably
```
```bash yarn
yarn add corsair @corsair-dev/ably
```
```bash pnpm
pnpm install corsair @corsair-dev/ably
```
```bash bun
bun add corsair @corsair-dev/ably
```
</CodeGroup>
...
See the full list on the [API](/plugins/ably/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/ably/api">
    Every `ably.api.*` operation with input and output types.
  </Card>
