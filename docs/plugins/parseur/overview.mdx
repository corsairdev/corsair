---
title: Overview
description: "Parseur plugin for Corsair"
---

Use **Parseur** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 30 typed API operations
- 6 synced entities (`documents`, `exportConfigs`, `logs`, `parsers`, `templates`, `webhooks`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/parseur
```
```bash yarn
yarn add corsair @corsair-dev/parseur
```
```bash pnpm
pnpm install corsair @corsair-dev/parseur
```
```bash bun
bun add corsair @corsair-dev/parseur
```
</CodeGroup>
...
import { parseur } from '@corsair-dev/parseur';
...
See the full list on the [API](/plugins/parseur/api) page.

## Query synced data

Synced entities support `tenant.parseur.db.<entity>.search()` and `.list()`: `documents`, `exportConfigs`, `logs`, `parsers`, `templates`, `webhooks`.

See [Database](/plugins/parseur/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/parseur/api">
    Every `parseur.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/parseur/database">
