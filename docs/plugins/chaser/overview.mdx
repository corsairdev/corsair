...
Use **Chaser** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/chaser
```
```bash yarn
yarn add corsair @corsair-dev/chaser
```
```bash pnpm
pnpm install corsair @corsair-dev/chaser
```
```bash bun
bun add corsair @corsair-dev/chaser
```
</CodeGroup>
...
import { chaser } from '@corsair-dev/chaser';
...
await tenant.chaser.api.creditNotes.list({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/chaser/api) reference._


See the full list on the [API](/plugins/chaser/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/chaser/api">
    Every `chaser.api.*` operation with input and output types.
  </Card>
