---
title: Database
description: "Anthropic Admin API local sync: searchable entities, `.search()` filters, and operators."
---
...
const rows = await tenant.anthropicadministrator.db.apiKeys.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.anthropicadministrator.db.invites.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.anthropicadministrator.db.users.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.anthropicadministrator.db.workspaceMembers.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.anthropicadministrator.db.workspaces.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
