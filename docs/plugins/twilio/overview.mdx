...
await tenant.twilio.api.calls.get({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.twilio.api.calls.create({});
```
...
Synced entities support `tenant.twilio.db.<entity>.search()` and `.list()`: `calls`, `messages`.

See [Database](/plugins/twilio/database) for filters and operators.
...
See [Webhooks](/plugins/twilio/webhooks) for every event path and payload shape, and [Webhooks concept](/concepts/webhooks) for routing.
