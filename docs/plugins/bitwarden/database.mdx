...
const rows = await tenant.bitwarden.db.ciphers.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.bitwarden.db.collections.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.bitwarden.db.members.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.bitwarden.db.organizations.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
