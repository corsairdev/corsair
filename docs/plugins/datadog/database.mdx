...
```ts
const rows = await corsair.datadog.db.dashboards.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const rows = await corsair.datadog.db.incidents.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const rows = await corsair.datadog.db.monitors.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const rows = await corsair.datadog.db.slos.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
