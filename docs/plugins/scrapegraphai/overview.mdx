---
title: Overview
description: "ScrapeGraphAI plugin for Corsair"
---

Use **ScrapeGraphAI** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 27 typed API operations
- 8 synced entities (`crawls`, `credits`, `generatedSchemas`, `jobs`, `markdownifies`, `scheduledJobs`, and 2 more) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/scrapegraphai
```
```bash yarn
yarn add corsair @corsair-dev/scrapegraphai
```
```bash pnpm
pnpm install corsair @corsair-dev/scrapegraphai
```
```bash bun
bun add corsair @corsair-dev/scrapegraphai
```
</CodeGroup>
...
See the full list on the [API](/plugins/scrapegraphai/api) page.

## Query synced data

Synced entities support `tenant.scrapegraphai.db.<entity>.search()` and `.list()`: `crawls`, `credits`, `generatedSchemas`, `jobs`, `markdownifies`, `scheduledJobs`, `searchScrapers`, `smartScrapers`.

See [Database](/plugins/scrapegraphai/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/scrapegraphai/api">
    Every `scrapegraphai.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/scrapegraphai/database">
