...
Use **CampaignCleaner** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 5 typed API operations
- 2 synced entities (`campaigns`, `credits`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/campaigncleaner
```
```bash yarn
yarn add corsair @corsair-dev/campaigncleaner
```
```bash pnpm
pnpm install corsair @corsair-dev/campaigncleaner
```
```bash bun
bun add corsair @corsair-dev/campaigncleaner
```
</CodeGroup>
...
import { campaigncleaner } from '@corsair-dev/campaigncleaner';
...
await tenant.campaigncleaner.api.campaign.list({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.campaigncleaner.api.campaign.delete({});
```


See the full list on the [API](/plugins/campaigncleaner/api) page.

## Query synced data

Synced entities support `tenant.campaigncleaner.db.<entity>.search()` and `.list()`: `campaigns`, `credits`.

See [Database](/plugins/campaigncleaner/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/campaigncleaner/api">
    Every `campaigncleaner.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/campaigncleaner/database">
