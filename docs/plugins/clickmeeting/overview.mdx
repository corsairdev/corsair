...
```
```bash pnpm
pnpm install corsair @corsair-dev/clickmeeting
```
```bash bun
...
await tenant.clickmeeting.api.chats.getChatDetails({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.clickmeeting.api.conferences.createConference({});
```
