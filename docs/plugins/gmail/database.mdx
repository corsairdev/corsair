---
title: Database
description: "Gmail local sync: searchable entities, `.search()` filters, and operators."
---
...
const rows = await tenant.gmail.db.drafts.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.gmail.db.labels.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.gmail.db.threads.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
