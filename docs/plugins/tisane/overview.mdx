---
title: Overview
description: "Tisane plugin for Corsair"
---

Use **Tisane** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/tisane
```
```bash yarn
yarn add corsair @corsair-dev/tisane
```
```bash pnpm
pnpm install corsair @corsair-dev/tisane
```
```bash bun
bun add corsair @corsair-dev/tisane
```
</CodeGroup>
...
import { tisane } from '@corsair-dev/tisane';
...
await tenant.tisane.api.text.extractEntities({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/tisane/api) reference._


See the full list on the [API](/plugins/tisane/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/tisane/api">
    Every `tisane.api.*` operation with input and output types.
  </Card>
