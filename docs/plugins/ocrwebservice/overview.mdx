---
title: Overview
description: "OcrWebService plugin for Corsair"
---

Use **OcrWebService** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 4 typed API operations
- 2 synced entities (`accounts`, `documents`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/ocrwebservice
```
```bash yarn
yarn add corsair @corsair-dev/ocrwebservice
```
```bash pnpm
pnpm install corsair @corsair-dev/ocrwebservice
```
```bash bun
bun add corsair @corsair-dev/ocrwebservice
```
</CodeGroup>
...
import { ocrwebservice } from '@corsair-dev/ocrwebservice';
...
await tenant.ocrwebservice.api.account.getCredentials({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.ocrwebservice.api.ocr.recognize({});
```


See the full list on the [API](/plugins/ocrwebservice/api) page.

## Query synced data

Synced entities support `tenant.ocrwebservice.db.<entity>.search()` and `.list()`: `accounts`, `documents`.

See [Database](/plugins/ocrwebservice/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/ocrwebservice/api">
    Every `ocrwebservice.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/ocrwebservice/database">
