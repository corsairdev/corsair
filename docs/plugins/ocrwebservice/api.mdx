---
title: API
description: "API reference for OcrWebService: every `ocrwebservice.api.*` operation with input and output types."
---
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.ocrwebservice.api.account.getInformation({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.ocrwebservice.api.account.log({});
```
