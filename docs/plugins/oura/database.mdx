...
const rows = await tenant.oura.db.dailyActivity.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.oura.db.dailyReadiness.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.oura.db.dailySleep.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.oura.db.personalInfo.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
