---
title: Database
description: "Monday local sync: searchable entities, `.search()` filters, and operators."
---
...
const rows = await tenant.monday.db.boards.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.monday.db.items.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.monday.db.updates.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.monday.db.users.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
