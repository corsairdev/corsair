...
Use **ApiLabz** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/apilabz
```
```bash yarn
yarn add corsair @corsair-dev/apilabz
```
```bash pnpm
pnpm install corsair @corsair-dev/apilabz
```
```bash bun
bun add corsair @corsair-dev/apilabz
```
</CodeGroup>
...
await tenant.apilabz.api.airtable.listTables({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.apilabz.api.deals.integrate({});
```


See the full list on the [API](/plugins/apilabz/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/apilabz/api">
    Every `apilabz.api.*` operation with input and output types.
  </Card>
