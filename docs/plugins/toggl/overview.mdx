...
Use **Toggl** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 73 typed API operations
- 4 synced entities (`clients`, `projects`, `tags`, `workspaces`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/toggl
```
```bash yarn
yarn add corsair @corsair-dev/toggl
```
```bash pnpm
pnpm install corsair @corsair-dev/toggl
```
```bash bun
bun add corsair @corsair-dev/toggl
```
</CodeGroup>
...
See the full list on the [API](/plugins/toggl/api) page.

## Query synced data

Synced entities support `tenant.toggl.db.<entity>.search()` and `.list()`: `clients`, `projects`, `tags`, `workspaces`.

See [Database](/plugins/toggl/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/toggl/api">
    Every `toggl.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/toggl/database">
