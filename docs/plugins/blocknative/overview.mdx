---
title: Overview
description: "Blocknative plugin for Corsair"
---

Use **Blocknative** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 5 typed API operations
- 5 synced entities (`baseFeeEstimates`, `blockPrices`, `chains`, `gasDistributions`, `oracles`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/blocknative
```
```bash yarn
yarn add corsair @corsair-dev/blocknative
```
```bash pnpm
pnpm install corsair @corsair-dev/blocknative
```
```bash bun
bun add corsair @corsair-dev/blocknative
```
</CodeGroup>
...
import { blocknative } from '@corsair-dev/blocknative';
...
await tenant.blocknative.api.gas.getBaseFeeEstimates({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/blocknative/api) reference._


See the full list on the [API](/plugins/blocknative/api) page.

## Query synced data

Synced entities support `tenant.blocknative.db.<entity>.search()` and `.list()`: `baseFeeEstimates`, `blockPrices`, `chains`, `gasDistributions`, `oracles`.

See [Database](/plugins/blocknative/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/blocknative/api">
    Every `blocknative.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/blocknative/database">
