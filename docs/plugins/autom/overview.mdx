...
Use **Autom** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/autom
```
```bash yarn
yarn add corsair @corsair-dev/autom
```
```bash pnpm
pnpm install corsair @corsair-dev/autom
```
```bash bun
bun add corsair @corsair-dev/autom
```
</CodeGroup>
...
await tenant.autom.api.google.countries({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/autom/api) reference._


See the full list on the [API](/plugins/autom/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/autom/api">
    Every `autom.api.*` operation with input and output types.
  </Card>
