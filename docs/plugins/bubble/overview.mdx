---
title: Overview
description: "Bubble (Data API & Workflow API) plugin for Corsair"
---

Use **Bubble (Data API & Workflow API)** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 10 typed API operations
- 1 synced entity (`things`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/bubble
```
```bash yarn
yarn add corsair @corsair-dev/bubble
```
```bash pnpm
pnpm install corsair @corsair-dev/bubble
```
```bash bun
bun add corsair @corsair-dev/bubble
```
</CodeGroup>
...
import { bubble } from '@corsair-dev/bubble';
...
await tenant.bubble.api.meta.getSwagger({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.bubble.api.things.bulkCreate({});
```


See the full list on the [API](/plugins/bubble/api) page.

## Query synced data

Synced entities support `tenant.bubble.db.<entity>.search()` and `.list()`: `things`.

See [Database](/plugins/bubble/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/bubble/api">
    Every `bubble.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/bubble/database">
