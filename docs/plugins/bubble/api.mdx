---
title: API
description: "API reference for Bubble (Data API & Workflow API): every `bubble.api.*` operation with input and output types."
---
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.bubble.api.things.create({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.bubble.api.things.delete({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.bubble.api.things.get({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.bubble.api.things.list({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.bubble.api.things.replace({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.bubble.api.things.update({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.bubble.api.workflows.run({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.bubble.api.workflows.runGet({});
```
