---
title: Overview
description: "Booqable plugin for Corsair"
---

Use **Booqable** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/booqable
```
```bash yarn
yarn add corsair @corsair-dev/booqable
```
```bash pnpm
pnpm install corsair @corsair-dev/booqable
```
```bash bun
bun add corsair @corsair-dev/booqable
```
</CodeGroup>
...
import { booqable } from '@corsair-dev/booqable';
...
await tenant.booqable.api.barcodes.listBarcodes({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.booqable.api.companies.updateCompany({});
```


See the full list on the [API](/plugins/booqable/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/booqable/api">
    Every `booqable.api.*` operation with input and output types.
  </Card>
