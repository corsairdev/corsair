...
Use **DevinMcp** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/devinmcp
```
```bash yarn
yarn add corsair @corsair-dev/devinmcp
```
```bash pnpm
pnpm install corsair @corsair-dev/devinmcp
```
```bash bun
bun add corsair @corsair-dev/devinmcp
```
</CodeGroup>
...
await tenant.devinmcp.api.session.get({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.devinmcp.api.session.create({});
```


See the full list on the [API](/plugins/devinmcp/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/devinmcp/api">
    Every `devinmcp.api.*` operation with input and output types.
  </Card>
