---
title: Overview
description: 'Neon plugin for Corsair'
---
...
Use the key name documented in [Get Credentials](/plugins/neon/get-credentials): `api_key=<neon_api_key>`. Neon uses API key auth only.
...
```ts
await corsair.neon.api.apiKeys.createApiKey({ key_name: 'ci-runner' })
```
