---
title: Overview
description: "Pdfco plugin for Corsair"
---

Use **Pdfco** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/pdfco
```
```bash yarn
yarn add corsair @corsair-dev/pdfco
```
```bash pnpm
pnpm install corsair @corsair-dev/pdfco
```
```bash bun
bun add corsair @corsair-dev/pdfco
```
</CodeGroup>
...
import { pdfco } from '@corsair-dev/pdfco';
...
await tenant.pdfco.api.accountBalance({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.pdfco.api.barcodeGenerate({});
```


See the full list on the [API](/plugins/pdfco/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/pdfco/api">
    Every `pdfco.api.*` operation with input and output types.
  </Card>
