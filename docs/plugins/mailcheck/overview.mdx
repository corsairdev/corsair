---
title: Overview
description: "Mailcheck plugin for Corsair"
---

Use **Mailcheck** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/mailcheck
```
```bash yarn
yarn add corsair @corsair-dev/mailcheck
```
```bash pnpm
pnpm install corsair @corsair-dev/mailcheck
```
```bash bun
bun add corsair @corsair-dev/mailcheck
```
</CodeGroup>
...
import { mailcheck } from '@corsair-dev/mailcheck';
...
await tenant.mailcheck.api.domain.validate({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/mailcheck/api) reference._


See the full list on the [API](/plugins/mailcheck/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/mailcheck/api">
    Every `mailcheck.api.*` operation with input and output types.
  </Card>
