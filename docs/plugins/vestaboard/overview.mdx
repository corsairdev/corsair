---
title: Overview
description: "Vestaboard plugin for Corsair"
---

Use **Vestaboard** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 2 typed API operations
- 2 synced entities (`messages`, `subscriptions`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/vestaboard
```
```bash yarn
yarn add corsair @corsair-dev/vestaboard
```
```bash pnpm
pnpm install corsair @corsair-dev/vestaboard
```
```bash bun
bun add corsair @corsair-dev/vestaboard
```
</CodeGroup>
...
import { vestaboard } from '@corsair-dev/vestaboard';
...
await tenant.vestaboard.api.subscriptions.list({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.vestaboard.api.subscriptions.postMessage({});
```


See the full list on the [API](/plugins/vestaboard/api) page.

## Query synced data

Synced entities support `tenant.vestaboard.db.<entity>.search()` and `.list()`: `messages`, `subscriptions`.

See [Database](/plugins/vestaboard/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/vestaboard/api">
    Every `vestaboard.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/vestaboard/database">
