...
const rows = await tenant.addresszen.db.autocompleteResults.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.addresszen.db.keyAvailability.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.addresszen.db.resolvedAddresses.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.addresszen.db.verifiedAddresses.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
