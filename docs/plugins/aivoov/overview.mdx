...
Use **AiVOOV text-to-speech** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 2 typed API operations
- 1 synced entity (`voices`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/aivoov
```
```bash yarn
yarn add corsair @corsair-dev/aivoov
```
```bash pnpm
pnpm install corsair @corsair-dev/aivoov
```
```bash bun
bun add corsair @corsair-dev/aivoov
```
</CodeGroup>
...
await tenant.aivoov.api.voices.list({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.aivoov.api.audio.create({});
```


See the full list on the [API](/plugins/aivoov/api) page.

## Query synced data

Synced entities support `tenant.aivoov.db.<entity>.search()` and `.list()`: `voices`.

See [Database](/plugins/aivoov/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/aivoov/api">
    Every `aivoov.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/aivoov/database">
