---
title: Database
description: "AiVOOV text-to-speech local sync: searchable entities, `.search()` filters, and operators."
---

The AiVOOV text-to-speech plugin syncs data locally. Use `corsair.aivoov.db.<entity>.search({ data, limit?, offset? })` with the filters listed per entity.
...
const rows = await tenant.aivoov.db.voices.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
