---
title: Overview
description: "Sourcegraph plugin for Corsair"
---

Use **Sourcegraph** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 8 typed API operations
- 5 synced entities (`commits`, `files`, `repositories`, `sites`, `users`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/sourcegraph
```
```bash yarn
yarn add corsair @corsair-dev/sourcegraph
```
```bash pnpm
pnpm install corsair @corsair-dev/sourcegraph
```
```bash bun
bun add corsair @corsair-dev/sourcegraph
```
</CodeGroup>
...
import { sourcegraph } from '@corsair-dev/sourcegraph';
...
await tenant.sourcegraph.api.repository.compareCommits({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/sourcegraph/api) reference._


See the full list on the [API](/plugins/sourcegraph/api) page.

## Query synced data

Synced entities support `tenant.sourcegraph.db.<entity>.search()` and `.list()`: `commits`, `files`, `repositories`, `sites`, `users`.

See [Database](/plugins/sourcegraph/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/sourcegraph/api">
    Every `sourcegraph.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/sourcegraph/database">
