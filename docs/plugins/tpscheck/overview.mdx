---
title: Overview
description: "UK phone number validation and TPS/CTPS register checking for marketing compliance."
---

Use **TPSCheck** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/tpscheck
```
```bash yarn
yarn add corsair @corsair-dev/tpscheck
```
```bash pnpm
pnpm install corsair @corsair-dev/tpscheck
```
```bash bun
bun add corsair @corsair-dev/tpscheck
```
</CodeGroup>
...
await tenant.tpscheck.api.batch.post({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/tpscheck/api) reference._


See the full list on the [API](/plugins/tpscheck/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/tpscheck/api">
    Every `tpscheck.api.*` operation with input and output types.
  </Card>
