...
```ts
const tenant = corsair.withTenant('acme');
await tenant.clickhouse.api.query.listDatabases({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.clickhouse.api.query.listTables({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.clickhouse.api.schema.getDatabase({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.clickhouse.api.schema.getTable({});
```
