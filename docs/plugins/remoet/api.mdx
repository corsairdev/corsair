...
```ts
await corsair.remoet.api.companies.get({});
```
...
```ts
await corsair.remoet.api.education.create({});
```
...
| `studyLevel` | `HIGH_SCHOOL \| ASSOCIATE \| BACHELOR \| MASTER \| DOCTORATE \| BOOTCAMP \| OTHER` | No | — |
...
```ts
await corsair.remoet.api.education.delete({});
```
...
```ts
await corsair.remoet.api.education.update({});
```
...
```ts
await corsair.remoet.api.jobContext.get({});
```
...
```ts
await corsair.remoet.api.linkTrees.get({});
```
...
```ts
await corsair.remoet.api.profile.update({});
```
...
| `youtubeUrl` | `string` | No | — |
| `summary` | `string` | No | — |
| `visibility` | `NONE \| STARRED \| ALL` | No | — |
...
```ts
await corsair.remoet.api.projects.create({});
```
...
```ts
await corsair.remoet.api.projects.delete({});
```
...
```ts
await corsair.remoet.api.projects.update({});
```
...
```ts
await corsair.remoet.api.savedJobs.create({});
```
...
```ts
await corsair.remoet.api.savedJobs.delete({});
```
...
```ts
await corsair.remoet.api.savedJobs.update({});
```
...
```ts
await corsair.remoet.api.stars.create({});
```
...
```ts
await corsair.remoet.api.stars.delete({});
```
...
```ts
await corsair.remoet.api.workExperience.delete({});
```
...
```ts
await corsair.remoet.api.workExperience.update({});
```
