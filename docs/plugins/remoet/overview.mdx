---
title: Overview
description: "Read and manage a Remoet user's career profile (work experience, projects, education, visibility) and feed, look up what Remoet knows about a job page, search jobs and companies, and star or save jobs. Remoet is an AI-agent-first tech job platform for tracking tech jobs from companies you love."
---

Use **Remoet** through Corsair: one client, typed API calls.

**What you get:**

- 29 typed API operations

## Setup

<Steps>

<Step title="Install">
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/remoet
```
```bash yarn
yarn add corsair @corsair-dev/remoet
```
```bash pnpm
pnpm install corsair @corsair-dev/remoet
```
```bash bun
bun add corsair @corsair-dev/remoet
```
</CodeGroup>

</Step>

<Step title="Add the plugin">
```ts corsair.ts
import Database from 'better-sqlite3';
import { createCorsair } from 'corsair';
import { remoet } from '@corsair-dev/remoet';

export const corsair = createCorsair({
	plugins: [
		remoet(),
	],
	database: new Database('corsair.db'),
	kek: process.env.CORSAIR_KEK!,
	hub: {
		projectApiKey: process.env.CORSAIR_API_KEY!,
		signingSecret: process.env.CORSAIR_SIGNING_SECRET!,
	},
});
```

Multi-tenancy is the default — scope calls with `corsair.withTenant(id)`. See [Quick start](/quick-start) for KEK + Hub keys, and [Multi-tenancy](/concepts/multi-tenancy) for account isolation.

</Step>

<Step title="Choose authentication">
<Tabs>
<Tab title="API Key (Recommended)">

No setup required yet. When you make your first request as a tenant, Corsair prompts for the API key.

```ts
remoet()
```

More: [API Key](/concepts/api-key)

</Tab>
</Tabs>

</Step>

<Step title="Connect a tenant">
Mint a connect link and send the tenant to it. Hub hosts the page and delivers the result to your app — see [Connect / OAuth](/management/connect).

```ts
const { connectUrl } = await corsair.manage.connect.createLink({
	plugin: 'remoet',
	tenantId: 'acme',
});
// redirect the user's browser to connectUrl
```

</Step>

</Steps>

## Example API calls

**Search open jobs**

```ts
const tenant = corsair.withTenant('acme');
await tenant.remoet.api.jobs.search({
	searchQuery: 'typescript',
	remotePolicy: ['remote'],
	pageSize: 5,
});
```


**Add a work experience entry**

```ts
const tenant = corsair.withTenant('acme');
await tenant.remoet.api.workExperience.create({
	title: 'Senior Engineer',
	companyName: 'Acme',
	startDate: '2023-04-01',
	technologies: ['TypeScript', 'Go'],
});
```


See the full list on the [API](/plugins/remoet/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/remoet/api">
    Every `remoet.api.*` operation with input and output types.
  </Card>
  <Card title="Connect / OAuth" href="/management/connect">
    createLink, Hub delivery, and tenant connect flows.
  </Card>
  <Card title="Use with agents" href="/mcp-adapters/mcp-adapters">
    Expose this plugin's operations as MCP tools.
  </Card>
</CardGroup>
