...
Use **Serpapi** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/serpapi
```
```bash yarn
yarn add corsair @corsair-dev/serpapi
```
```bash pnpm
pnpm install corsair @corsair-dev/serpapi
```
```bash bun
bun add corsair @corsair-dev/serpapi
```
</CodeGroup>
...
_No write-style endpoint inferred; pick any operation from the [API](/plugins/serpapi/api) reference._


See the full list on the [API](/plugins/serpapi/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/serpapi/api">
    Every `serpapi.api.*` operation with input and output types.
  </Card>
