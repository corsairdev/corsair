---
title: Overview
description: "AI notetaker for calls and meetings — transcribe, analyse, and summarize conversations."
---

Use **Leexi** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/leexi
```
```bash yarn
yarn add corsair @corsair-dev/leexi
```
```bash pnpm
pnpm install corsair @corsair-dev/leexi
```
```bash bun
bun add corsair @corsair-dev/leexi
```
</CodeGroup>
...
await tenant.leexi.api.calls.list({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.leexi.api.calls.requestPresignedUrl({});
```


See the full list on the [API](/plugins/leexi/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/leexi/api">
    Every `leexi.api.*` operation with input and output types.
  </Card>
