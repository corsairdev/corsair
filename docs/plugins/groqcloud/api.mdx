---
title: API
description: "API reference for GroqCloud: every `groqcloud.api.*` operation with input and output types."
---
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.groqcloud.api.audio.createTranslation({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.groqcloud.api.chat.createCompletion({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.groqcloud.api.chat.createResponse({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.groqcloud.api.models.listModels({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.groqcloud.api.models.retrieveModel({});
```
