...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.slack.db.files.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.slack.db.messages.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.slack.db.userGroups.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.slack.db.users.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
