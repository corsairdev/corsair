...
Use **AltText.ai** through Corsair: one client, typed API calls, local DB sync.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/alttextai
```
```bash yarn
yarn add corsair @corsair-dev/alttextai
```
```bash pnpm
pnpm install corsair @corsair-dev/alttextai
```
```bash bun
bun add corsair @corsair-dev/alttextai
```
</CodeGroup>
...
await tenant.alttextai.api.account.get({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.alttextai.api.account.update({});
```


See the full list on the [API](/plugins/alttextai/api) page.

## Query synced data

Synced entities support `tenant.alttextai.db.<entity>.search()` and `.list()`: `images`.

See [Database](/plugins/alttextai/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/alttextai/api">
    Every `alttextai.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/alttextai/database">
