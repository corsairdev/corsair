...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.apps.deleteDatabricksApp({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.apps.deployDatabricksApp({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.catalog.assignMetastoreToWorkspace({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.catalog.batchCreateAccessRequests({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.catalog.createCatalogConnection({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.catalog.createCatalogCredential({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.catalog.createExternalLocation({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.catalog.createMetastore({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.catalog.createStorageCredential({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.catalog.deleteCatalog({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.catalog.deleteCatalogConnection({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.catalog.deleteCatalogCredential({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.catalog.deleteCatalogTable({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.catalog.deleteExternalLocation({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.catalog.deleteMetastore({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.catalog.deleteOnlineTable({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.catalog.deleteStorageCredential({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.catalog.disableSystemSchema({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.cleanrooms.createCleanRoom({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.cleanrooms.createCleanRoomAutoApprovalRule({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.compute.addComputeInstanceProfile({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.compute.createComputeClusterPolicy({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.compute.createComputeInstancePool({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.compute.createDatabricksCluster({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.compute.createGlobalInitScript({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.compute.deleteComputeClusterPolicy({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.compute.deleteComputeInstancePool({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.compute.deleteDatabricksCluster({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.compute.deleteGlobalInitScript({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.dashboards.createGenieMessage({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.dashboards.createGenieSpace({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.dashboards.createLakeviewDashboard({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.dashboards.deleteGenieConversation({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.dashboards.deleteGenieConversationMessage({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.dashboards.deleteLakeviewDashboardSchedule({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.database.createDatabaseInstance({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.database.deleteDatabaseInstance({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.database.deleteSyncedDatabaseTable({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.dataquality.createDataQualityMonitor({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.dataquality.createQualityMonitorV2({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.dbfs.addBlockToDbfsStream({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.dbfs.createDbfsFileStream({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.dbfs.deleteDbfsFileOrDirectory({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.iam.addMemberToSecurityGroup({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.iam.createIamGroupV2({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.iam.createIamServicePrincipalV2({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.iam.createIamUserV2({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.iam.createIpAccessList({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.iam.deleteIamGroupV2({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.iam.deleteIamServicePrincipalV2({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.iam.deleteIamUserV2({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.jobs.cancelAllJobRuns({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.jobs.cancelJobRun({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.jobs.deleteDatabricksJobRun({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.marketplace.batchGetMarketplaceConsumerListings({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.marketplace.batchGetMarketplaceConsumerProviders({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.marketplace.createMarketplaceConsumerInstallation({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.marketplace.createMarketplaceProviderListing({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.marketplace.createProviderAnalyticsDashboard({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.marketplace.deleteListingFromExchange({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.marketplace.deleteMarketplaceConsumerInstallation({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.ml.createLoggedModel({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.ml.createMlExperiment({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.ml.createMlFeatureStoreOnlineStore({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.ml.createMlflowExperimentRun({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.ml.createMlForecastingExperiment({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.ml.deleteLoggedModel({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.ml.deleteLoggedModelTag({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.ml.deleteMlExperiment({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.ml.deleteMlExperimentRun({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.ml.deleteMlExperimentRuns({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.ml.deleteMlExperimentRunTag({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.ml.deleteMlFeatureEngKafkaConfig({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.ml.deleteMlFeatureStoreOnlineStore({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.ml.deleteMlFeatureTag({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.security.createNotificationDestination({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.security.createOAuthServicePrincipalSecret({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.security.createPersonalAccessToken({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.security.deleteNotificationDestination({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.security.deleteOAuth2ServicePrincipalSecret({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.security.deleteTokenManagement({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.serving.createProvisionedThroughputEndpoint({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.serving.createVectorSearchEndpoint({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.serving.deleteServingEndpoint({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.serving.deleteVectorSearchIndex({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.sharing.createShare({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.sharing.createSharingProvider({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.sharing.createSharingRecipient({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.sharing.deleteShare({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.sharing.deleteSharingRecipient({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.sql.cancelSqlStatementExecution({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.sql.createLegacySqlAlert({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.sql.createLegacySqlQuery({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.sql.createLegacySqlQueryVisualization({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.sql.createSqlAlert({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.sql.createSqlQuery({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.sql.createSqlQueryVisualization({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.sql.deleteLegacySqlAlert({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.sql.deleteLegacySqlQuery({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.sql.deleteLegacySqlQueryVisualization({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.sql.deleteSqlAlert({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.sql.deleteSqlDashboard({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.sql.deleteSqlQuery({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.sql.deleteSqlWarehouse({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.createSecretScope({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.createTagPolicy({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.createWorkspaceDirectory({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.createWorkspaceGitCredentials({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.createWorkspaceRepo({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.deleteAibiDashboardEmbeddingAccessPolicy({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.deleteAibiDashboardEmbeddingApprovedDomains({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.deleteCustomLlmAgent({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.deleteDashboardEmailSubscriptionsSetting({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.deleteDatabricksPipeline({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.deleteDefaultNamespaceSetting({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.deleteDefaultWarehouseIdSetting({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.deleteDisableLegacyAccessSetting({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.deleteDisableLegacyDbfsSetting({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.deleteLlmProxyPartnerSetting({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.deleteRestrictWorkspaceAdminsSetting({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.deleteSecretsAcl({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.deleteSecretScope({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.deleteSqlResultsDownloadSetting({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.deleteTagPolicy({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.deleteWorkspaceGitCredentials({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.deleteWorkspaceObject({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.deleteWorkspaceRepo({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.databricks.api.workspace.deleteWorkspaceSecret({});
```
