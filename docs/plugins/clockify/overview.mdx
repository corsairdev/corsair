...
Use **Clockify** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/clockify
```
```bash yarn
yarn add corsair @corsair-dev/clockify
```
```bash pnpm
pnpm install corsair @corsair-dev/clockify
```
```bash bun
bun add corsair @corsair-dev/clockify
```
</CodeGroup>
...
See the full list on the [API](/plugins/clockify/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/clockify/api">
    Every `clockify.api.*` operation with input and output types.
  </Card>
