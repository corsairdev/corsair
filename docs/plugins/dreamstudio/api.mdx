---
title: API
description: "API reference for DreamStudio (Stability AI): every `dreamstudio.api.*` operation with input and output types."
---
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.dreamstudio.api.user.account({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.dreamstudio.api.user.balance({});
```
