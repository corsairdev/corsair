---
title: Overview
description: "DreamStudio (Stability AI) plugin for Corsair"
---

Use **DreamStudio (Stability AI)** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 4 typed API operations
- 5 synced entities (`accounts`, `balances`, `engines`, `images`, `organizations`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/dreamstudio
```
```bash yarn
yarn add corsair @corsair-dev/dreamstudio
```
```bash pnpm
pnpm install corsair @corsair-dev/dreamstudio
```
```bash bun
bun add corsair @corsair-dev/dreamstudio
```
</CodeGroup>
...
import { dreamstudio } from '@corsair-dev/dreamstudio';
...
await tenant.dreamstudio.api.engines.list({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.dreamstudio.api.generation.imageFromImage({});
```


See the full list on the [API](/plugins/dreamstudio/api) page.

## Query synced data

Synced entities support `tenant.dreamstudio.db.<entity>.search()` and `.list()`: `accounts`, `balances`, `engines`, `images`, `organizations`.

See [Database](/plugins/dreamstudio/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/dreamstudio/api">
    Every `dreamstudio.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/dreamstudio/database">
