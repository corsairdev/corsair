...
Use **CrowTerminal** through Corsair: one client, typed API calls, local DB sync, and incoming webhooks.

**What you get:**

- 28 typed API operations
- 4 synced entities (`dataPoints`, `incidents`, `skills`, `webhooks`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/crowterminal
```
```bash yarn
yarn add corsair @corsair-dev/crowterminal
```
```bash pnpm
pnpm install corsair @corsair-dev/crowterminal
```
```bash bun
bun add corsair @corsair-dev/crowterminal
```
</CodeGroup>
...
See the full list on the [API](/plugins/crowterminal/api) page.

## Query synced data

Synced entities support `tenant.crowterminal.db.<entity>.search()` and `.list()`: `dataPoints`, `incidents`, `skills`, `webhooks`.

See [Database](/plugins/crowterminal/database) for filters and operators.
...
See [Webhooks](/plugins/crowterminal/webhooks) for every event path and payload shape, and [Webhooks concept](/concepts/webhooks) for routing.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/crowterminal/api">
    Every `crowterminal.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/crowterminal/database">
    Synced entities, search filters, and operators.
  </Card>
  <Card title="Webhooks" href="/plugins/crowterminal/webhooks">
