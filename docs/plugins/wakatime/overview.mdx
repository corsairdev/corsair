---
title: Overview
description: "WakaTime plugin for Corsair"
---

Use **WakaTime** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/wakatime
```
```bash yarn
yarn add corsair @corsair-dev/wakatime
```
```bash pnpm
pnpm install corsair @corsair-dev/wakatime
```
```bash bun
bun add corsair @corsair-dev/wakatime
```
</CodeGroup>
...
await tenant.wakatime.api.users.current({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/wakatime/api) reference._


See the full list on the [API](/plugins/wakatime/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/wakatime/api">
    Every `wakatime.api.*` operation with input and output types.
  </Card>
