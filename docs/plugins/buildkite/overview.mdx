---
title: Overview
description: "Buildkite plugin for Corsair"
---

Use **Buildkite** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 5 typed API operations
- 5 synced entities (`accessTokens`, `agents`, `meta`, `organizations`, `users`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/buildkite
```
```bash yarn
yarn add corsair @corsair-dev/buildkite
```
```bash pnpm
pnpm install corsair @corsair-dev/buildkite
```
```bash bun
bun add corsair @corsair-dev/buildkite
```
</CodeGroup>
...
await tenant.buildkite.api.getCurrentAccessToken({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/buildkite/api) reference._


See the full list on the [API](/plugins/buildkite/api) page.

## Query synced data

Synced entities support `tenant.buildkite.db.<entity>.search()` and `.list()`: `accessTokens`, `agents`, `meta`, `organizations`, `users`.

See [Database](/plugins/buildkite/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/buildkite/api">
    Every `buildkite.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/buildkite/database">
