...
```ts
const tenant = corsair.withTenant('acme');
await tenant.buildkite.api.getMeta({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.buildkite.api.getUser({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.buildkite.api.listOrganizations({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.buildkite.api.listPipelineAgents({});
```
