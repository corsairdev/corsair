...
Use **Anonyflow** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/anonyflow
```
```bash yarn
yarn add corsair @corsair-dev/anonyflow
```
```bash pnpm
pnpm install corsair @corsair-dev/anonyflow
```
```bash bun
bun add corsair @corsair-dev/anonyflow
```
</CodeGroup>
...
await tenant.anonyflow.api.core.testConnection({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.anonyflow.api.core.anonymize({});
```


See the full list on the [API](/plugins/anonyflow/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/anonyflow/api">
    Every `anonyflow.api.*` operation with input and output types.
  </Card>
