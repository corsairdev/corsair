...
const rows = await tenant.amplitude.db.cohorts.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.amplitude.db.events.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.amplitude.db.users.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
