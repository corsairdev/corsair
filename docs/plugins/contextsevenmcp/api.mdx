---
title: API
description: "API reference for ContextSevenMcp: every `contextsevenmcp.api.*` operation with input and output types."
---
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.contextsevenmcp.api.library.search({});
```
