---
title: Overview
description: "ContextSevenMcp plugin for Corsair"
---

Use **ContextSevenMcp** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/contextsevenmcp
```
```bash yarn
yarn add corsair @corsair-dev/contextsevenmcp
```
```bash pnpm
pnpm install corsair @corsair-dev/contextsevenmcp
```
```bash bun
bun add corsair @corsair-dev/contextsevenmcp
```
</CodeGroup>
...
import { contextsevenmcp } from '@corsair-dev/contextsevenmcp';
...
await tenant.contextsevenmcp.api.context.get({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/contextsevenmcp/api) reference._


See the full list on the [API](/plugins/contextsevenmcp/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/contextsevenmcp/api">
    Every `contextsevenmcp.api.*` operation with input and output types.
  </Card>
