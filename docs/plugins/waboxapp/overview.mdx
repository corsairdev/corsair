---
title: Overview
description: "Waboxapp plugin for Corsair"
---

Use **Waboxapp** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 5 typed API operations
- 2 synced entities (`accounts`, `messages`) for fast `.search()` / `.list()`
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.waboxapp.api.messages.sendChat({});
```
...
Synced entities support `tenant.waboxapp.db.<entity>.search()` and `.list()`: `accounts`, `messages`.

See [Database](/plugins/waboxapp/database) for filters and operators.
...
<CardGroup cols={2}>
  ...
  <Card title="Database" href="/plugins/waboxapp/database">
