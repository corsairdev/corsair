...
Use **Hookdeck** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/hookdeck
```
```bash yarn
yarn add corsair @corsair-dev/hookdeck
```
```bash pnpm
pnpm install corsair @corsair-dev/hookdeck
```
```bash bun
bun add corsair @corsair-dev/hookdeck
```
</CodeGroup>
...
import { hookdeck } from '@corsair-dev/hookdeck';
...
await tenant.hookdeck.api.connections.get({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.hookdeck.api.connections.create({});
```


See the full list on the [API](/plugins/hookdeck/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/hookdeck/api">
    Every `hookdeck.api.*` operation with input and output types.
  </Card>
