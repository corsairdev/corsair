...
```ts
const rows = await corsair.backendless.db.dataObjects.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const rows = await corsair.backendless.db.files.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const rows = await corsair.backendless.db.messages.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const rows = await corsair.backendless.db.users.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
