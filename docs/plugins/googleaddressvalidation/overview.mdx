...
Use **Google Address Validation** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/googleaddressvalidation
```
```bash yarn
yarn add corsair @corsair-dev/googleaddressvalidation
```
```bash pnpm
pnpm install corsair @corsair-dev/googleaddressvalidation
```
```bash bun
bun add corsair @corsair-dev/googleaddressvalidation
```
</CodeGroup>
...
const { connectUrl } = await corsair.manage.connect.createLink({
	plugin: 'googleaddressvalidation',
	tenantId: 'acme',
});
...
await tenant.googleaddressvalidation.api.address.validate({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.googleaddressvalidation.api.address.provideFeedback({});
```


See the full list on the [API](/plugins/googleaddressvalidation/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/googleaddressvalidation/api">
    Every `googleaddressvalidation.api.*` operation with input and output types.
  </Card>
