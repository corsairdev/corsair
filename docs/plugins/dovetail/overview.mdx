---
title: Overview
description: "Dovetail plugin for Corsair"
---

Use **Dovetail** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 51 typed API operations
- 13 synced entities (`channels`, `contacts`, `data`, `dataPoints`, `docs`, `files`, and 7 more) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/dovetail
```
```bash yarn
yarn add corsair @corsair-dev/dovetail
```
```bash pnpm
pnpm install corsair @corsair-dev/dovetail
```
```bash bun
bun add corsair @corsair-dev/dovetail
```
</CodeGroup>
...
import { dovetail } from '@corsair-dev/dovetail';
...
await tenant.dovetail.api.contacts.get({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.dovetail.api.channels.create({});
```


See the full list on the [API](/plugins/dovetail/api) page.

## Query synced data

Synced entities support `tenant.dovetail.db.<entity>.search()` and `.list()`: `channels`, `contacts`, `data`, `dataPoints`, `docs`, `files`, `folders`, `highlights`, `insights`, `notes`, `projects`, `tags`, `topics`.

See [Database](/plugins/dovetail/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/dovetail/api">
    Every `dovetail.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/dovetail/database">
