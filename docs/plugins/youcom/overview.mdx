...
Use **You.com Web Search** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 1 typed API operations
- 1 synced entity (`searchResults`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/youcom
```
```bash yarn
yarn add corsair @corsair-dev/youcom
```
```bash pnpm
pnpm install corsair @corsair-dev/youcom
```
```bash bun
bun add corsair @corsair-dev/youcom
```
</CodeGroup>
...
await tenant.youcom.api.yousearch.youSearch({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/youcom/api) reference._


See the full list on the [API](/plugins/youcom/api) page.

## Query synced data

Synced entities support `tenant.youcom.db.<entity>.search()` and `.list()`: `searchResults`.

See [Database](/plugins/youcom/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/youcom/api">
    Every `youcom.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/youcom/database">
