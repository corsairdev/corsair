---
title: Database
description: "You.com Web Search local sync: searchable entities, `.search()` filters, and operators."
---

The You.com Web Search plugin syncs data locally. Use `corsair.youcom.db.<entity>.search({ data, limit?, offset? })` with the filters listed per entity.
...
const rows = await tenant.youcom.db.searchResults.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
