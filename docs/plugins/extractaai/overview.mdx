---
title: Overview
description: "Extractaai plugin for Corsair"
---

Use **Extractaai** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 10 typed API operations
- 2 synced entities (`classifications`, `extractions`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/extractaai
```
```bash yarn
yarn add corsair @corsair-dev/extractaai
```
```bash pnpm
pnpm install corsair @corsair-dev/extractaai
```
```bash bun
bun add corsair @corsair-dev/extractaai
```
</CodeGroup>
...
import { extractaai } from '@corsair-dev/extractaai';
...
await tenant.extractaai.api.classification.view({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.extractaai.api.classification.create({});
```


See the full list on the [API](/plugins/extractaai/api) page.

## Query synced data

Synced entities support `tenant.extractaai.db.<entity>.search()` and `.list()`: `classifications`, `extractions`.

See [Database](/plugins/extractaai/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/extractaai/api">
    Every `extractaai.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/extractaai/database">
