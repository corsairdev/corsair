---
title: Overview
description: "remove.bg plugin for Corsair"
---

Use **remove.bg** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/removebg
```
```bash yarn
yarn add corsair @corsair-dev/removebg
```
```bash pnpm
pnpm install corsair @corsair-dev/removebg
```
```bash bun
bun add corsair @corsair-dev/removebg
```
</CodeGroup>
...
import { removebg } from '@corsair-dev/removebg';
...
await tenant.removebg.api.account.get({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.removebg.api.improvement.submit({});
```


See the full list on the [API](/plugins/removebg/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/removebg/api">
    Every `removebg.api.*` operation with input and output types.
  </Card>
