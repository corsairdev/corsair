---
title: Overview
description: "Beamer plugin for Corsair"
---

Use **Beamer** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/beamer
```
```bash yarn
yarn add corsair @corsair-dev/beamer
```
```bash pnpm
pnpm install corsair @corsair-dev/beamer
```
```bash bun
bun add corsair @corsair-dev/beamer
```
</CodeGroup>
...
await tenant.beamer.api.posts.get({});
```
...
See the full list on the [API](/plugins/beamer/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/beamer/api">
    Every `beamer.api.*` operation with input and output types.
  </Card>
