---
title: Overview
description: "Rootly plugin for Corsair"
---

Use **Rootly** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/rootly
```
```bash yarn
yarn add corsair @corsair-dev/rootly
```
```bash pnpm
pnpm install corsair @corsair-dev/rootly
```
```bash bun
bun add corsair @corsair-dev/rootly
```
</CodeGroup>
...
import { rootly } from '@corsair-dev/rootly';
...
await tenant.rootly.api.actionItems.get({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.rootly.api.actionItems.delete({});
```


See the full list on the [API](/plugins/rootly/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/rootly/api">
    Every `rootly.api.*` operation with input and output types.
  </Card>
