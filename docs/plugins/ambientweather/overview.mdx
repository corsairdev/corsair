...
Use **AmbientWeather** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 2 typed API operations
- 2 synced entities (`devices`, `readings`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/ambientweather
```
```bash yarn
yarn add corsair @corsair-dev/ambientweather
```
```bash pnpm
pnpm install corsair @corsair-dev/ambientweather
```
```bash bun
bun add corsair @corsair-dev/ambientweather
```
</CodeGroup>
...
await tenant.ambientweather.api.devices.getData({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/ambientweather/api) reference._


See the full list on the [API](/plugins/ambientweather/api) page.

## Query synced data

Synced entities support `tenant.ambientweather.db.<entity>.search()` and `.list()`: `devices`, `readings`.

See [Database](/plugins/ambientweather/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/ambientweather/api">
    Every `ambientweather.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/ambientweather/database">
