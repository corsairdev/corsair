---
title: API
description: "API reference for AmbientWeather: every `ambientweather.api.*` operation with input and output types."
---
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.ambientweather.api.devices.list({});
```
