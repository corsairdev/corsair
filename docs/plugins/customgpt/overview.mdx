...
Use **CustomGPT** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 40 typed API operations
- 7 synced entities (`conversations`, `leads`, `licenses`, `messages`, `pages`, `projects`, and 1 more) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/customgpt
```
```bash yarn
yarn add corsair @corsair-dev/customgpt
```
```bash pnpm
pnpm install corsair @corsair-dev/customgpt
```
```bash bun
bun add corsair @corsair-dev/customgpt
```
</CodeGroup>
...
See the full list on the [API](/plugins/customgpt/api) page.

## Query synced data

Synced entities support `tenant.customgpt.db.<entity>.search()` and `.list()`: `conversations`, `leads`, `licenses`, `messages`, `pages`, `projects`, `sources`.

See [Database](/plugins/customgpt/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/customgpt/api">
    Every `customgpt.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/customgpt/database">
