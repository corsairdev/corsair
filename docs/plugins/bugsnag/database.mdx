...
const rows = await tenant.bugsnag.db.collaborators.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.bugsnag.db.organizations.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.bugsnag.db.projects.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.bugsnag.db.teams.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
