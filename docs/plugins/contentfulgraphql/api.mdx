---
title: API
description: "API reference for Contentful GraphQL: every `contentfulgraphql.api.*` operation with input and output types."
---
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.contentfulgraphql.api.graphQlContentApiPersistedQuery({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.contentfulgraphql.api.graphQlContentApiQuery({});
```
