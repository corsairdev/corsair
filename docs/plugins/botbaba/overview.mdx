---
title: Overview
description: "Botbaba plugin for Corsair"
---

Use **Botbaba** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 43 typed API operations
- 7 synced entities (`broadcasts`, `contacts`, `flows`, `messages`, `tags`, `templates`, and 1 more) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/botbaba
```
```bash yarn
yarn add corsair @corsair-dev/botbaba
```
```bash pnpm
pnpm install corsair @corsair-dev/botbaba
```
```bash bun
bun add corsair @corsair-dev/botbaba
```
</CodeGroup>
...
import { botbaba } from '@corsair-dev/botbaba';
...
See the full list on the [API](/plugins/botbaba/api) page.

## Query synced data

Synced entities support `tenant.botbaba.db.<entity>.search()` and `.list()`: `broadcasts`, `contacts`, `flows`, `messages`, `tags`, `templates`, `webhooks`.

See [Database](/plugins/botbaba/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/botbaba/api">
    Every `botbaba.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/botbaba/database">
