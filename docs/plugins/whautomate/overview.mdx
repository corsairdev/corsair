---
title: Overview
description: "Whautomate plugin for Corsair"
---

Use **Whautomate** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/whautomate
```
```bash yarn
yarn add corsair @corsair-dev/whautomate
```
```bash pnpm
pnpm install corsair @corsair-dev/whautomate
```
```bash bun
bun add corsair @corsair-dev/whautomate
```
</CodeGroup>
...
import { whautomate } from '@corsair-dev/whautomate';
...
await tenant.whautomate.api.account.getAccountInfo({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.whautomate.api.contacts.addContact({});
```


See the full list on the [API](/plugins/whautomate/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/whautomate/api">
    Every `whautomate.api.*` operation with input and output types.
  </Card>
