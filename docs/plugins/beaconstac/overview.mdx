---
title: Overview
description: "Beaconstac plugin for Corsair"
---

Use **Beaconstac** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 21 typed API operations
- 7 synced entities (`bulkQrcodes`, `organizations`, `places`, `qrcodes`, `qrTemplates`, `tags`, and 1 more) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/beaconstac
```
```bash yarn
yarn add corsair @corsair-dev/beaconstac
```
```bash pnpm
pnpm install corsair @corsair-dev/beaconstac
```
```bash bun
bun add corsair @corsair-dev/beaconstac
```
</CodeGroup>
...
import { beaconstac } from '@corsair-dev/beaconstac';
...
See the full list on the [API](/plugins/beaconstac/api) page.

## Query synced data

Synced entities support `tenant.beaconstac.db.<entity>.search()` and `.list()`: `bulkQrcodes`, `organizations`, `places`, `qrcodes`, `qrTemplates`, `tags`, `users`.

See [Database](/plugins/beaconstac/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/beaconstac/api">
    Every `beaconstac.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/beaconstac/database">
