...
const rows = await tenant.mailchimp.db.campaigns.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.mailchimp.db.lists.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.mailchimp.db.members.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
