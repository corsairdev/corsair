...
```ts
const rows = await corsair.brandfetch.db.brands.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const rows = await corsair.brandfetch.db.companies.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const rows = await corsair.brandfetch.db.webhooks.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
