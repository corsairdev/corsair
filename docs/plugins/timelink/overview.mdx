---
title: Overview
description: "Timelink plugin for Corsair"
---

Use **Timelink** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/timelink
```
```bash yarn
yarn add corsair @corsair-dev/timelink
```
```bash pnpm
pnpm install corsair @corsair-dev/timelink
```
```bash bun
bun add corsair @corsair-dev/timelink
```
</CodeGroup>
...
import { timelink } from '@corsair-dev/timelink';
...
_No read-style endpoint inferred; pick any operation from the [API](/plugins/timelink/api) reference._
...
await tenant.timelink.api.deletePerson.delete({});
```


See the full list on the [API](/plugins/timelink/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/timelink/api">
    Every `timelink.api.*` operation with input and output types.
  </Card>
