...
Use **Aeroleads** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 1 typed API operations
- 1 synced entity (`linkedinDetails`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/aeroleads
```
```bash yarn
yarn add corsair @corsair-dev/aeroleads
```
```bash pnpm
pnpm install corsair @corsair-dev/aeroleads
```
```bash bun
bun add corsair @corsair-dev/aeroleads
```
</CodeGroup>
...
await tenant.aeroleads.api.linkedinDetails.get({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/aeroleads/api) reference._


See the full list on the [API](/plugins/aeroleads/api) page.

## Query synced data

Synced entities support `tenant.aeroleads.db.<entity>.search()` and `.list()`: `linkedinDetails`.

See [Database](/plugins/aeroleads/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/aeroleads/api">
    Every `aeroleads.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/aeroleads/database">
