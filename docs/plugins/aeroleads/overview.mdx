...
Use **Aeroleads** through Corsair: one client, typed API calls, optional local DB sync.
...
Follow [Get Credentials](/plugins/aeroleads/get-credentials) if you need help getting keys.
...
Use the key names documented in [Get Credentials](/plugins/aeroleads/get-credentials) (for example `api_key=`, `bot_token=`, or OAuth client fields).
</Tab>
<Tab title="Multi-Tenant">
```bash
...
Store credentials with `pnpm corsair setup --plugin=aeroleads` (see [Get Credentials](/plugins/aeroleads/get-credentials) for field names). For OAuth, you typically store integration keys at the provider level and tokens per account or tenant.
...
**Read-style (read):** `linkedinDetails.get`
