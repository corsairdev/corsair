---
title: Overview
description: "Bannerbear plugin for Corsair"
---

Use **Bannerbear** through Corsair: one client, typed API calls, and incoming webhooks.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/bannerbear
```
```bash yarn
yarn add corsair @corsair-dev/bannerbear
```
```bash pnpm
pnpm install corsair @corsair-dev/bannerbear
```
```bash bun
bun add corsair @corsair-dev/bannerbear
```
</CodeGroup>
...
import { bannerbear } from '@corsair-dev/bannerbear';
...
See the full list on the [API](/plugins/bannerbear/api) page.
...
See [Webhooks](/plugins/bannerbear/webhooks) for every event path and payload shape, and [Webhooks concept](/concepts/webhooks) for routing.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/bannerbear/api">
    Every `bannerbear.api.*` operation with input and output types.
  </Card>
  <Card title="Webhooks" href="/plugins/bannerbear/webhooks">
