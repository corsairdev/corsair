...
Use the key names documented in [Get Credentials](/plugins/cloudinary/get-credentials) (`api_key=`, `api_secret=`, `cloud_name=`).
</Tab>
<Tab title="Multi-Tenant">
```bash
...
See [Get Credentials](/plugins/cloudinary/get-credentials) for how to obtain and store secrets. This plugin supports **API key** auth only (`api_key` + `api_secret` + `cloud_name`).
