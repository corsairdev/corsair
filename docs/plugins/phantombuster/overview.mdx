---
title: Overview
description: "Web automation and data extraction platform for scraping websites and automating workflows."
---

Use **PhantomBuster** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/phantombuster
```
```bash yarn
yarn add corsair @corsair-dev/phantombuster
```
```bash pnpm
pnpm install corsair @corsair-dev/phantombuster
```
```bash bun
bun add corsair @corsair-dev/phantombuster
```
</CodeGroup>
...
import { phantombuster } from '@corsair-dev/phantombuster';
...
await tenant.phantombuster.api.agents.fetch({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.phantombuster.api.agents.delete({});
```


See the full list on the [API](/plugins/phantombuster/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/phantombuster/api">
    Every `phantombuster.api.*` operation with input and output types.
  </Card>
