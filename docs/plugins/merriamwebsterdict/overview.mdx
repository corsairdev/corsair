---
title: Overview
description: "Merriam-Webster Dictionary plugin for Corsair"
---

Use **Merriam-Webster Dictionary** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 1 typed API operations
- 1 synced entity (`entries`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/merriamwebsterdict
```
```bash yarn
yarn add corsair @corsair-dev/merriamwebsterdict
```
```bash pnpm
pnpm install corsair @corsair-dev/merriamwebsterdict
```
```bash bun
bun add corsair @corsair-dev/merriamwebsterdict
```
</CodeGroup>
...
import { merriamwebsterdict } from '@corsair-dev/merriamwebsterdict';
...
await tenant.merriamwebsterdict.api.words.get({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/merriamwebsterdict/api) reference._


See the full list on the [API](/plugins/merriamwebsterdict/api) page.

## Query synced data

Synced entities support `tenant.merriamwebsterdict.db.<entity>.search()` and `.list()`: `entries`.

See [Database](/plugins/merriamwebsterdict/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/merriamwebsterdict/api">
    Every `merriamwebsterdict.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/merriamwebsterdict/database">
