---
title: Overview
description: "Bolt IoT plugin for Corsair"
---

Use **Bolt IoT** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 7 typed API operations
- 2 synced entities (`commands`, `devices`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/boltiot
```
```bash yarn
yarn add corsair @corsair-dev/boltiot
```
```bash pnpm
pnpm install corsair @corsair-dev/boltiot
```
```bash bun
bun add corsair @corsair-dev/boltiot
```
</CodeGroup>
...
import { boltiot } from '@corsair-dev/boltiot';
...
await tenant.boltiot.api.device.analogRead({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.boltiot.api.device.digitalWrite({});
```


See the full list on the [API](/plugins/boltiot/api) page.

## Query synced data

Synced entities support `tenant.boltiot.db.<entity>.search()` and `.list()`: `commands`, `devices`.

See [Database](/plugins/boltiot/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/boltiot/api">
    Every `boltiot.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/boltiot/database">
