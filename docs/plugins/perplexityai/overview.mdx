...
Use **PerplexityAi** through Corsair: one client, typed API calls.
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/perplexityai
```
```bash yarn
yarn add corsair @corsair-dev/perplexityai
```
```bash pnpm
pnpm install corsair @corsair-dev/perplexityai
```
```bash bun
bun add corsair @corsair-dev/perplexityai
```
</CodeGroup>
...
await tenant.perplexityai.api.chat.completions({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/perplexityai/api) reference._


See the full list on the [API](/plugins/perplexityai/api) page.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/perplexityai/api">
    Every `perplexityai.api.*` operation with input and output types.
  </Card>
