---
title: Database
description: "Open Weather Map local sync: searchable entities, `.search()` filters, and operators."
---
...
const rows = await tenant.openweathermap.db.daySummaries.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.openweathermap.db.overviews.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
