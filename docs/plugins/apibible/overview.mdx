...
Use **API.Bible** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 19 typed API operations
- 8 synced entities (`audioBibles`, `audioChapters`, `bibles`, `books`, `chapters`, `passages`, and 2 more) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/apibible
```
```bash yarn
yarn add corsair @corsair-dev/apibible
```
```bash pnpm
pnpm install corsair @corsair-dev/apibible
```
```bash bun
bun add corsair @corsair-dev/apibible
```
</CodeGroup>
...
await tenant.apibible.api.audioBibles.get({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/apibible/api) reference._


See the full list on the [API](/plugins/apibible/api) page.

## Query synced data

Synced entities support `tenant.apibible.db.<entity>.search()` and `.list()`: `audioBibles`, `audioChapters`, `bibles`, `books`, `chapters`, `passages`, `sections`, `verses`.

See [Database](/plugins/apibible/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/apibible/api">
    Every `apibible.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/apibible/database">
