---
title: Overview
description: "Document generation and template rendering API for creating files from data-driven templates."
---
...
- 3 synced entities (`categories`, `tags`, `templates`) for fast `.search()` / `.list()`
...
await tenant.carbone.api.templates.list({ id: 'tmpl_1234567890abcdef', cursor: 0 });
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.carbone.api.render.generateReport({
...
Search synced templates without calling Carbone.

```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.carbone.db.templates.search({
	data: { category: 'Finance' },
	limit: 50,
});
```

Synced entities: `categories`, `tags`, `templates`. See [Database](/plugins/carbone/database) for filters and operators.
