...
const rows = await tenant.agentql.db.accountUsage.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.agentql.db.browserSessions.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.agentql.db.documentQueryResults.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.agentql.db.queryResults.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
