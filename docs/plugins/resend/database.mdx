...
Path: `resend.db.contacts.search`

```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.resend.db.contacts.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.resend.db.domains.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
