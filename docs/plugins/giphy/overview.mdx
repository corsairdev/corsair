---
title: Overview
description: "The world's largest library of animated GIFs, stickers, and emojis for apps and agents."
---

Use **GIPHY** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 13 typed API operations
- 2 synced entities (`categories`, `gifs`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/giphy
```
```bash yarn
yarn add corsair @corsair-dev/giphy
```
```bash pnpm
pnpm install corsair @corsair-dev/giphy
```
```bash bun
bun add corsair @corsair-dev/giphy
```
</CodeGroup>
...
import { giphy } from '@corsair-dev/giphy';
...
await tenant.giphy.api.categories.list({});
```


**Write**

_No write-style endpoint inferred; pick any operation from the [API](/plugins/giphy/api) reference._


See the full list on the [API](/plugins/giphy/api) page.

## Query synced data

Synced entities support `tenant.giphy.db.<entity>.search()` and `.list()`: `categories`, `gifs`.

See [Database](/plugins/giphy/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/giphy/api">
    Every `giphy.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/giphy/database">
