...
```ts
const tenant = corsair.withTenant('acme');
await tenant.airtable.api.bases.getSchema({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.airtable.api.records.get({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.airtable.api.records.search({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.airtable.api.webhooks.getPayloads({});
```
