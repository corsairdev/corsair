...
const rows = await tenant.ambee.db.airQualityReadings.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.ambee.db.geocodedPlaces.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.ambee.db.weatherObservations.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
