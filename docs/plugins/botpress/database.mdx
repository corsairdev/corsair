...
const rows = await tenant.botpress.db.bots.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.botpress.db.integrations.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
...
```ts
const tenant = corsair.withTenant('acme');
const rows = await tenant.botpress.db.workspaces.search({
    data: { /* filters below */ },
    limit: 100,
    offset: 0,
});
