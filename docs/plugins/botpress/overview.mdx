---
title: Overview
description: "Botpress plugin for Corsair"
---

Use **Botpress** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 53 typed API operations
- 3 synced entities (`bots`, `integrations`, `workspaces`) for fast `.search()` / `.list()`

## Setup

<Steps>

<Step title="Install">
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/botpress
```
```bash yarn
yarn add corsair @corsair-dev/botpress
```
```bash pnpm
pnpm install corsair @corsair-dev/botpress
```
```bash bun
bun add corsair @corsair-dev/botpress
```
</CodeGroup>

</Step>

<Step title="Add the plugin">
```ts corsair.ts
import Database from 'better-sqlite3';
import { createCorsair } from 'corsair';
import { botpress } from '@corsair-dev/botpress';

export const corsair = createCorsair({
	plugins: [
		botpress(),
	],
	database: new Database('corsair.db'),
	kek: process.env.CORSAIR_KEK!,
	hub: {
		projectApiKey: process.env.CORSAIR_DEV_API_KEY!,
		signingSecret: process.env.CORSAIR_DEV_SIGNING_SECRET!,
	},
});
```

Multi-tenancy is the default — scope calls with `corsair.withTenant(id)`. See [Quick start](/quick-start) for KEK + Hub keys, and [Multi-tenancy](/concepts/multi-tenancy) for account isolation.

</Step>

<Step title="Choose authentication">
<Tabs>
<Tab title="API Key (Recommended)">

No setup required yet. When you make your first request as a tenant, Corsair prompts for the API key.

```ts
botpress()
```

More: [API Key](/concepts/api-key)

</Tab>
</Tabs>

</Step>

<Step title="Connect a tenant">
Mint a connect link and send the tenant to it. Hub hosts the page and delivers the result to your app — see [Connect / OAuth](/management/connect).

```ts
const { connectUrl } = await corsair.manage.connect.createLink({
	plugin: 'botpress',
	tenantId: 'acme',
});
// redirect the user's browser to connectUrl
```

</Step>

</Steps>

## Example API calls

**`account.get`**

```ts
const tenant = corsair.withTenant('acme');
await tenant.botpress.api.account.get({});
```


**`account.setPreference`**

```ts
const tenant = corsair.withTenant('acme');
await tenant.botpress.api.account.setPreference({});
```


See the full list on the [API](/plugins/botpress/api) page.

## Query synced data

Synced entities support `tenant.botpress.db.<entity>.search()` and `.list()`: `bots`, `integrations`, `workspaces`.

See [Database](/plugins/botpress/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/botpress/api">
    Every `botpress.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/botpress/database">
    Synced entities, search filters, and operators.
  </Card>
  <Card title="Connect / OAuth" href="/management/connect">
    createLink, Hub delivery, and tenant connect flows.
  </Card>
  <Card title="Use with agents" href="/mcp-adapters/mcp-adapters">
    Expose this plugin's operations as MCP tools.
  </Card>
</CardGroup>
