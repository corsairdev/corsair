---
title: Overview
description: "Spoki integration for Corsair"
---

Use **Spoki integration for Corsair** through Corsair: one client, typed API calls, local DB sync.

**What you get:**

- 60 typed API operations
- 2 synced entities (`accounts`, `channels`) for fast `.search()` / `.list()`
...
<CodeGroup>
```bash npm
npm install corsair @corsair-dev/spoki
```
```bash yarn
yarn add corsair @corsair-dev/spoki
```
```bash pnpm
pnpm install corsair @corsair-dev/spoki
```
```bash bun
bun add corsair @corsair-dev/spoki
```
</CodeGroup>
...
import { spoki } from '@corsair-dev/spoki';
...
await tenant.spoki.api.accounts.getAccount({});
```
...
```ts
const tenant = corsair.withTenant('acme');
await tenant.spoki.api.accounts.createOnboardingLink({});
```


See the full list on the [API](/plugins/spoki/api) page.

## Query synced data

Synced entities support `tenant.spoki.db.<entity>.search()` and `.list()`: `accounts`, `channels`.

See [Database](/plugins/spoki/database) for filters and operators.

## What's next

<CardGroup cols={2}>
  <Card title="API reference" href="/plugins/spoki/api">
    Every `spoki.api.*` operation with input and output types.
  </Card>
  <Card title="Database" href="/plugins/spoki/database">
