...
<Note>These examples assume `multiTenancy: true` (the [quick-start](/getting-started/quick-start) setup), so they scope each call with `corsair.withTenant(ctx.tenantId)`. On a single-tenant instance, drop `withTenant` and call the plugin `.api` directly.</Note>
