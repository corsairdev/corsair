...
If several calls for the same tenant hit an expired token at once — parallel route handlers, or an agent firing multiple tools concurrently — Corsair runs the refresh once and shares the result. This matters for providers with rotating refresh tokens, where a second concurrent refresh would invalidate the connection.
