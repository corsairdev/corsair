...
If you don't want to build connect pages, use `hub: { ... }` instead of `manual`. Call the same `createLink` API, and the URL points to Corsair Hub's hosted UI. Hub hosts the connect surfaces and stores none of your credentials. See [Hub overview](/hub/overview) for the model and [Connect / OAuth](/management/connect#hub-mode-hosted-connect-ui) for the API.
