...
Or from the client via the [management client](/management/client):
...
<Info>
See [Production: OAuth Process](/production/oauth-process) for a full implementation with security best practices — authenticated link creation, HTML escaping, and production checklists.
</Info>
...
If you don't want to build connect pages, use `hub: { ... }` instead of `manual`. Call the same `createLink` API — the URL points to Corsair Hub's hosted UI. See [Connect / OAuth](/management/connect#hub-mode-hosted-connect-ui).
