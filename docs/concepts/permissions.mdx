...
Constraints only ever loosen or tighten a decision you already configured; they never grant access the mode matrix wouldn't. A constraint is **not** satisfied when:

- the argument is missing, or is the wrong type for the operator (a number never matches a regex)
- the regex is invalid — a malformed pattern is a config bug, not an open door
- the constraints object is empty

In each case the override contributes nothing and evaluation falls back to `otherwise` or the mode matrix.
