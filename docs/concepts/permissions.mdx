...
Control how blocked calls behave with `approval.mode`. Agent-facing messages come from **hub** (hosted, automatic) or **`manual.onApprovalRequired`** (self-hosted review URLs).
...
With **hub** config, Corsair automatically returns a hosted approval URL in the agent message. With **manual** config, set `approvalBaseUrl` and optionally customize via `onApprovalRequired`:
...
When using [MCP adapters](/mcp-adapters/mcp-adapters), permissions gate `run_script` calls automatically. Configure `permissions` on each plugin and `approval` on `createCorsair`.
