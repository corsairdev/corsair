...
Run the migration once, then pass your connection to `createCorsair({ database, ... })`. See [Quick Start](/quick-start) for a full setup example. Existing installs: add `corsair_connects` if it is missing, then run the two `CREATE INDEX` statements.
...
<CodeGroup>
  ```bash npm npm install better-sqlite3 ``` ```bash yarn yarn add
  better-sqlite3 ``` ```bash pnpm pnpm install better-sqlite3 ``` ```bash bun
  bun add better-sqlite3 ```
</CodeGroup>
<CodeGroup>
  ```bash npm npm install --save-dev @types/better-sqlite3 ``` ```bash yarn yarn
  add --dev @types/better-sqlite3 ``` ```bash pnpm pnpm install --save-dev
  @types/better-sqlite3 ``` ```bash bun bun add --dev @types/better-sqlite3 ```
</CodeGroup>
...
<CodeGroup>
  ```bash npm npm install pg ``` ```bash yarn yarn add pg ``` ```bash pnpm pnpm
  install pg ``` ```bash bun bun add pg ```
</CodeGroup>
<CodeGroup>
  ```bash npm npm install --save-dev @types/pg ``` ```bash yarn yarn add --dev
  @types/pg ``` ```bash pnpm pnpm install --save-dev @types/pg ``` ```bash bun
...
export const corsairConnects = pgTable('corsair_connects', {
...
model CorsairAccount {
  ...
  @@index([tenantId, integrationId], map: "corsair_accounts_tenant_integration_idx")
...
model CorsairEntity {
  ...
  @@index([accountId, entityType, entityId], map: "corsair_entities_account_type_entity_idx")
...
Six tables. The first five are shared by every integration — Slack, GitHub, Linear, and the rest use the same schema. `corsair_connects` records a tenant's pending connect prompt.
...
| `corsair_connects`     | A tenant's pending connect prompt, written when a call needs auth and read by the client to open the connect dialog (`tenant_id`, `plugin`, `connect_url`) |
...
await tenant.github.db.repositories.search({
  data: { name: { contains: 'api' } },
})
```
...
const [entity] = await tenant.slack.db.messages.search({
  entity_id: posted.ts!,
})
...
const rows = await db
  ...
  .innerJoin(
    corsairEntities,
    eq(supportTickets.slackMessageId, corsairEntities.id)
  )
