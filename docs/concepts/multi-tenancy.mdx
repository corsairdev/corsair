...
Pass any stable user identifier, such as a database ID or auth provider ID:
...
## Concurrent requests for one tenant

Call `withTenant()` as often as you like — once per request handler, or once
per parallel tool call in an agent workflow. Clients for the same tenant share
one credential manager, so concurrent OAuth refreshes collapse into a single
token request instead of spending the same rotating refresh token twice (which
would revoke the connection). Tenants never share refresh state with each
other.
