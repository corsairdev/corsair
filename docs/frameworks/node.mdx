...
The adapter ships in core `corsair` — nothing extra for `node:http`. Add a plugin package per service you integrate (`@corsair-dev/github`, and so on).
...
`./corsair` is your `createCorsair({ ... })` instance — see [Getting Started](/getting-started/quick-start) if you haven't built it yet. Already have a server? Mount it behind a path check instead: `if (req.url?.startsWith('/api/corsair')) return handler(req, res)`.
