...
export const { GET, POST } = toTanStackHandler(corsair, { basePath: '/api/corsair' });
```

`~/server/corsair` is your `createCorsair({ ... })` instance — see [Getting Started](/getting-started/quick-start) if you haven't built it yet.

<Note>Server-route helpers have shifted across TanStack Start releases. If your version wraps routes differently, keep the shape: a splat route under `/api/corsair` whose `GET`/`POST` come from `toTanStackHandler`.</Note>

## Resolve the tenant

Every read and write is scoped to a tenant id — whatever stable id identifies the current user or org. A server function runs on the server, so read your session from its request context (however your auth works) and pass the id to `withTenant`.

```tsx
const getTenant = createServerFn().handler(async () => {
    const tenantId = await getTenantId(); // getTenantId is YOUR code
...
Deploy the TanStack Start app as usual — the server route ships with it. In production, `/api/corsair` at your deployed origin is the delivery URL Hub calls.
