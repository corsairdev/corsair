...
Corsair only needs the tenant id — your own auth produces it. Decorate the request with the authenticated user (in a hook or auth plugin), then read it off `request.user`:
...
`getTenantId(request)` is your own code — read the signed-in user from your auth; it can return any stable id.

## Read and write

Once a tenant is connected, reads never leave your database — perfect for a hot handler that can't afford a third-party round trip. Scope with `withTenant()`, then query the plugin's entities:
