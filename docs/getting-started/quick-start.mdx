...
import { WindowsEnvVarsNote, WindowsMigrationNote } from '/snippets/windows-shell.mdx';
...
Corsair needs four tables. Run this once, then you're done.
...
Pass the **underlying `pg` Pool** to Corsair — not the ORM client. Use Drizzle or Prisma for your own app tables on the same pool.
...
See [Database](/concepts/database) for SQLite, postgres.js, and Kysely options.
...
```
```bash yarn
yarn corsair setup --plugin=github api_key=$GITHUB_TOKEN --backfill
```
```bash pnpm
pnpm corsair setup --plugin=github api_key=$GITHUB_TOKEN --backfill
```
```bash bun
bunx corsair setup --plugin=github api_key=$GITHUB_TOKEN --backfill
```
</CodeGroup>
...
<CodeGroup>
```bash npm
npx corsair setup --plugin=github api_key=$env:GITHUB_TOKEN --backfill
```
```bash yarn
yarn corsair setup --plugin=github api_key=$env:GITHUB_TOKEN --backfill
```
```bash pnpm
pnpm corsair setup --plugin=github api_key=$env:GITHUB_TOKEN --backfill
```
```bash bun
bunx corsair setup --plugin=github api_key=$env:GITHUB_TOKEN --backfill
```
</CodeGroup>
...
See [MCP Adapters](/mcp-adapters/mcp-adapters) for Vercel AI SDK, Mastra, and more.
...
<CodeGroup>
```bash npm
npx corsair setup --plugin=github api_key=$GITHUB_TOKEN
```
```bash yarn
yarn corsair setup --plugin=github api_key=$GITHUB_TOKEN
```
```bash pnpm
pnpm corsair setup --plugin=github api_key=$GITHUB_TOKEN
```
```bash bun
bunx corsair setup --plugin=github api_key=$GITHUB_TOKEN
```
</CodeGroup>
...
<CodeGroup>
```bash npm
npx corsair setup --plugin=github api_key=$env:GITHUB_TOKEN
```
```bash yarn
yarn corsair setup --plugin=github api_key=$env:GITHUB_TOKEN
```
```bash pnpm
pnpm corsair setup --plugin=github api_key=$env:GITHUB_TOKEN
```
```bash bun
bunx corsair setup --plugin=github api_key=$env:GITHUB_TOKEN
```
</CodeGroup>
...
<CodeGroup>
```bash npm
npx corsair auth --plugin=slack
```
```bash yarn
yarn corsair auth --plugin=slack
```
```bash pnpm
pnpm corsair auth --plugin=slack
```
```bash bun
bunx corsair auth --plugin=slack
```
</CodeGroup>
...
<CodeGroup>
```bash npm
npx corsair list --plugin=github
```
```bash yarn
yarn corsair list --plugin=github
```
```bash pnpm
pnpm corsair list --plugin=github
```
```bash bun
bunx corsair list --plugin=github
```
</CodeGroup>

Run a script with `corsair` injected — use `return` to output a value:

<CodeGroup>
```bash npm
npx corsair script --code "return await corsair.github.api.repositories.list({})"
```
```bash yarn
yarn corsair script --code "return await corsair.github.api.repositories.list({})"
```
```bash pnpm
pnpm corsair script --code "return await corsair.github.api.repositories.list({})"
```
```bash bun
bunx corsair script --code "return await corsair.github.api.repositories.list({})"
```
</CodeGroup>
...
<CardGroup cols={2}>
  <Card title="Set up Webhooks" href="/concepts/webhooks">
