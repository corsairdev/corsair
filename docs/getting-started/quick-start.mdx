...
<CodeGroup>
```bash npm
npx corsair script --code "return (await corsair.github.db.repositories.search({})).map(r => ({ name: r.data?.name }))"
```
```bash yarn
yarn corsair script --code "return (await corsair.github.db.repositories.search({})).map(r => ({ name: r.data?.name }))"
```
```bash pnpm
pnpm corsair script --code "return (await corsair.github.db.repositories.search({})).map(r => ({ name: r.data?.name }))"
```
```bash bun
bunx corsair script --code "return (await corsair.github.db.repositories.search({})).map(r => ({ name: r.data?.name }))"
```
</CodeGroup>
