...
<Note>
Want to host those surfaces yourself instead? Every step below is the same; you just swap the `hub` block for `manual`. See [Manual or Hub](/hub/manual-vs-hub).
</Note>
...
Create a project in the [Hub dashboard](https://app.corsair.dev) and copy its API key and signing secret. Then generate a KEK — Corsair encrypts every stored credential with it:

<GenerateKEK />

```bash .env
CORSAIR_KEK=your-generated-kek
CORSAIR_API_KEY=your-hub-project-key
CORSAIR_SIGNING_SECRET=your-hub-signing-secret
...
export const corsair = createCorsair({
    ...
    hub: {
        ...
        deliveryUrl: `${process.env.APP_URL}/api/corsair`,
    },
});
```

Mount the handler once — it serves Hub delivery and the management API:
