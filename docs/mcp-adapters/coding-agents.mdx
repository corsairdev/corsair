...
Corsair provides two CLI commands that expose its full API surface without executing any operations:

- `pnpm corsair list`
- `pnpm corsair schema <path>`

These commands expose Corsair's API surface without executing operations, making them ideal for agent-driven discovery workflows.
...
Lists every available endpoint path.
