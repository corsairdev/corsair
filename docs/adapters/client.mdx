...
await client.connect.createLink({                 // POST /connect/link
  plugin: "github",
  tenantId: "acme",
});
```

`client.*` is the HTTP mirror of `corsair.manage.*`; `client.connect` also carries `.resolve` and `.oauthCallback`. Connect/OAuth methods are documented on the [Connect page](/management/connect).
