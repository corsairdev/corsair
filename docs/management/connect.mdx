---
title: Connect / OAuth
description: Connect users to Slack, GitHub, and other plugins with one createLink API, config-driven hub or manual mode.
---
...
The same API works in both modes. The **`hub`** or **`manual`** config on `createCorsair` picks the backend:
...
The signed `state` is embedded in `connectUrl` as a query parameter, so you do not need to handle it separately.
