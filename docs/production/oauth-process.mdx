...
If you prefer Corsair to host the connect UI, use `hub: { ... }` instead of `manual`. You only mint a link and mount a delivery endpoint — no connect page or callback route needed.

See [Connect / OAuth](/management/connect) for the full hub setup.
