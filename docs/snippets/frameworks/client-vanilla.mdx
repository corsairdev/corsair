...
`connectionStatus.get` returns a map of plugin id → `connected | missing_credentials | not_connected`, so you can show what's live and what still needs connecting.
