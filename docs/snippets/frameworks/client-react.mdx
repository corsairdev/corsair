...
`useConnectionStatus` returns a map of plugin id → `connected | missing_credentials | not_connected`, so your UI knows what's live and what still needs a connect flow.
