...
project. It has no dependencies and works on Python 3.9+. You give it two
values, and every plugin your runtime has is callable over HTTP.
...
Open your project's **Overview** page in the dashboard and copy:

- the **API key** (`ck_cloud_…`), a server secret
- the **URL** (`https://<vm>.corsair.cloud/<env>/api/corsair`)
...
The client refuses a plain `http://` URL unless the host is `localhost`, so the
key is never sent in cleartext by accident.
...
`POST {url}/acme/notion/call/pages.searchPage` with body `{"args": {"query": "roadmap"}}`.
