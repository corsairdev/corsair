...
The package lives in the `corsair` monorepo, under `clients/swift` — not at
the repository root, so SPM's `.package(url:)` can't resolve it as a remote
dependency yet (that needs a dedicated mirror repo or a root-level manifest).
Until then, add it as a local path dependency from within a checkout of
`corsair`:
