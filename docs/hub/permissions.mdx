...
<Info>
Approvals require the `corsair_permissions` table in both modes. That table is your system of record; Hub only holds the pending session while the review is open. See [Permissions](/concepts/permissions#add-the-permissions-table) for the migration.
</Info>
