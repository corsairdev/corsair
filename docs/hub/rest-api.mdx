...
Every request authenticates with your environment API key (`ck_dev_…` locally, `ck_prod_…` in production):
...
curl -X POST https://auth.corsair.dev/connect/sessions \
  ...
  -d '{
    ...
    "plugins": [{ "plugin": "github", "providerName": "GitHub", "authKind": "oauth", "oauthMode": "managed" }]
  }'
```
...
res = requests.post(
    ...
    headers={"Authorization": f"Bearer {os.environ['CORSAIR_API_KEY']}"},
    json={
        "tenantId": "user_123",
        "deliveryUrl": "https://yourapp.com/api/corsair",
        "plugins": [{"plugin": "github", "providerName": "GitHub", "authKind": "oauth", "oauthMode": "managed"}],
...
payload, _ := json.Marshal(map[string]any{
    "tenantId":    "user_123",
    "deliveryUrl": "https://yourapp.com/api/corsair",
    "plugins":     []map[string]string{{"plugin": "github", "providerName": "GitHub", "authKind": "oauth", "oauthMode": "managed"}},
})
req, _ := http.NewRequest("POST", "https://auth.corsair.dev/connect/sessions", bytes.NewReader(payload))
req.Header.Set("Authorization", "Bearer "+os.Getenv("CORSAIR_API_KEY"))
...
| Header | Value |
|--------|-------|
| `x-corsair-signature` | `sha256=<hex>`, the HMAC-SHA256 of the **raw request body**, keyed with your signing secret |
...
```bash cURL
curl https://auth.corsair.dev/projects/connections \
  -H "Authorization: Bearer $CORSAIR_API_KEY"
```
