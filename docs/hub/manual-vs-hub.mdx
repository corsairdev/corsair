...
- **Hub** is an optional upgrade. Corsair hosts those surfaces for you. It stores none of your credentials. See [Hub overview](/hub/overview).
...
| ------- | -------------------- | ---------------------- |
| OAuth callback URL | One per environment, registered with each provider | One callback, fanned out to every [delivery URL](/hub/delivery-urls) |
...
You also build the connect page, the OAuth callback route, and the approval review page. See [OAuth Process](/production/oauth-process) for the full implementation.
...
No connect page, callback route, or approval page to build. Mount the handler once and Hub delivers results to it.
...
sequenceDiagram
    ...
    Surface->>App: delivers result to your deliveryUrl
    App->>App: tokens encrypted into your DB
```
...
Choose **hub** when you would rather not build and host those surfaces, or when you want one provider callback to cover local, staging, and production at once.
