...
```json title="demo/testing/package.json"
{
  "dependencies": {
...
```ts title="demo/testing/src/server/corsair.ts"
import { stripe } from '@corsair-dev/stripe';
...
```ts title="demo/testing/src/scripts/test-script.ts"
...
<Callout type="info" title="Build watch required">
