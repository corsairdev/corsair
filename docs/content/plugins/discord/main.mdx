...
import { discord } from "corsair/plugins";
...
```ts title="corsair.ts"
discord({
    key: process.env.DISCORD_BOT_TOKEN, // [!code highlight]
})
```

For webhook signature verification, provide your app's Ed25519 public key:
...
The `publicKey` is found in the Discord Developer Portal under **Application > General Information**.
