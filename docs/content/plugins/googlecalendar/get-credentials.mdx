...
3. If prompted, configure the OAuth consent screen (see [Gmail plugin](/plugins/gmail/get-credentials) for detailed steps)
...
```ts
await corsair.withTenant('default').keys.googlecalendar.setClientId('your-client-id');
await corsair.withTenant('default').keys.googlecalendar.setClientSecret('your-client-secret');
await corsair.withTenant('default').googlecalendar.keys.setAccessToken('your-access-token');
await corsair.withTenant('default').googlecalendar.keys.setRefreshToken('your-refresh-token');
```
