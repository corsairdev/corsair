...
await corsair.slack.keys.setBotToken(process.env.SLACK_BOT_TOKEN!);
...
```ts
await corsair.slack.keys.setBotToken('xoxb-your-bot-token');
```
...
Store credentials after your OAuth app is configured:

```ts
await corsair.slack.keys.setClientId(process.env.SLACK_CLIENT_ID!);
await corsair.slack.keys.setClientSecret(process.env.SLACK_CLIENT_SECRET!);
await corsair.slack.keys.setRedirectUrl('https://yourapp.com/auth/slack/callback');
```

After users complete the OAuth flow, access tokens are stored automatically per tenant. See [Multi-Tenancy](/multi-tenancy) for the full setup.
