...
For API key authentication, provide a bot token from your Slack app:
...
The plugin will automatically retrieve the API key from your database if you don't provide it directly. See [Authentication](/concepts/auth) for details on managing credentials.
...
The plugin will automatically retrieve the access token from your database. See [Authentication](/concepts/auth) for OAuth 2.0 setup details.

### Webhook Signature Verification

For webhook signature verification, you can provide a signing secret:

```ts title="corsair.ts"
slack({
    signingSecret: process.env.SLACK_SIGNING_SECRET,
    // Optional: if not provided, uses database
})
```
    ...
    async ({ event }) => {
        ...
        if (channels[0]?.entity_id) {
            await tenant.slack.api.messages.post({
                ...
                text: `🆕 New Linear issue: *${issue.title}*\n${issue.url}`,
            });
        }
    }
);
