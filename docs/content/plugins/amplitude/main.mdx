...
import { amplitude } from "corsair/plugins";
...
```ts title="corsair.ts"
amplitude({
    key: process.env.AMPLITUDE_API_KEY, // [!code highlight]
})
```
...
amplitude({
    ...
    webhookSecret: process.env.AMPLITUDE_WEBHOOK_SECRET, // [!code highlight]
})
```
