...
Each plugin lives under `docs/content/plugins/<plugin>/`:

```text
docs/content/plugins/<plugin>/
...
| `error-handlers.mdx` | Plugin-specific error types and recovery patterns beyond [concepts/error-handling](/concepts/error-handling). |

The Slack docs use both; the **four-page skeleton** still applies—treat optional files as supplements.
...
| Optional | [`slack/get-credentials.mdx`](./slack/get-credentials.mdx), [`slack/error-handlers.mdx`](./slack/error-handlers.mdx) |
