...
- **[`api_key`](/content/concepts/api-key)** (default) - Use an Oura personal access token
...
<Tab title="Solo">
```bash
pnpm corsair setup --plugin=oura api_key=your-access-token
```
...
<Tab title="Multi-Tenant">
```bash
pnpm corsair setup --plugin=oura api_key=their-access-token --tenant=user_abc123
```
