...
Corsair stores your key using [envelope encryption](/concepts/auth#envelope-encryption). Your KEK encrypts a per-connection DEK, which encrypts the key itself. The key is never stored in plaintext.
...
```bash
pnpm corsair setup --linear api_key=lin_api_your-key-here
```
...
```bash
pnpm corsair setup --linear api_key=lin_api_tenant_key --tenant=user_abc123
```
