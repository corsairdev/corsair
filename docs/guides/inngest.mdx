...
export const notifyAssignee = inngest.createFunction(
    ...
    async ({ event }) => {
        const { title, url, assigneeId, tenantId } = event.data;

        // Use withTenant if you have multi-tenancy enabled
        const client = tenantId ? corsair.withTenant(tenantId) : corsair;

        // assigneeId is a Linear user id; resolve their email for the Slack lookup
        const assignee = await client.linear.api.users.get({ id: assigneeId });
...
export const weeklySprintDigest = inngest.createFunction(
    ...
    async () => {
        ...
        // Rows store flat stateId/teamId/assigneeId, not nested objects.
        // Compare stateId against configured open-state ids (comma-separated).
        const openStateIds = process.env.LINEAR_OPEN_STATE_IDS!.split(',');
        // Filter and sort in JS (list returns all rows; fields live on .data)
        const issues = all
            .filter((i) => openStateIds.includes(i.data.stateId))
        ...
        const lines = [
            ...
            '',
            // Rows carry assigneeId/stateId, not names. Resolve the related
            // records separately if you need human-readable names.
            ...issues.map(
                (i) => `• *${i.data.title}* (${i.data.identifier}) — ${i.data.assigneeId ?? 'Unassigned'}`,
            ),
        ];
