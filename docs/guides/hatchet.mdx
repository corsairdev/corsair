...
deployWorkflow.task('update-linear-project', async (ctx) => {
    ...
    const inReview = all.filter(
        (i) => i.data.state?.name === 'In Review' && i.data.team?.id === process.env.LINEAR_TEAM_ID,
    );
...
sprintReportWorkflow.task('post-sprint-report', async () => {
    ...
    const inProgress = all.filter((i) => i.data.state?.type === 'started').sort(byPriority);
    const blocked = all.filter((i) => i.data.state?.name === 'Blocked');
    const unstarted = all
        .filter((i) => i.data.state?.type === 'unstarted')
        .sort(byPriority)
        .slice(0, 5);
    ...
    const lines = [
        ...
        ...inProgress.map((i) => `• *${i.data.title}* — ${i.data.assignee?.name ?? 'Unassigned'}`),
        '',
        `*🚧 Blocked (${blocked.length})*`,
        ...blocked.map((i) => `• *${i.data.title}* — ${i.data.assignee?.name ?? 'Unassigned'}`),
        '',
