...
export const weeklyErrorReport = schedules.task({
    ...
    run: async () => {
        // Query Corsair's synced Sentry database (fields live on .data)
        const unresolved = await corsair.sentry.db.issues.search({
            data: { status: 'unresolved' },
        });
        // Sort newest-first and take the top 10 in JS
        const issues = unresolved
            .sort((a, b) => Date.parse(b.data.lastSeen) - Date.parse(a.data.lastSeen))
            .slice(0, 10);
        ...
        const lines = [
            ...
            ...issues.map(
                (i, idx) =>
                    `${idx + 1}. *${i.data.title}* — ${i.data.count} events, last seen ${new Date(i.data.lastSeen).toLocaleDateString()}`,
            ),
            '',
...
<Info>
Register scheduled tasks in your Trigger.dev worker entry point. They're automatically deployed and managed, so you need no separate cron infrastructure.
</Info>
