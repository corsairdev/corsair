...
Webhooks let external services push events to you the moment something happens, like a GitHub star, a Slack message, or a PR merged. You expose one endpoint. Corsair verifies the signature, identifies the plugin, and calls your handler.
...
Free ngrok accounts get a random URL on every restart, so you'd have to re-register your webhook every time. Claim a **free static domain** at [dashboard.ngrok.com/domains](https://dashboard.ngrok.com/domains) and it'll never change.
</Info>
...
Pass the secret to the plugin in `createCorsair`. Corsair uses it to verify incoming webhook signatures.
...
1. Go to [api.slack.com/apps](https://api.slack.com/apps), then your app
...
Pass the signing secret to the plugin in `createCorsair`. Corsair uses it to verify incoming webhook signatures.
