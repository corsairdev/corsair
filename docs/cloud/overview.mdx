...
<Note>
`tenants` gives you each tenant's id and connected plugins, and
`connectionStatus` gives the per-plugin auth state. Both redact secrets: you
see *that* a tenant connected Notion, never their token. Reading a tenant's
stored *data* rows directly, or pointing the runtime at your own database,
