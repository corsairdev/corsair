...
import CloudProxyNext from '/snippets/connect-next.mdx';
...
<Warning>
The API key is a server secret — it's sent as a bearer token. Keep it on your
...
# slug (the segment between ck_cloud_ and the last _): https://api.corsair.cloud/<slug>/api/corsair
...
<Note>
This gives you tenant ids and connection state. Reading a tenant's stored *data*
directly from the managed store — or pointing the runtime at your own database —
comes with bring-your-own-DB (coming soon).
</Note>
...
- **From a server or agent.** Use the client directly — it holds the key and
  calls the runtime. This is the path in every example above.
...
`corsairCloud` takes no plugin list — the plugins live on the runtime — so
...
you allowed, and writes a local `corsair-env.d.ts` that TypeScript picks up — no
plugin import to maintain. Coding agents can skip it and list operations with
