...
async function safely(operation: () => Promise<unknown>, what: string) {
	try {
		...
		console.warn(`[BIGML] ${what}:`, error);
	}
}

/**
 * Every BigML resource keys on its own `resource` field - a compound
 * `{type}/{hex24}` string confirmed live to be globally unique across the
 * account, not scoped to a parent the way some other providers' ids are.
 * No composite key is needed anywhere in this plugin.
 */
...
export async function cacheEntity<Schema extends z.ZodType>(
	...
	if (!parsed.success) {
		console.warn(
			`[BIGML] skipped caching a ${options.label} that does not match its schema:`,
			parsed.error.issues,
		);
		return;
	}
...
/** Drops a record from the local mirror after BigML has deleted it. */
