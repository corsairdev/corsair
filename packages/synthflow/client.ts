...
export class SynthflowAPIError extends Error {
	constructor(
		message: string,
		...
		super(message);
		this.name = 'SynthflowAPIError';
	}
}

const SYNTHFLOW_API_BASE = 'https://api.synthflow.ai/v2';

export type SynthflowRequestOptions = {
	method?: 'GET' | 'POST' | 'PUT' | 'DELETE' | 'PATCH';
	body?: Record<string, unknown> | unknown;
	query?: Record<string, string | number | boolean | undefined>;
};

export async function makeSynthflowRequest<T>(
	endpoint: string,
	apiKey: string,
	options: SynthflowRequestOptions = {},
): Promise<T> {
	...
	try {
		...
		if (error instanceof Error) {
			throw new SynthflowAPIError(error.message);
		}
		throw new SynthflowAPIError('Unknown error');
	}
}
