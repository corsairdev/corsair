...
export const add: YoutubeEndpoints['playlistItemsAdd'] = async (ctx, input) => {
	const response = await makeYoutubeRequest<YoutubeEndpointOutputs['playlistItemsAdd']>(
		'/playlistItems',
		ctx.key,
		{
			method: 'POST',
	...
	await logEventFromContext(ctx, 'youtube.playlistItems.add', { videoId: input.videoId, playlistId: input.playlistId }, 'completed');
	return response;
};

export const list: YoutubeEndpoints['playlistItemsList'] = async (ctx, input) => {
	const response = await makeYoutubeRequest<YoutubeEndpointOutputs['playlistItemsList']>(
		'/playlistItems',
		ctx.key,
		{
			method: 'GET',
	...
	await logEventFromContext(ctx, 'youtube.playlistItems.list', { playlistId: input.playlistId }, 'completed');
	return response;
};

export const update: YoutubeEndpoints['playlistItemsUpdate'] = async (ctx, input) => {
	const response = await makeYoutubeRequest<YoutubeEndpointOutputs['playlistItemsUpdate']>(
		'/playlistItems',
		ctx.key,
		{
			method: 'PUT',
	...
	await logEventFromContext(ctx, 'youtube.playlistItems.update', { id: input.id }, 'completed');
	return response;
};

export const del: YoutubeEndpoints['playlistItemsDelete'] = async (ctx, input) => {
