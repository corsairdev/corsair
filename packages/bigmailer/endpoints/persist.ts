...
async function safely(operation: () => Promise<unknown>, what: string) {
	try {
		...
		console.warn(`[BIGMAILER] ${what}:`, error);
	}
}
...
/**
 * Almost every BigMailer entity is scoped to a brand and keys by an `id`
 * that is only unique *within* that brand - callers must pass a composite
 * entityId (`brand:id`) for those. Kept overridable per call site rather
 * than assumed.
 */
...
export async function cacheEntity<Schema extends z.ZodType>(
	...
	if (!parsed.success) {
		console.warn(
			`[BIGMAILER] skipped caching a ${options.label} that does not match its schema:`,
			parsed.error.issues,
		);
		return;
	}
...
/**
 * Drops a record from the local mirror after BigMailer has deleted it.
 *
...
export async function evictEntity(
	...
	try {
		...
		console.error(
			`[BIGMAILER] required eviction of ${label} ${entityId} failed - the local mirror still holds it:`,
			error,
		);
		throw error;
	}
