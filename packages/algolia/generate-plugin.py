#!/usr/bin/env python3
"""Generate the Algolia Corsair plugin from Composio toolkit MD and manual routes."""
...
COMPOSIO_MD = Path("/tmp/algolia_composio.md")

PLUGIN_ID = "algolia"
PASCAL = "Algolia"
CAMEL = "algolia"
PACKAGE_NAME = "@corsair-dev/algolia"
PREFIX = "ALGOLIA_"
...
def derive_group(host_type: str, path: str) -> str:
    if host_type == "insights":
        return "insights"
    if host_type == "querySuggestions":
        return "querySuggestions"
    if host_type == "usage":
        return "usage"
    if host_type == "personalization":
        return "personalization"
    if host_type == "analytics":
        if "abtests" in path:
            return "abTests"
        return "analytics"
    if host_type == "ingestion":
        if "/sources" in path:
            return "sources"
        if "/destinations" in path:
            return "destinations"
        if "/authentications" in path:
    ...
    if "/dictionaries" in path:
        return "dictionaries"
    if path.startswith("/1/logs"):
        return "logs"
    if "/strategies/personalization" in path or "/profiles/personalization" in path:
        return "personalization"
    if path.startswith("/1/task/"):
        return "tasks"
    if (
        path.endswith("/query")
        or "/facets/" in path
        or "/indexes/*/queries" in path
        or path.endswith("/objects")
    ):
        return "search"
    if "/indexes/" in path or path == "/1/indexes":
        return "indices"
    return "search"
...
def build_routes(operations: list[Operation]) -> list[Route]:
    ...
    for op in operations:
        ...
        method, path, host_type = MANUAL_ROUTES[op.slug]
        group = derive_group(host_type, path)
...
def generate_types(routes: list[Route]) -> str:
    lines = [
        "import { z } from 'zod';",
        "",
        f"// Algolia response payloads vary across {len(routes)} endpoints; per-route schemas are not yet mapped from API docs.",
    ...
    for route in routes:
        ...
        if "region" not in seen:
            fields.append("\tregion: z.string().optional(),")
            seen.add("region")
        if "baseUrl" not in seen:
            fields.append("\tbaseUrl: z.string().optional(),")
            seen.add("baseUrl")
        schema = "z.object({\n" + "\n".join(fields) + "\n})"
        lines.extend(
            [
...
def generate_factory() -> str:
    return textwrap.dedent(
        ...
        const BODY_CONTROL_KEYS = new Set(['body', 'query', 'headers', 'region', 'baseUrl']);
        ...
        export async function resolveBaseUrl(
        	ctx: {PASCAL}Context,
        	input: {PASCAL}EndpointInput,
        	route: Pick<{PASCAL}Route, 'hostType'>,
        ): Promise<string> {{
        ...
        export async function request{PASCAL}Operation(
        	...
        	return make{PASCAL}Request(resolvePath(route.path, input, route), applicationId, apiKey, {{
        		method: route.method,
...
def generate_client() -> str:
    return textwrap.dedent(
        ...
        export async function make{PASCAL}Request<T>(
        	...
        	if (!baseUrl) {{
        		throw new {PASCAL}APIError('[{PLUGIN_ID}] baseUrl is required');
        	}}
    ...
    return textwrap.dedent(
        ...
        export const errorHandlers = {{
        	...
        	AUTH_ERROR: {{
        		...
        		handler: async () => {{
        			console.error(
        				'[{PASCAL.upper()}] Authentication failed — check your Algolia Application ID and API key.',
        			);
        			return {{ maxRetries: 0 }};
        		}},
        	}},
...
def generate_schema_database() -> str:
    return textwrap.dedent(
        ...
        export const {PASCAL}Index = z.object({{
        ...
        export const {PASCAL}Record = z.object({{
        ...
        export type {PASCAL}Index = z.infer<typeof {PASCAL}Index>;
        export type {PASCAL}Record = z.infer<typeof {PASCAL}Record>;
...
def generate_schema_index() -> str:
    return textwrap.dedent(
        f"""
        import {{ {PASCAL}Index, {PASCAL}Record, {PASCAL}Task }} from './database';

        export const {PASCAL}Schema = {{
        	version: '1.0.0',
        	entities: {{
        		indices: {PASCAL}Index,
...
def generate_index() -> str:
    return textwrap.dedent(
        ...
        export type {PASCAL}Context = CorsairPluginContext<
        	...
        	undefined,
        	typeof {CAMEL}AuthConfig
        >;

        export type {PASCAL}KeyBuilderContext = KeyBuilderContext<
        	{PASCAL}PluginOptions,
        	typeof {CAMEL}AuthConfig
        >;
        	...
        	return {{
        		...
        		keyBuilder: async (ctx: {PASCAL}KeyBuilderContext, source) => {{
        			...
        			if (source === 'endpoint' && ctx.authType === 'api_key') {{
        				...
        				if (!apiKey) {{
        					console.error(
        						'[{PASCAL.upper()}] API key missing — connect Algolia or pass key in plugin options.',
        					);
        				...
        				if (!options.applicationId) {{
        					...
        					if (!applicationId) {{
        						console.error(
        							'[{PASCAL.upper()}] Application ID missing — connect Algolia or pass applicationId in plugin options.',
        						);
        						throw new AuthMissingError('{PLUGIN_ID}', 'api_key');
        					}}
        				}}
        			...
        			console.error(
        				'[{PASCAL.upper()}] Authentication required for Algolia API requests.',
        			);
        			throw new AuthMissingError('{PLUGIN_ID}', 'api_key');
        		}},
        ...
        json.dumps(
            {
                "name": PACKAGE_NAME,
                "version": "0.1.0",
                "description": "Algolia plugin for Corsair",
                "type": "module",
                "main": "./dist/index.js",
                ...
                "keywords": ["corsair", "algolia", "search", "plugin"],
                "author": "",
                "license": "Apache-2.0",
                "files": ["dist"],
            },
...
def main() -> None:
    ...
    write(ROOT / "client.ts", generate_client())
    write(ROOT / "error-handlers.ts", generate_error_handlers())
    write(ROOT / "schema/database.ts", generate_schema_database())
    write(ROOT / "schema/index.ts", generate_schema_index())
    write(ROOT / "endpoints/types.ts", generate_types(routes))
    write(ROOT / "endpoints/routes.ts", generate_routes_ts(routes))
    write(ROOT / "endpoints/factory.ts", generate_factory())
    write(ROOT / "endpoints/index.ts", generate_endpoints_index(groups))
    for group, group_routes in sorted(groups.items()):
        write(ROOT / f"endpoints/{group}.ts", generate_group_file(group, group_routes))
    write(ROOT / "index.ts", generate_index())
    write(ROOT / "package.json", generate_package_json())
    write(
        ROOT / "tsconfig.json",
    ...
    write(ROOT / "api.test.ts", generate_api_test(len(routes)))
