...
export const Leaves = {
	list: async (
		...
		return await makeBreatheHrRequest<LeavesListResponse>('/leaves', apiKey, {
			query,
		});
	},
	...
	get: async (
		ctx: BreatheHrContext,
		input: LeavesGetInput,
	): Promise<LeavesGetResponse> => {
		const apiKey = ctx.key;
		return await makeBreatheHrRequest<LeavesGetResponse>(
			`/leaves/${input.id}`,
			apiKey,
		);
	},
