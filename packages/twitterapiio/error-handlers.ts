...
export const errorHandlers = {
	...
	AUTH_ERROR: {
		...
		handler: async (error, context) => {
			console.log(
				`[TWITTERAPIIO:${context.operation}] Authentication failed - check your API key`,
			);
	...
	PERMISSION_ERROR: {
		...
		handler: async (error, context) => {
			console.warn(
				`[TWITTERAPIIO:${context.operation}] Permission denied: ${error.message}`,
			);
	...
	DEFAULT: {
		...
		handler: async (error, context) => {
			console.error(
				`[TWITTERAPIIO:${context.operation}] Unhandled error: ${error.message}`,
			);
