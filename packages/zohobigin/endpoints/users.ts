...
export const getUsers: ZohoBiginEndpoints['getUsers'] = async (ctx, input) => {
	...
	const response = await makeZohoBiginRequest<
		ZohoBiginEndpointOutputs['getUsers']
	>('users', ctx.key, {
		method: 'GET',
...
export const getUser: ZohoBiginEndpoints['getUser'] = async (ctx, input) => {
	const { id } = input;
	const response = await makeZohoBiginRequest<
		ZohoBiginEndpointOutputs['getUser']
	>(`users/${id}`, ctx.key, { method: 'GET' });

	await logEventFromContext(
		ctx,
		'zohobigin.users.getUser',
		{ id },
		'completed',
	);
	return response;
...
export const getRoles: ZohoBiginEndpoints['getRoles'] = async (ctx) => {
	const response = await makeZohoBiginRequest<
		ZohoBiginEndpointOutputs['getRoles']
	>('settings/roles', ctx.key, { method: 'GET' });
...
export const getProfiles: ZohoBiginEndpoints['getProfiles'] = async (ctx) => {
	const response = await makeZohoBiginRequest<
		ZohoBiginEndpointOutputs['getProfiles']
	>('settings/profiles', ctx.key, { method: 'GET' });

	await logEventFromContext(
		ctx,
		'zohobigin.users.getProfiles',
		{},
		'completed',
	);
	return response;
...
export const updateUser: ZohoBiginEndpoints['updateUser'] = async (
	...
	const { id, data } = input;
	const response = await makeZohoBiginRequest<
		ZohoBiginEndpointOutputs['updateUser']
	>(`users/${id}`, ctx.key, {
		method: 'PUT',
		body: { users: [data] },
	});
...
export const updateUsers: ZohoBiginEndpoints['updateUsers'] = async (
	...
	const { users } = input;
	const response = await makeZohoBiginRequest<
		ZohoBiginEndpointOutputs['updateUsers']
	>('users', ctx.key, {
		method: 'PUT',
		body: { users },
	});
