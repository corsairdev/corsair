 ...
 * the event log's retention. Better Stack inputs carry incident summaries,
 * comment bodies, status page announcements, requester e-mail addresses and
 * webhook credentials, none of which belong in an audit trail.
 *
...
/** Counts rows without recording any of their content. */
export function auditCount(count: number): Record<string, unknown> {
	return { count };
}
