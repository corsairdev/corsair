 ...
 * Works with CodacyAPIError (which copies status from ApiError)
 * and any error that exposes a numeric `status` property.
 */
function getStatus(error: Error): number | undefined {
	return (error as Partial<CodacyAPIError>).status;
}
...
function getRetryAfter(error: Error): number | undefined {
	return (error as Partial<CodacyAPIError>).retryAfter;
}

/**
 * Error handlers for the Codacy plugin.
 *
 * Codacy returns standard HTTP status codes:
...
export const errorHandlers = {
	...
	AUTH_ERROR: {
		...
		handler: async (error: Error) => {
			console.warn(
				'[CODACY] Authentication failed — check that the API token is ' +
					'valid and active for the account in use.',
			);
			return { maxRetries: 0 };
		},
	},
	FORBIDDEN_ERROR: {
		...
		handler: async (error: Error) => {
			console.warn(
				'[CODACY] Request forbidden — the user behind the API token ' +
	...
	NOT_FOUND_ERROR: {
		...
		handler: async (error: Error) => {
			console.warn(
				'[CODACY] Resource not found — the requested record does not ' +
	...
	VALIDATION_ERROR: {
		...
		handler: async (error: Error) => {
			console.warn(
				'[CODACY] Request rejected — a field is missing, malformed, ' +
	...
	DEFAULT: {
		...
		handler: async (error: Error) => {
			console.error(`[CODACY] Unhandled error: ${error.message}`);
			return { maxRetries: 0 };
		},
	},
