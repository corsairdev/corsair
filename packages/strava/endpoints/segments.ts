...
export const explore: StravaEndpoints['segmentsExplore'] = async (ctx, input) => {
	const result = await makeStravaRequest<StravaEndpointOutputs['segmentsExplore']>(
		'segments/explore',
		ctx.key,
		{
			method: 'GET',
...
export const getStreams: StravaEndpoints['segmentsGetStreams'] = async (ctx, input) => {
	const result = await makeStravaRequest<StravaEndpointOutputs['segmentsGetStreams']>(
		`segments/${input.id}/streams`,
		ctx.key,
		{
			method: 'GET',
			query: {
	...
	if (Array.isArray(result) && ctx.db.segments) {
		try {
			for (const segment of result) {
				if (segment.id) {
					await ctx.db.segments.upsertByEntityId(String(segment.id), { ...segment });
				}
			}
		} catch (error) {
...
export const star: StravaEndpoints['segmentsStar'] = async (ctx, input) => {
	...
	await logEventFromContext(ctx, 'strava.segments.star', { ...input }, 'completed');
	return result;
};

export const effortsGet: StravaEndpoints['segmentEffortsGet'] = async (ctx, input) => {
	const result = await makeStravaRequest<StravaEndpointOutputs['segmentEffortsGet']>(
		`segment_efforts/${input.id}`,
		ctx.key,
		{
			method: 'GET',
		},
	...
	if (result.id && ctx.db.segmentEfforts) {
		try {
			await ctx.db.segmentEfforts.upsertByEntityId(String(result.id), { ...result });
		} catch (error) {
...
export const effortsGetStreams: StravaEndpoints['segmentEffortsGetStreams'] = async (
	ctx,
	input,
) => {
	const result = await makeStravaRequest<StravaEndpointOutputs['segmentEffortsGetStreams']>(
		`segment_efforts/${input.id}/streams`,
		ctx.key,
		{
			method: 'GET',
			query: {
