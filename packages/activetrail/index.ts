...
export type ActiveTrailKeyBuilderContext =
	KeyBuilderContext<ActiveTrailPluginOptions>;

export type ActiveTrailBoundEndpoints = BindEndpoints<
	typeof activeTrailEndpointsNested
>;
...
export type BaseActiveTrailPlugin<T extends ActiveTrailPluginOptions> =
	CorsairPlugin<
		'activetrail',
		typeof ActiveTrailSchema,
		typeof activeTrailEndpointsNested,
...
export type InternalActiveTrailPlugin =
	BaseActiveTrailPlugin<ActiveTrailPluginOptions>;

export type ExternalActiveTrailPlugin<T extends ActiveTrailPluginOptions> =
	BaseActiveTrailPlugin<T>;

export function activetrail<const T extends ActiveTrailPluginOptions>(
	incomingOptions: ActiveTrailPluginOptions &
		T = {} as ActiveTrailPluginOptions & T,
		...
		keyBuilder: async (ctx: ActiveTrailKeyBuilderContext, source) => {
			...
			if (source === 'endpoint' && ctx.authType === 'api_key') {
				...
				if (!res) {
					...
					throw new AuthMissingError('activetrail', 'api_key');
				}
				return res;
			}
