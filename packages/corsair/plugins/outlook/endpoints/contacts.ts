...
const toContactRecord = (contact: OutlookEndpointOutputs['contactsCreate']) => ({
	...
	createdAt: contact.createdDateTime ? new Date(contact.createdDateTime) : undefined,
});

// ── Endpoints ─────────────────────────────────────────────────────────────────

export const createContact: OutlookEndpoints['contactsCreate'] = async (ctx, input) => {
	const result = await makeOutlookRequest<OutlookEndpointOutputs['contactsCreate']>(
		`${userPath(input.user_id)}/contacts`,
		ctx.key,
		{
			method: 'POST',
			body: {
				...
				...(input.emailAddresses?.length && { emailAddresses: input.emailAddresses }),
				...(input.mobilePhone && { mobilePhone: input.mobilePhone }),
				...(input.homePhone && { homePhones: [input.homePhone] }),
				...(input.businessPhones?.length && { businessPhones: input.businessPhones }),
	...
	if (result.id && ctx.db.contacts) {
		try {
			await ctx.db.contacts.upsertByEntityId(result.id, toContactRecord(result));
		} catch (error) {
...
export const listContacts: OutlookEndpoints['contactsList'] = async (ctx, input) => {
	const base = userPath(input.user_id);
	const folderPath = input.contact_folder_id
		? `/contactFolders/${input.contact_folder_id}`
		: '';

	const result = await makeOutlookRequest<OutlookEndpointOutputs['contactsList']>(
		`${base}${folderPath}/contacts`,
		ctx.key,
		{
			query: {
	...
	if (result.value?.length && ctx.db.contacts) {
		try {
			for (const contact of result.value) {
				if (contact.id) {
					await ctx.db.contacts.upsertByEntityId(contact.id, toContactRecord(contact));
				}
			}
		} catch (error) {
	...
	await logEventFromContext(ctx, 'outlook.contacts.list', { ...input }, 'completed');
	return result;
};

export const updateContact: OutlookEndpoints['contactsUpdate'] = async (ctx, input) => {
	const result = await makeOutlookRequest<OutlookEndpointOutputs['contactsUpdate']>(
	...
	await logEventFromContext(ctx, 'outlook.contacts.update', { contact_id: input.contact_id }, 'completed');
	return result;
};

export const deleteContact: OutlookEndpoints['contactsDelete'] = async (ctx, input) => {
	...
	await logEventFromContext(ctx, 'outlook.contacts.delete', { contact_id: input.contact_id }, 'completed');
	return { success: true };
};
