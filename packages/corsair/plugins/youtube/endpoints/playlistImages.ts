...
export const list: YoutubeEndpoints['playlistImagesList'] = async (ctx, input) => {
	const response = await makeYoutubeRequest<YoutubeEndpointOutputs['playlistImagesList']>(
		'/playlistImages',
		ctx.key,
		{
			method: 'GET',
	...
	await logEventFromContext(ctx, 'youtube.playlistImages.list', { parent: input.parent }, 'completed');
	return response;
};
