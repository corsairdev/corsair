...
export const getById: TwitterApiIOEndpoints['communitiesGetById'] = async (
	ctx,
	input,
) => {
	...
	if (response.data && ctx.db.communities) {
		try {
			await ctx.db.communities.upsertByEntityId(
				response.data.id,
				response.data,
			);
		} catch (error) {
			console.warn(
				'[twitterapiio] Failed to save community to database:',
				error,
			);
		}
	}
...
export const getMembers: TwitterApiIOEndpoints['communitiesGetMembers'] =
	async (ctx, input) => {
		const response = await makeTwitterApiIORequest<
...
export const getModerators: TwitterApiIOEndpoints['communitiesGetModerators'] =
	async (ctx, input) => {
		const response = await makeTwitterApiIORequest<
...
export const getTweets: TwitterApiIOEndpoints['communitiesGetTweets'] = async (
	ctx,
	input,
) => {
	const response = await makeTwitterApiIORequest<
...
export const searchTweets: TwitterApiIOEndpoints['communitiesSearchTweets'] =
	async (ctx, input) => {
		const response = await makeTwitterApiIORequest<
...
export const create: TwitterApiIOEndpoints['communitiesCreate'] = async (
	ctx,
	input,
) => {
	const response = await makeTwitterApiIORequest<
...
export const deleteCommunity: TwitterApiIOEndpoints['communitiesDelete'] =
	async (ctx, input) => {
		const response = await makeTwitterApiIORequest<
...
export const join: TwitterApiIOEndpoints['communitiesJoin'] = async (
	ctx,
	input,
) => {
	const response = await makeTwitterApiIORequest<
...
export const leave: TwitterApiIOEndpoints['communitiesLeave'] = async (
	ctx,
	input,
) => {
	const response = await makeTwitterApiIORequest<
