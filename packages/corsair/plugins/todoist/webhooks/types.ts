 ...
 * - Each event type should extend TodoistWebhookPayload
 * - Add all event-specific fields in the data object
 * 
 * Example:
 * export interface UserCreatedEvent extends TodoistWebhookPayload {
 *   type: 'user.created';
 *   data: {
 *     user_id: string;
 *     email: string;
 ...
 *   const signature = request.headers['x-todoist-signature'];
 *   if (!signature) {
