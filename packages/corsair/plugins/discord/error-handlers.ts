...
export const errorHandlers = {
	...
	AUTH_ERROR: {
		...
		handler: async (error, context) => {
			console.log(
				`[DISCORD:${context.operation}] Authentication failed - check your API key`,
			);
	...
	PERMISSION_ERROR: {
		...
		handler: async (error, context) => {
			console.warn(
				`[DISCORD:${context.operation}] Permission denied: ${error.message}`,
			);
	...
	DEFAULT: {
		...
		handler: async (error, context) => {
			console.error(
				`[DISCORD:${context.operation}] Unhandled error: ${error.message}`,
			);
