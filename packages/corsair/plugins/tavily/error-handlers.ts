...
export const errorHandlers = {
	...
	AUTH_ERROR: {
		...
		handler: async (error, context) => {
			console.log(
				`[TAVILY:${context.operation}] Authentication failed - check your API key`,
			);
	...
	PERMISSION_ERROR: {
		...
		handler: async (error, context) => {
			console.warn(
				`[TAVILY:${context.operation}] Permission denied: ${error.message}`,
			);
	...
	DEFAULT: {
		...
		handler: async (error, context) => {
			console.error(
				`[TAVILY:${context.operation}] Unhandled error: ${error.message}`,
			);
