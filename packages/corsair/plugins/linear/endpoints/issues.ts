...
export const list: LinearEndpoints['issuesList'] = async (ctx, input) => {
	...
	if (result.nodes && ctx.issues) {
		try {
			for (const issue of result.nodes) {
				await ctx.issues.upsert(issue.id, {
					id: issue.id,
					title: issue.title,
...
export const get: LinearEndpoints['issuesGet'] = async (ctx, input) => {
	...
	if (result && ctx.issues) {
		try {
			await ctx.issues.upsert(result.id, {
				id: result.id,
				title: result.title,
...
export const update: LinearEndpoints['issuesUpdate'] = async (ctx, input) => {
	...
	if (result && ctx.issues) {
		try {
			const existing = await ctx.issues.findByResourceId(result.id);
			await ctx.issues.upsert(result.id, {
				id: result.id,
				title: result.title,
				description: result.description,
...
export const deleteIssue: LinearEndpoints['issuesDelete'] = async (
	...
	if (result && ctx.issues) {
		try {
			await ctx.issues.deleteByResourceId(input.id);
		} catch (error) {
