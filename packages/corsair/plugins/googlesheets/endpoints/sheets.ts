...
import type { ValueRange, BatchUpdateValuesRequest } from '../types';
...
export const appendRow: GoogleSheetsEndpoints['sheetsAppendRow'] = async (
	...
	const result = await makeSheetsRequest<GoogleSheetsEndpointOutputs['sheetsAppendRow']>(
		`/spreadsheets/${input.spreadsheetId}/values/${range}:append`,
		ctx.key,
		{
			method: 'POST',
			query: {
...
export const appendOrUpdateRow: GoogleSheetsEndpoints['sheetsAppendOrUpdateRow'] =
	async (ctx, input) => {
		...
		const values = input.values ? [input.values] : [];
		const updateRange = rowIndex >= 0
			? `${sheetName}!${rowIndex + 1}:${rowIndex + 1}`
			: `${sheetName}!A:Z`;

		if (rowIndex >= 0) {
			const result = await makeSheetsRequest<GoogleSheetsEndpointOutputs['sheetsAppendOrUpdateRow']>(
...
export const getRows: GoogleSheetsEndpoints['sheetsGetRows'] = async (
	...
	const result = await makeSheetsRequest<GoogleSheetsEndpointOutputs['sheetsGetRows']>(
...
export const updateRow: GoogleSheetsEndpoints['sheetsUpdateRow'] = async (
	...
	const range = input.range || `${input.sheetName || 'Sheet1'}!${input.rowIndex || 1}:${input.rowIndex || 1}`;
	const values = input.values ? [input.values] : [];

	const result = await makeSheetsRequest<GoogleSheetsEndpointOutputs['sheetsUpdateRow']>(
		`/spreadsheets/${input.spreadsheetId}/values/${range}`,
		ctx.key,
		{
			method: 'PUT',
...
export const clearSheet: GoogleSheetsEndpoints['sheetsClearSheet'] = async (
	...
	const result = await makeSheetsRequest<GoogleSheetsEndpointOutputs['sheetsClearSheet']>(
		`/spreadsheets/${input.spreadsheetId}/values/${range}:clear`,
		ctx.key,
		{
			method: 'POST',
		},
...
export const createSheet: GoogleSheetsEndpoints['sheetsCreateSheet'] = async (
	...
	const result = await makeSheetsRequest<GoogleSheetsEndpointOutputs['sheetsCreateSheet']>(
...
export const deleteSheet: GoogleSheetsEndpoints['sheetsDeleteSheet'] = async (
	...
	const result = await makeSheetsRequest<GoogleSheetsEndpointOutputs['sheetsDeleteSheet']>(
		`/spreadsheets/${input.spreadsheetId}:batchUpdate`,
		ctx.key,
		{
			method: 'POST',
...
export const deleteRowsOrColumns: GoogleSheetsEndpoints['sheetsDeleteRowsOrColumns'] =
	async (ctx, input) => {
		...
		const result = await makeSheetsRequest<GoogleSheetsEndpointOutputs['sheetsDeleteRowsOrColumns']>(
			`/spreadsheets/${input.spreadsheetId}:batchUpdate`,
			ctx.key,
			{
				method: 'POST',
