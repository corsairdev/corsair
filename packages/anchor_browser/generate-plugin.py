#!/usr/bin/env python3
"""Generate the Anchor Browser Corsair plugin from Composio toolkit MD and manual routes."""
...
COMPOSIO_MD = Path("/tmp/anchor_browser_composio.md")


PLUGIN_ID = "anchor_browser"
PASCAL = "AnchorBrowser"
CAMEL = "anchorBrowser"
PACKAGE_NAME = "@corsair-dev/anchor_browser"
BASE_URL = "https://api.anchorbrowser.io/v1"
PREFIX = "ANCHOR_BROWSER_"
...
def derive_group(path: str) -> str:
    if path.startswith("/sessions"):
        ...
        if path.endswith("/screenshot"):
            return "screenshots"
        if "/uploads" in path or "/agent/files" in path:
            return "uploads"
        return "sessions"
...
def generate_types(routes: list[Route]) -> str:
    lines = [
        "import { z } from 'zod';",
        "",
        f"// Anchor Browser response payloads vary across {len(routes)} endpoints; per-route schemas are not yet mapped from API docs.",
    ...
    return textwrap.dedent(
        ...
        export const errorHandlers = {{
        	...
        	AUTH_ERROR: {{
        		...
        		handler: async () => {{
        			console.error(
        				'[{PASCAL.upper()}] Authentication failed — check your Anchor Browser API key.',
        			);
        			return {{ maxRetries: 0 }};
        		}},
        	}},
...
def generate_schema_database() -> str:
    return textwrap.dedent(
        ...
        export const {PASCAL}Session = z.object({{
        ...
        export const {PASCAL}Profile = z.object({{
        	name: z.string().optional(),
        	description: z.string().optional(),
        	status: z.string().optional(),
        }}).catchall(z.unknown());

        export type {PASCAL}Session = z.infer<typeof {PASCAL}Session>;
        export type {PASCAL}Task = z.infer<typeof {PASCAL}Task>;
        export type {PASCAL}Profile = z.infer<typeof {PASCAL}Profile>;
        """
    ).strip() + "\n"
...
def generate_schema_index() -> str:
    return textwrap.dedent(
        f"""
        import {{ {PASCAL}Profile, {PASCAL}Session, {PASCAL}Task }} from './database';

        export const {PASCAL}Schema = {{
        	version: '1.0.0',
        	entities: {{
        		sessions: {PASCAL}Session,
        		tasks: {PASCAL}Task,
        		profiles: {PASCAL}Profile,
        	}},
        }} as const;
        """
    ).strip() + "\n"
        	...
        	return {{
        		...
        		keyBuilder: async (ctx: {PASCAL}KeyBuilderContext, source) => {{
        			...
        			if (source === 'endpoint' && ctx.authType === 'api_key') {{
        				...
        				if (!res) {{
        					console.error(
        						'[{PASCAL.upper()}] API key missing — connect Anchor Browser or pass key in plugin options.',
        					);
        			...
        			console.error(
        				'[{PASCAL.upper()}] Authentication required for Anchor Browser API requests.',
        			);
        			throw new AuthMissingError('{PLUGIN_ID}', 'api_key');
        		}},
        ...
        json.dumps(
            {
                ...
                "keywords": ["corsair", "anchor_browser", "browser", "automation", "plugin"],
                "author": "",
                "license": "Apache-2.0",
                "files": ["dist"],
            },
    ...
    return textwrap.dedent(
        ...
        describe('{PASCAL} request client', () => {{
        	...
        	it('sends anchor-api-key header and JSON bodies', async () => {{
        		await make{PASCAL}Request('/sessions', 'test-api-key', {{
        			method: 'GET',
        		}});
