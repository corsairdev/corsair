...
import { WebvizioProject, WebvizioSchema, WebvizioWebhook } from './schema';
...
const TEST_API_KEY =
	process.env.WEBVIZIO_API_KEY ||
	'5099|lBO3kONhk7TpdKEB2QDa63ITQUbxnD0Yd0OSeRx7';
...
describeIfApiKey('Webvizio live API integration tests', () => {
	it('fetches real projects list with valid token', async () => {
		const { request: actualRequest } = jest.requireActual('corsair/http');
		mockRequest.mockImplementation(actualRequest);
