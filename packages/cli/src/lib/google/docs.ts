...
// Google Docs has no native push, so triggers ride on a Drive changes watch
// channel. The plugin narrows the change stream to Docs by mime type.
export async function setupDocsWatch(
	accessToken: string,
	webhookUrl: string,
	...
	if (!watchRes.ok) {
		...
		watchSpin.stop('Failed.');
		p.log.error(`Google Docs watch failed: ${error}`);
		p.outro('');
		process.exit(1);
	}
