...
export async function enrollDevTunnel(opts: {
	...
	if (spawnSync('zrok', ['version'], { stdio: 'ignore' }).error) {
		console.log(
			'[corsair] zrok is not installed — install it first (e.g. `brew install zrok`), then re-run `corsair setup`.',
		);
		return;
	}
	...
	spawnSync('zrok', ['config', 'set', 'apiEndpoint', apiEndpoint], {
		stdio: 'ignore',
	});
	// Idempotent: re-enabling an already-enrolled machine is a harmless no-op.
	spawnSync('zrok', ['enable', accountToken], { stdio: 'ignore' });
