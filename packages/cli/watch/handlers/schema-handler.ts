...
export class Schema {
	...
	public async parse(isInitialLoad: boolean = true): Promise<void> {
		try {
							...
							if (expression.getText() === 'CorsairDB.table') {
								...
								if (args.length >= 2 && args[0] && args[1]) {
									...
									if (columnsArg.isKind(SyntaxKind.ObjectLiteralExpression)) {
										const columns =
											this.parseColumnsFromCorsairDBTable(columnsArg);
			...
			console.error(
				`Can't find the schema file. Does it exist at ${this.filePath}?`,
			);
			console.error(error);
		}
	}
		...
		for (const property of columnsObj.getProperties()) {
			if (property.isKind(SyntaxKind.PropertyAssignment)) {
				...
				if (columnExpression?.isKind(SyntaxKind.CallExpression)) {
					const column = this.parseColumnExpression(
						columnName,
						columnExpression,
					);
					if (column) {
		...
		if (corsairDBCall) {
			...
			if (memberExpression.isKind(SyntaxKind.PropertyAccessExpression)) {
				...
				if (expression.getText().includes('.references(')) {
					...
					const referencesMatch = fullText.match(
						/\.references\(\(\)\s*=>\s*(\w+)\.(\w+)\)/,
					);
					if (referencesMatch) {
						references = {
	...
	private getChangesSummary(
		...
		for (const beforeTable of before.tables) {
			...
			if (afterTable && this.hasTableChanged(beforeTable, afterTable)) {
				const tableChanges = this.getTableChangesSummary(
					beforeTable,
					afterTable,
				);
				changes.push(
					...tableChanges.map((change) => `${beforeTable.name}.${change}`),
				);
			}
		}
	...
	private hasTableChanged(
		...
		for (let i = 0; i < before.columns.length; i++) {
			...
			const afterColumn = after.columns.find(
				(c) => c.name === beforeColumn.name,
			);
			...
			if (beforeRefs && afterRefs) {
				if (
					beforeRefs.table !== afterRefs.table ||
					beforeRefs.column !== afterRefs.column
				) {
					return true;
				}
			}
		...
		for (const afterColumn of after.columns) {
			const beforeColumn = before.columns.find(
				(c) => c.name === afterColumn.name,
			);
			if (!beforeColumn) {
	 ...
	 */
	private getTableChangesSummary(
		before: TableDefinition,
		after: TableDefinition,
	): string[] {
		const changes: string[] = [];
		...
		for (const beforeColumn of before.columns) {
			...
			if (afterColumn) {
				if (beforeColumn.type !== afterColumn.type) {
					changes.push(
						`${beforeColumn.name} type changed from ${beforeColumn.type} to ${afterColumn.type}`,
					);
				}
				...
				if (!beforeRefs && afterRefs) {
					changes.push(
						`${beforeColumn.name} reference added -> ${afterRefs.table}.${afterRefs.column}`,
					);
					...
					if (
						...
						changes.push(
							`${beforeColumn.name} reference changed from ${beforeRefs.table}.${beforeRefs.column} to ${afterRefs.table}.${afterRefs.column}`,
						);
					}
				}
			}
