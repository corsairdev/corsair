...
export const deleteGroup: GitlabEndpoints['groupsDelete'] = async (
	ctx,
	input,
) => {
	const result = await makeAuthenticatedGitlabRequest<
		GitlabEndpointOutputs['groupsDelete']
	>(`/groups/${enc(input.group_id)}`, ctx, {
		method: 'DELETE',
		baseUrl: ctx.options.baseUrl,
	});
...
export const listProjects: GitlabEndpoints['groupsListProjects'] = async (
	ctx,
	input,
) => {
	const { group_id, ...query } = input;
	...
	>(`/groups/${enc(group_id)}/projects`, ctx, {
		method: 'GET',
		query,
		baseUrl: ctx.options.baseUrl,
	});
