...
function parseBody(body: unknown): unknown {
	...
	// Let invalid JSON throw — callers wrap in try/catch and warn so the failure is
	...
	return (request: RawWebhookRequest) => {
		try {
			...
			if (typeof request.body === 'string') {
				console.warn('[facebook] Failed to parse string body in challenge matcher:', error);
			}
			return false;
		}
	};
...
export function createFacebookMessagingMatch(
	...
	return (request: RawWebhookRequest) => {
		try {
			...
			if (typeof request.body === 'string') {
				console.warn(`[facebook] Failed to parse string body in ${eventType} matcher:`, error);
			}
			return false;
		}
	};
