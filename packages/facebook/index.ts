...
export function facebook<const T extends FacebookPluginOptions>(
	...
	return {
		...
		pluginWebhookMatcher: (request) => {
			// Event notifications (message, delivery) always carry Facebook's HMAC signature header.
			if ('x-hub-signature-256' in request.headers) {
				return true;
			}

			// Challenge verification arrives without a signature — match only when the body
			// carries the exact fields Facebook sends: hub.mode=subscribe + hub.challenge.
			try {
				...
				const hub = typeof b.hub === 'object' && b.hub !== null
					? (b.hub as Record<string, unknown>)
					: null;
				const mode = typeof b['hub.mode'] === 'string'
					? b['hub.mode']
					: hub && typeof hub.mode === 'string' ? hub.mode : undefined;
				const challenge = typeof b['hub.challenge'] === 'string'
					? b['hub.challenge']
					: hub && typeof hub.challenge === 'string' ? hub.challenge : undefined;
