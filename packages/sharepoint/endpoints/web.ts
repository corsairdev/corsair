...
export const getInfo: SharepointEndpoints['webGetInfo'] = async (ctx, input) => {
	const siteId = (await ctx.keys.get_site_id()) ?? ctx.options?.siteId ?? '';
	const result = await makeGraphRequest<SharepointEndpointOutputs['webGetInfo']>(
		`/sites/${siteId}`,
		ctx.key,
		{ method: 'GET' },
	);
	...
	await logEventFromContext(ctx, 'sharepoint.web.getInfo', { ...input }, 'completed');
	return result;
};

export const getSiteCollectionInfo: SharepointEndpoints['webGetSiteCollectionInfo'] = async (ctx, input) => {
	const siteId = (await ctx.keys.get_site_id()) ?? ctx.options?.siteId ?? '';
	const result = await makeGraphRequest<SharepointEndpointOutputs['webGetSiteCollectionInfo']>(
		`/sites/${siteId}`,
		ctx.key,
...
export const getSitePage: SharepointEndpoints['webGetSitePage'] = async (ctx, input) => {
	const siteId = (await ctx.keys.get_site_id()) ?? ctx.options?.siteId ?? '';
	const result = await makeGraphRequest<SharepointEndpointOutputs['webGetSitePage']>(
		`/sites/${siteId}/pages/${encodeURIComponent(input.page_server_relative_url)}`,
		ctx.key,
		{ method: 'GET' },
	);

	await logEventFromContext(ctx, 'sharepoint.web.getSitePage', { ...input }, 'completed');
	return result;
};

export const createSubsite: SharepointEndpoints['webCreateSubsite'] = async (ctx, input) => {
	// Graph API v1.0 does not support creating subsites; return current site info as fallback
	const siteId = (await ctx.keys.get_site_id()) ?? ctx.options?.siteId ?? '';
	const result = await makeGraphRequest<SharepointEndpointOutputs['webCreateSubsite']>(
		`/sites/${siteId}`,
		ctx.key,
		{ method: 'GET' },
	);
	await logEventFromContext(ctx, 'sharepoint.web.createSubsite', { ...input }, 'completed');
	return result;
};
...
export const getContextInfo: SharepointEndpoints['webGetContextInfo'] = async (ctx, input) => {
	// Graph API does not require form digest values; return a stub for backwards compatibility
	await logEventFromContext(ctx, 'sharepoint.web.getContextInfo', { ...input }, 'completed');
	return {
...
export const getDriveItemByPath: SharepointEndpoints['webGetDriveItemByPath'] = async (ctx, input) => {
	const result = await makeGraphRequest<SharepointEndpointOutputs['webGetDriveItemByPath']>(
		`/sites/${encodeURIComponent(input.site_id)}/drive/root:/${input.path}`,
		ctx.key,
		{ method: 'GET' },
	);

	await logEventFromContext(ctx, 'sharepoint.web.getDriveItemByPath', { ...input }, 'completed');
	return result;
};

export const logEvent: SharepointEndpoints['webLogEvent'] = async (ctx, input) => {
	// Graph API does not expose a custom app event logging endpoint
	await logEventFromContext(ctx, 'sharepoint.web.logEvent', { ...input }, 'completed');
	return { success: true };
};
