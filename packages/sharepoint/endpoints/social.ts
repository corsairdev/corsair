...
export const follow: SharepointEndpoints['socialFollow'] = async (ctx, input) => {
	// Graph API following is site-level via /me/followedSites
	// For general follow operations, return stub
	await logEventFromContext(ctx, 'sharepoint.social.follow', { ...input }, 'completed');
	return { value: undefined };
};

export const isFollowed: SharepointEndpoints['socialIsFollowed'] = async (ctx, input) => {
	// Graph API does not expose a generic isFollowed check
	await logEventFromContext(ctx, 'sharepoint.social.isFollowed', { ...input }, 'completed');
	return { value: false };
};

export const getFollowed: SharepointEndpoints['socialGetFollowed'] = async (ctx, input) => {
	// Use Graph /me/followedSites for site-type followed entities
	const result = await makeGraphRequest<{ value?: Array<{ id?: string; displayName?: string; webUrl?: string }> }>(
		'/me/followedSites',
		ctx.key,
		{ method: 'GET' },
	);
...
export const getFollowers: SharepointEndpoints['socialGetFollowers'] = async (ctx, input) => {
	// Graph API does not expose a followers concept for SharePoint
	await logEventFromContext(ctx, 'sharepoint.social.getFollowers', { ...input }, 'completed');
	return { value: [] };
};
