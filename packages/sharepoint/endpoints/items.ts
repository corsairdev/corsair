...
export const list: SharepointEndpoints['itemsList'] = async (ctx, input) => {
	...
	await logEventFromContext(ctx, 'sharepoint.items.list', { ...input }, 'completed');
	return result;
};

export const listByGuid: SharepointEndpoints['itemsListByGuid'] = async (ctx, input) => {
	const siteId = (await ctx.keys.get_site_id()) ?? ctx.options?.siteId ?? '';
	const query: Record<string, string | number | boolean | undefined> = {
		$expand: 'fields',
	};
	...
	const result = await makeGraphRequest<SharepointEndpointOutputs['itemsListByGuid']>(
	...
	await logEventFromContext(ctx, 'sharepoint.items.listByGuid', { ...input }, 'completed');
	return result;
};
...
export const get: SharepointEndpoints['itemsGet'] = async (ctx, input) => {
	...
	await logEventFromContext(ctx, 'sharepoint.items.get', { ...input }, 'completed');
	return result;
};
...
export const create: SharepointEndpoints['itemsCreate'] = async (ctx, input) => {
	...
	const result = await makeGraphRequest<SharepointEndpointOutputs['itemsCreate']>(
		`/sites/${siteId}/lists/${encodeURIComponent(input.list_title)}/items`,
		ctx.key,
		{ method: 'POST', body: { fields: input.fields as Record<string, unknown> } },
	);
	...
	await logEventFromContext(ctx, 'sharepoint.items.create', { ...input }, 'completed');
	return result;
};
...
export const createByGuid: SharepointEndpoints['itemsCreateByGuid'] = async (ctx, input) => {
	...
	await logEventFromContext(ctx, 'sharepoint.items.createByGuid', { ...input }, 'completed');
	return result;
};
...
export const createInFolder: SharepointEndpoints['itemsCreateInFolder'] = async (ctx, input) => {
	...
	await logEventFromContext(ctx, 'sharepoint.items.createInFolder', { ...input }, 'completed');
	return result;
};
...
export const update: SharepointEndpoints['itemsUpdate'] = async (ctx, input) => {
	...
	await logEventFromContext(ctx, 'sharepoint.items.update', { ...input }, 'completed');
	return { success: true };
};

export const deleteItem: SharepointEndpoints['itemsDelete'] = async (ctx, input) => {
	const siteId = (await ctx.keys.get_site_id()) ?? ctx.options?.siteId ?? '';
	await makeGraphRequest<Record<string, unknown>>(
		`/sites/${siteId}/lists/${encodeURIComponent(input.list_title)}/items/${input.item_id}`,
		ctx.key,
	...
	await logEventFromContext(ctx, 'sharepoint.items.delete', { ...input }, 'completed');
	return { success: true };
};

export const recycle: SharepointEndpoints['itemsRecycle'] = async (ctx, input) => {
	...
	await logEventFromContext(ctx, 'sharepoint.items.recycle', { ...input }, 'completed');
	return { value: undefined };
};

export const getVersion: SharepointEndpoints['itemsGetVersion'] = async (ctx, input) => {
	const siteId = (await ctx.keys.get_site_id()) ?? ctx.options?.siteId ?? '';
	const result = await makeGraphRequest<SharepointEndpointOutputs['itemsGetVersion']>(
		`/sites/${siteId}/lists/${encodeURIComponent(input.list_title)}/items/${input.item_id}/versions/${input.version_id}`,
		ctx.key,
		{ method: 'GET' },
	);

	await logEventFromContext(ctx, 'sharepoint.items.getVersion', { ...input }, 'completed');
	return result;
};

export const getEtag: SharepointEndpoints['itemsGetEtag'] = async (ctx, input) => {
	const siteId = (await ctx.keys.get_site_id()) ?? ctx.options?.siteId ?? '';
	...
	await logEventFromContext(ctx, 'sharepoint.items.getEtag', { ...input }, 'completed');
	return { etag };
};

export const addAttachment: SharepointEndpoints['itemsAddAttachment'] = async (ctx, input) => {
	...
	await logEventFromContext(ctx, 'sharepoint.items.addAttachment', { ...input }, 'completed');
	return { FileName: input.file_name, ServerRelativeUrl: undefined };
};

export const getAttachmentContent: SharepointEndpoints['itemsGetAttachmentContent'] = async (ctx, input) => {
	// Graph API does not support SharePoint list item attachments
	await logEventFromContext(ctx, 'sharepoint.items.getAttachmentContent', { ...input }, 'completed');
	return { content: undefined, fileName: input.file_name };
};

export const listAttachments: SharepointEndpoints['itemsListAttachments'] = async (ctx, input) => {
	// Graph API does not expose SP list item attachments; return an empty list
	await logEventFromContext(ctx, 'sharepoint.items.listAttachments', { ...input }, 'completed');
	return { value: [] };
};
