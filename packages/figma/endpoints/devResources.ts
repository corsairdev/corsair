		...
		try {
			...
			for (const link of result.links_created) {
				if (link.file_key && !seenFileKeys.has(link.file_key)) {
					...
					await ctx.db.fileMetadata.upsertByEntityId(link.file_key, { id: link.file_key });
				}
			}
		} catch (error) {
			console.warn('Failed to save dev resource file metadata to database:', error);
		}
	}
...
export const deleteDevResource: FigmaEndpoints['devResourcesDelete'] = async (ctx, input) => {
	...
	if (ctx.db.fileMetadata) {
		try {
			...
			console.warn('Failed to update file metadata after dev resource deletion:', error);
		}
	}
...
export const get: FigmaEndpoints['devResourcesGet'] = async (ctx, input) => {
	const result = await makeFigmaRequest<FigmaEndpointOutputs['devResourcesGet']>(
		...
		ctx.key,
		{ method: 'GET', query: { node_ids: input.node_ids } },
	);
