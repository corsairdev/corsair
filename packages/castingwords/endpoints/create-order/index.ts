...
export const createOrder: CastingwordsEndpoints['createOrder'] = async (ctx, input) => {
	const response = await makeCastingwordsRequest<unknown>('order_url', ctx.key, {
		method: 'POST',
		form: {
			url: input.url,
			sku: input.sku,
	...
	await logEventFromContext(ctx, 'castingwords.create_order', { url: input.url }, 'completed');
	return parsed;
};
