...
const runtimeProxy = createProxyMiddleware({
	...
	on: {
		...
		error: (err, req, res) => {
			console.error(
				`[proxy] runtime error on ${(req as express.Request).path}: ${(err as Error).message}`,
			);
...
app.use((req, res, next) => {
	if (
		RUNTIME_BYPASS.some((p) => req.path === p || req.path.startsWith(p + '/'))
	) {
...
app.get('/health', (_req, res) =>
	res.json({ ok: true, runtime: getRuntimeStatus().status }),
);
...
app.get('/approve/:token', async (req, res) => {
	...
	const html = corsairPermissions(
		...
		(perm) => ({
			...
			url: `/approve/${perm.token}/approve`,
		}),
...
async function doApproval(
	...
	const { rows } = await pool.query(
		`SELECT id, status FROM corsair_permissions WHERE token = $1`,
		[token],
	);
	const p = rows[0];
	if (!p) {
	...
	if (p.status !== 'pending') {
		res.json({ message: `This request was already ${p.status}.` });
		return;
	}
	await pool.query(
...
app.post(
	...
	(req, res) => void doApproval(req.params.token, 'approve', res),
);
app.post(
	'/approve/:token/deny',
	(req, res) => void doApproval(req.params.token, 'deny', res),
);
...
app.use((req, res, next) => {
	...
	console.log(
		`[auth] unauthenticated — session: ${JSON.stringify(req.session)}, cookie header: ${req.headers.cookie ?? '(none)'}`,
	);
...
app.get('/credentials/:plugin', async (req, res) => {
	...
	try {
		const r = await runtimeFetch('/api/root-keys', {
			method: 'POST',
			body: JSON.stringify({ plugins: [plugin] }),
		});
		rootKeys = r[plugin] ?? {};
	} catch {}
	...
	const flash = req.query.installed
		? `${def.label} installed. Now set up your credentials.`
		: undefined;

	res.setHeader('Content-Type', 'text/html');
	res.send(
		credentialsPage(plugin, def.label, def.authType, rootKeys, accountKeys),
	);
...
app.post('/api/credentials/:plugin/root', async (req, res) => {
	...
	await runtimeFetch('/api/root-keys', {
		method: 'PATCH',
		body: JSON.stringify({ plugin, key: 'client_id', value: client_id }),
	});
	await runtimeFetch('/api/root-keys', {
		method: 'PATCH',
		body: JSON.stringify({
...
app.post('/api/credentials/:plugin/oauth', async (req, res) => {
	...
	try {
		const { url } = await runtimeFetch('/api/oauth/start', {
			method: 'POST',
			body: JSON.stringify({ plugin, tenantId: 'default' }),
		});
		res.redirect(url);
	} catch (err: any) {
...
app.get('/connect', async (req, res) => {
	...
	const newKey = req.session?.newKey as
		| { clientId: string; clientSecret: string }
		| undefined;
	if (req.session?.newKey) req.session!.newKey = undefined;
