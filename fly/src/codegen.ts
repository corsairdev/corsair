...
export const PLUGIN_CATALOG: PluginDef[] = [
	...
	},
	{
		id: 'googledrive',
		label: 'Google Drive',
		package: '@corsair-dev/googledrive',
	...
	},
	{
		id: 'googlesheets',
		label: 'Google Sheets',
		package: '@corsair-dev/googlesheets',
	...
	},
	{
		id: 'jira',
		label: 'Jira',
		package: '@corsair-dev/jira',
		description: 'Manage tickets and sprints',
		authType: 'api_key',
	},
	{
		id: 'hubspot',
		label: 'HubSpot',
		package: '@corsair-dev/hubspot',
	...
	},
	{
		id: 'airtable',
		label: 'Airtable',
		package: '@corsair-dev/airtable',
		description: 'Read and write bases and tables',
		authType: 'api_key',
	},
	{
		id: 'stripe',
		label: 'Stripe',
		package: '@corsair-dev/stripe',
...
export function buildCorsairFile(plugins: PluginsConfig): string {
	...
	const imports = ids.length
		? ids
				.map(
					(id) =>
						`import { ${id} } from "${CATALOG_BY_ID[id]?.package ?? `@corsair-dev/${id}`}";`,
				)
				.join('\n')
		: '';

	const pluginLines = ids
		.map(
			(id) => `    ${id}({ permissions: { mode: "${plugins[id]!.mode}" } }),`,
		)
		.join('\n');
