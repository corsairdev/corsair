...
async function setKey(keysObj: any, key: string, value: string): Promise<void> {
	...
	if (typeof keysObj?.[method] !== 'function')
...
async function ensureAccount(pluginId: string): Promise<void> {
	const internal = (corsair as any)[CORSAIR_INTERNAL] as {
		database: any;
		kek: string;
	};
	...
	if (!integration) {
		...
		integration = await orm.integrations.create({
			name: pluginId,
			config: {},
			dek,
		});
...
setInterval(() => {
	...
	for (const [k, v] of oauthTokens)
		if (v.expiresAt < now) oauthTokens.delete(k);
}, 60_000);
...
async function verifyMcpAuth(
	...
	if (auth?.startsWith('Bearer cors_')) {
		...
		const { rows } = await pool.query(
			`SELECT id FROM mcp_api_keys WHERE key_hash = $1`,
			[keyHash],
		);
		if (!rows[0]) {
...
app.get('/authorize', async (req, res) => {
	...
	const { rows } = await pool.query(
		`SELECT id FROM mcp_api_keys WHERE client_id = $1`,
		[client_id],
	);
	if (!rows[0]) {
...
app.post('/oauth/token', (req, res) => {
	...
	pool
		.query(
			`UPDATE mcp_api_keys SET last_used_at = NOW() WHERE client_id = $1`,
			[entry.clientId],
		)
		.catch(() => {});
...
app.patch('/api/keys', requireAdminKey, async (req, res) => {
	const { plugin, key, value } = req.body as {
		plugin: string;
		key: string;
		value: string;
	};
...
app.post('/api/oauth/start', requireAdminKey, async (req, res) => {
	...
	if (!oauthCfg) {
		res.status(400).json({ error: `${pluginId} has no OAuth config` });
		return;
	}

	const rootKeys = (corsair as any).keys?.[pluginId];
	const clientId = await Promise.resolve(rootKeys?.get_client_id?.()).catch(
		() => null,
	);
	if (!clientId) {
		res.status(400).json({ error: 'client_id not set' });
		return;
	}

	const state = Buffer.from(JSON.stringify({ plugin: pluginId })).toString(
		'base64url',
	);
...
app.get('/api/oauth/callback', async (req, res) => {
	...
	const { plugin: pluginId } = JSON.parse(
		Buffer.from(state, 'base64url').toString(),
	) as { plugin: string };
	const oauthCfg = getOAuthConfig(pluginId);
	if (!oauthCfg) {
		res.status(400).send('Plugin not found');
		return;
	}
	...
	const clientSecret = await Promise.resolve(
		rootKeys?.get_client_secret?.(),
	).catch(() => null);
	if (!clientId || !clientSecret) {
		res.status(400).send('Missing client credentials');
		return;
	}
	...
	if (tokens.access_token)
		await accountKeys?.set_access_token(tokens.access_token as string);
	if (tokens.refresh_token)
		await accountKeys?.set_refresh_token(tokens.refresh_token as string);
	if (typeof tokens.expires_in === 'number') {
		await accountKeys?.set_expires_at(
